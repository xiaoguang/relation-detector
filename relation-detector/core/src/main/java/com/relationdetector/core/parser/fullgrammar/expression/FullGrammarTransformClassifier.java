...
/**
 * CN: 从typed operator/function roles归纳lineage transform类别；输入是parse tree adapter与方言函数语义，
 * 输出是稳定transform名称。上游是expression analyzer，下游是typed event sinks；本类不收集物理列、
 * 不读取terminal文本，也不创建lineage fact。
 *
 * <p>EN: Classifies lineage transforms from typed operator and function roles. Expression analysis is upstream and
 * typed event sinks consume the stable transform name. It does not collect physical columns, inspect terminal text,
 * or create lineage facts.
 */
public final class FullGrammarTransformClassifier {
