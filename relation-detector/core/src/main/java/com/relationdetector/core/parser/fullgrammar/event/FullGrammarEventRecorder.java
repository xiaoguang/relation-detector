...
/**
 * CN: 把单次full-grammar解析中已分类的typed事件构造成sealed event并按稳定键去重；输入来自各focused sink，
 * 输出是有序事件列表。上游是event facade，下游是core parse-result validator；本类不遍历parse tree、
 * 不解析名称，也不生成relationship或lineage事实。
 *
 * <p>EN: Constructs sealed typed events from focused sinks during one full-grammar parse and deduplicates them by a
 * stable key. The event facade is upstream and the core parse-result validator is downstream. It does not traverse
 * parse trees, resolve identifiers, or create relationship/lineage facts.
 */
public final class FullGrammarEventRecorder {
