...
import com.relationdetector.core.parser.fullgrammar.expression.SubqueryProjectionTraceSupport;
...
/**
 * CN: 从typed equality、JOIN、IN与EXISTS context组装predicate事件和guard；输入是direct/subquery traces及当前
 * rowset scope，输出经recorder进入structured parse result。上游是full-grammar facade，下游是relationship
 * extraction；本类不搜索raw SQL、不按名称猜测语法，也不直接创建relationship。
 *
 * <p>EN: Builds predicate events and guards from typed equality, JOIN, IN, and EXISTS contexts using direct/subquery
 * traces and the current rowset scope. The full-grammar facade is upstream and relationship extraction is downstream.
 * It does not search raw SQL, infer syntax from names, or create relationships directly.
 */
public final class PredicateEventSink {
