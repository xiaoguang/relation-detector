...
import com.relationdetector.core.parser.fullgrammar.event.ProjectionEventSink;
...
/**
 * CN: 将typed INSERT/UPDATE/MERGE assignment及控制表达式转换为write mapping事件；输入是方言visitor确认的
 * target和expression context，输出经recorder进入parse result。上游是full-grammar facade，下游是event recorder；
 * 本类不发现statement、不解析raw SQL，也不直接创建lineage fact。
 *
 * <p>EN: Converts typed INSERT/UPDATE/MERGE assignments and control expressions into write-mapping events. Dialect
 * visitors provide confirmed targets and expression contexts, while the event recorder consumes the output. It does
 * not discover statements, parse raw SQL, or create lineage facts directly.
 */
public final class WriteMappingSink {
