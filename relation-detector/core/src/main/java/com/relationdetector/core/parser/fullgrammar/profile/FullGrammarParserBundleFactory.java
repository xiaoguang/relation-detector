...
import com.relationdetector.core.parser.fullgrammar.FullGrammarStructuredSqlParser;

import com.relationdetector.core.parser.fullgrammar.profile.SqlGrammarProfileSelection;

import com.relationdetector.core.parser.fullgrammar.profile.SqlGrammarProfileRegistry;

import com.relationdetector.core.parser.fullgrammar.profile.FullGrammarProfileRequest;
...
import com.relationdetector.core.parser.runtime.ParserSelectionResult;

/**
 * full-grammar SQL/DDL parser bundle factory.
 *
 * <p>CN: 该工厂统一执行 full-grammar profile selection，并同时返回 SQL 与 DDL parser。
 * 选不中 profile 时返回传入的 token-event parser bundle 作为 selection 层 fallback。
 *
 * <p>EN: Unified full-grammar parser bundle factory. It performs profile
 ...
 */
public final class FullGrammarParserBundleFactory {
    private FullGrammarParserBundleFactory() {
    }
    ...
    public static CreatedBundle create(
        ...
        return SqlGrammarProfileRegistry.moduleFor(selection.profile(), modules)
                .map(module -> fullGrammarBundle(selection, module))
                .orElseGet(() -> tokenEventBundle(selection, tokenEventSqlParser, tokenEventDdlParser));
    }

    private static CreatedBundle fullGrammarBundle(
            SqlGrammarProfileSelection selection,
            FullGrammarDialectModule module
    ) {
        StructuredSqlParser sqlParser = new FullGrammarStructuredSqlParser(
                selection,
                module.sqlParser(),
                module.implementationName());
        ParserSelectionResult result = selectionResult(selection, "full-grammar",
