...
import com.relationdetector.oracle.fullgrammar.v26ai.OracleFullGrammarParser;
import com.relationdetector.oracle.fullgrammar.v26ai.OracleFullGrammarParser.*;

final class OracleParseTreeAdapter extends AbstractOracleFullGrammarParseTreeAdapter {
    OracleParseTreeAdapter() {
        super(Map.of(
                        Symbol.PRIMARY, OracleFullGrammarParser.PRIMARY,
                        Symbol.UNIQUE, OracleFullGrammarParser.UNIQUE,
                        Symbol.IN, OracleFullGrammarParser.IN,
                        Symbol.NOT, OracleFullGrammarParser.NOT,
                        Symbol.EXISTS, OracleFullGrammarParser.EXISTS,
                        Symbol.LEFT, OracleFullGrammarParser.LEFT,
                        Symbol.RIGHT, OracleFullGrammarParser.RIGHT,
                        Symbol.FULL, OracleFullGrammarParser.FULL,
                        Symbol.CROSS, OracleFullGrammarParser.CROSS,
                        Symbol.EQUAL, OracleFullGrammarParser.EQUALS_OP),
                ...
                role(Role.DML_TABLE_EXPRESSION, ctx -> ctx instanceof Dml_table_expression_clauseContext),
                role(Role.TABLEVIEW_NAME, ctx -> ctx instanceof Tableview_nameContext), role(Role.TABLE_ALIAS, ctx -> ctx instanceof Table_aliasContext),
                role(Role.COLUMN_ALIAS, ctx -> ctx instanceof Column_aliasContext),
                role(Role.PAREN_COLUMN_LIST, ctx -> ctx instanceof Paren_column_listContext), role(Role.COLUMN_LIST, ctx -> ctx instanceof Column_listContext),
                role(Role.COLUMN_NAME, ctx -> ctx instanceof Column_nameContext), role(Role.REFERENCES_CLAUSE, ctx -> ctx instanceof References_clauseContext),
                role(Role.TABLE_INDEX_CLAUSE, ctx -> ctx instanceof Table_index_clauseContext), role(Role.INDEX_EXPRESSION, ctx -> ctx instanceof Index_exprContext),
                role(Role.INLINE_CONSTRAINT, ctx -> ctx instanceof Inline_constraintContext), role(Role.SCHEMA_NAME, ctx -> ctx instanceof Schema_nameContext),
                role(Role.IDENTIFIER, ctx -> ctx instanceof IdentifierContext), role(Role.QUOTED_STRING, ctx -> ctx instanceof Quoted_stringContext),
                role(Role.SYSTEM_VALUE, ctx -> ctx instanceof Oracle_system_valueContext),
                role(Role.SELECT_LIST_ELEMENT, ctx -> ctx instanceof Select_list_elementsContext), role(Role.SELECTED_LIST, ctx -> ctx instanceof Selected_listContext),
                role(Role.EXPRESSION, ctx -> ctx instanceof ExpressionContext), role(Role.SIMPLE_CASE_EXPRESSION, ctx -> ctx instanceof Simple_case_expressionContext),
                role(Role.SEARCHED_CASE_EXPRESSION, ctx -> ctx instanceof Searched_case_expressionContext),
                ...
                role(Role.TABLE_REF_INTERNAL_WRAPPER, ctx -> ctx instanceof Table_ref_aux_internalContext),
                role(Role.GENERAL_ELEMENT_PART, ctx -> ctx instanceof General_element_partContext), role(Role.FUNCTION_ARGUMENT, ctx -> ctx instanceof Function_argumentContext),
                role(Role.WINDOW_CLAUSE, ctx -> ctx instanceof Over_clauseContext),
                role(Role.QUERY_NAME, ctx -> ctx instanceof Query_nameContext), role(Role.FROM_CLAUSE, ctx -> ctx instanceof From_clauseContext),
                role(Role.HIERARCHICAL_QUERY_CLAUSE, ctx -> ctx instanceof Hierarchical_query_clauseContext), role(Role.GROUP_BY_CLAUSE, ctx -> ctx instanceof Group_by_clauseContext),
                role(Role.MODEL_CLAUSE, ctx -> ctx instanceof Model_clauseContext), role(Role.RELATIONAL_OPERATOR, ctx -> ctx instanceof Relational_operatorContext),
                role(Role.IN_ELEMENTS, ctx -> ctx instanceof In_elementsContext), role(Role.SELECT_ONLY_STATEMENT, ctx -> ctx instanceof Select_only_statementContext),
                role(Role.INSERT_INTO_CLAUSE, ctx -> ctx instanceof Insert_into_clauseContext), role(Role.MERGE_UPDATE_CLAUSE, ctx -> ctx instanceof Merge_update_clauseContext),
                role(Role.MERGE_ELEMENT, ctx -> ctx instanceof Merge_elementContext), role(Role.CONDITION, ctx -> ctx instanceof ConditionContext),
                role(Role.TABLE_NAME, ctx -> ctx instanceof Table_nameContext), role(Role.FOREIGN_KEY_CLAUSE, ctx -> ctx instanceof Foreign_key_clauseContext));
    }
