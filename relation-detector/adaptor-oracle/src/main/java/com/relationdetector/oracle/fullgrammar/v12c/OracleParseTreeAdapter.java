...
final class OracleParseTreeAdapter extends AbstractOracleFullGrammarParseTreeAdapter {
    OracleParseTreeAdapter() {
        super(Map.of(Symbol.PRIMARY, OracleFullGrammarParser.PRIMARY, Symbol.UNIQUE, OracleFullGrammarParser.UNIQUE,
                ...
                role(Role.CREATE_TABLE, ctx -> ctx instanceof Create_tableContext),
                role(Role.ALTER_TABLE, ctx -> ctx instanceof Alter_tableContext),
                ...
                role(Role.FOREIGN_KEY, ctx -> ctx instanceof Foreign_key_clauseContext),
                role(Role.CREATE_INDEX, ctx -> ctx instanceof Create_indexContext),
                role(Role.SELECT_STATEMENT, ctx -> ctx instanceof Select_statementContext),
                role(Role.QUERY_BLOCK, ctx -> ctx instanceof Query_blockContext),
                role(Role.TABLE_REF_AUX, ctx -> ctx instanceof Table_ref_auxContext),
                ...
                role(Role.SELECTED_TABLEVIEW, ctx -> ctx instanceof Selected_tableviewContext),
                role(Role.JOIN_CLAUSE, ctx -> ctx instanceof Join_clauseContext),
                role(Role.JOIN_ON, ctx -> ctx instanceof Join_on_partContext),
                role(Role.JOIN_USING, ctx -> ctx instanceof Join_using_partContext),
                role(Role.WHERE_CLAUSE, ctx -> ctx instanceof Where_clauseContext),
                role(Role.GROUP_BY_ELEMENT, ctx -> ctx instanceof Group_by_elementsContext),
                ...
                role(Role.SUBQUERY, ctx -> ctx instanceof SubqueryContext),
                role(Role.COLUMN_REFERENCE, ctx -> ctx instanceof Column_nameContext
                        || ctx instanceof Table_elementContext),
                role(Role.GENERAL_ELEMENT, ctx -> ctx instanceof General_elementContext),
