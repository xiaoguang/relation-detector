...
/*
 * PostgreSQL token-event structural grammar.
 *
 * This grammar covers the portable token-event subset plus PostgreSQL-specific lexical and structural extensions:
 ...
 * DELETE ... WHERE. Full PostgreSQL version syntax belongs in full-grammar; this grammar is the token-event fallback subset.
 */
...
routineParameter
    : (IN | OUT | INOUT | VARIADIC)? identifier routineParameterToken*
    ;

routineParameterToken
    : LPAREN routineParameterNestedToken* RPAREN
    | ~(COMMA | RPAREN | LPAREN)
    ;

routineParameterNestedToken
    : LPAREN routineParameterNestedToken* RPAREN
    | ~(RPAREN | LPAREN)
    ;

routineDeclarationElement
    : AS DOLLAR_QUOTED_STRING
    | LANGUAGE identifier
    | LPAREN routineDeclarationElement* RPAREN
...
querySpecification
    : SELECT distinctClause? selectList fromClause? whereClause? groupByClause? havingClause? orderByClause? limitClause?
    ;
...
insertSelectStatement
    : INSERT INTO qualifiedName LPAREN identifierList RPAREN selectStatement onConflictClause?
    ;

insertValuesStatement
    : INSERT INTO qualifiedName LPAREN identifierList RPAREN VALUES valuesRow (COMMA valuesRow)* onConflictClause?
    ;
