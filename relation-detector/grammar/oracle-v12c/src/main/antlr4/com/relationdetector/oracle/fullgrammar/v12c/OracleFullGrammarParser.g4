/**
 * Oracle PL/SQL grammar vendored from antlr/grammars-v4 commit 994628b6d261f5313b72e76039818549352684ce
 *
...
parser grammar OracleFullGrammarParser;

options {
    tokenVocab = OracleFullGrammarLexer;
    superClass = OracleFullGrammarParserBase;
}
...
sql_macro_body
    : {false}? SQL_MACRO ('(' id_expression? ')')? IS BEGIN RETURN quoted_string SEMICOLON END
    ;
...
create_sequence
    : CREATE SEQUENCE sequence_name sequence_spec* (SHARING '=' (METADATA | DATA | NONE))?
    ;
...
create_view
    : CREATE (OR REPLACE)? (NO? FORCE)? editioning_clause? VIEW (schema_name '.')? v = id_expression ()? (SHARING '=' (METADATA | EXTENDED? DATA | NONE))? view_options? (
...
memoptimize_read_write_clause
    : {false}? NO? MEMOPTIMIZE FOR (READ | WRITE)
    ;
