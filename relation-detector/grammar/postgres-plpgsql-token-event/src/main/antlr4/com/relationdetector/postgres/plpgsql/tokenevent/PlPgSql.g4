...
/*
 * PostgreSQL routine-body structural grammar.
 *
 ...
 * DELETE ... WHERE. This grammar is local to the PostgreSQL dialect and parses
 * PL/pgSQL routine bodies after the outer parser extracts the CREATE
...
declarationItem
    : identifier declarationItemToken* SEMI
    ;

declarationItemToken
    : LPAREN declarationItemToken* RPAREN
    | ~(SEMI | BEGIN | LPAREN | RPAREN)
    ;
...
dynamicExecuteStatement
    : EXECUTE dynamicSqlToken* SEMI?
    ;

dynamicSqlToken
    : LPAREN dynamicSqlToken* RPAREN
    | ~(SEMI | LPAREN | RPAREN)
    ;
...
querySpecification
    : SELECT distinctClause? selectList intoClause? fromClause? whereClause? groupByClause? havingClause? orderByClause? limitClause?
    ;
...
sqlToken
    ...
    | IS
    | USING | GROUP | BY | HAVING | ORDER | LIMIT | INSERT | INTO | UPDATE
    | SET | DELETE | MERGE | MATCHED | VALUES | RETURNING | EXECUTE | DO | NOTHING
