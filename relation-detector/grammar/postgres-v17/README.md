...
Vendored for the `postgresql-17` full-grammar parser. PostgreSQL does not
...
- Renamed `PostgreSQLLexer` to `PostgresFullGrammarLexer`.
- Renamed `PostgreSQLParser` to `PostgresFullGrammarParser`.
- Updated `tokenVocab` to reference the renamed lexer.
- Java helper/base classes live under
  `com.relationdetector.postgres.fullgrammar.v17`.
...
This grammar backs the PostgreSQL 17 full-grammar profile. Runtime selection is
