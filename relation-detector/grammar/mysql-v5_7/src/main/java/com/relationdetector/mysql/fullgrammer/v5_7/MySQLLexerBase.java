...
public abstract class MySQLLexerBase extends Lexer {
    ...
    public boolean supportMle = false;
    ...
    protected int determineFunction(int proposed) {
        ...
        if (this.isSqlModeActive(MySqlGrammarSqlMode.IgnoreSpace)) {
            while (input == ' ' || input == '\t' || input == '\r' || input == '\n') {
                ...
                this._type = MySqlFullGrammerLexer.WHITESPACE;
                input = (char) this._input.LA(1);
            }
        }
        return input == '(' ? proposed : MySqlFullGrammerLexer.IDENTIFIER;
    }

    protected int determineNumericType(String text) {
        int length = text.length() - 1;
        if (length < MySQLLexerBase.longLength) {
            return MySqlFullGrammerLexer.INT_NUMBER;
        }
        ...
        if (negative) {
            if (length == MySQLLexerBase.longLength) {
                ...
                smaller = MySqlFullGrammerLexer.INT_NUMBER;
                bigger = MySqlFullGrammerLexer.LONG_NUMBER;
            } else if (length < MySQLLexerBase.signedLongLongLength) {
                return MySqlFullGrammerLexer.LONG_NUMBER;
            } else if (length > MySQLLexerBase.signedLongLongLength) {
                return MySqlFullGrammerLexer.DECIMAL_NUMBER;
            } else {
                cmp = MySQLLexerBase.signedLongLongString.substring(1);
                smaller = MySqlFullGrammerLexer.LONG_NUMBER;
                bigger = MySqlFullGrammerLexer.DECIMAL_NUMBER;
            }
        } else {
            if (length == MySQLLexerBase.longLength) {
                ...
                smaller = MySqlFullGrammerLexer.ULONGLONG_NUMBER;
                bigger = MySqlFullGrammerLexer.DECIMAL_NUMBER;
            } else {
                cmp = MySQLLexerBase.longLongString;
                smaller = MySqlFullGrammerLexer.LONG_NUMBER;
                bigger = MySqlFullGrammerLexer.ULONGLONG_NUMBER;
            }
        }
    ...
    protected int checkCharset(String text) {
        return this.charSets.contains(text) ? MySqlFullGrammerLexer.UNDERSCORE_CHARSET : MySqlFullGrammerLexer.IDENTIFIER;
    }

    protected void emitDot() {
        var len = this.getText().length();
        pendingTokens.add(this._factory.create(this._tokenFactorySourcePair, MySqlFullGrammerLexer.DOT_SYMBOL,
    ...
    public void doLogicalOr() { this._type = isSqlModeActive(MySqlGrammarSqlMode.PipesAsConcat) ? MySqlFullGrammerLexer.CONCAT_PIPES_SYMBOL : MySqlFullGrammerLexer.LOGICAL_OR_OPERATOR; }
    public void doIntNumber() { this._type = determineNumericType(this.getText()); }
    public void doAdddate() { this._type = determineFunction(MySqlFullGrammerLexer.ADDDATE_SYMBOL); }
    public void doBitAnd() { this._type = determineFunction(MySqlFullGrammerLexer.BIT_AND_SYMBOL); }
    public void doBitOr() { this._type = determineFunction(MySqlFullGrammerLexer.BIT_OR_SYMBOL); }
    public void doBitXor() { this._type = determineFunction(MySqlFullGrammerLexer.BIT_XOR_SYMBOL); }
    public void doCast() { this._type = determineFunction(MySqlFullGrammerLexer.CAST_SYMBOL); }
    public void doCount() { this._type = determineFunction(MySqlFullGrammerLexer.COUNT_SYMBOL); }
    public void doCurdate() { this._type = determineFunction(MySqlFullGrammerLexer.CURDATE_SYMBOL); }
    public void doCurrentDate() { this._type = determineFunction(MySqlFullGrammerLexer.CURDATE_SYMBOL); }
    public void doCurrentTime() { this._type = determineFunction(MySqlFullGrammerLexer.CURTIME_SYMBOL); }
    public void doCurtime() { this._type = determineFunction(MySqlFullGrammerLexer.CURTIME_SYMBOL); }
    public void doDateAdd() { this._type = determineFunction(MySqlFullGrammerLexer.DATE_ADD_SYMBOL); }
    public void doDateSub() { this._type = determineFunction(MySqlFullGrammerLexer.DATE_SUB_SYMBOL); }
    public void doExtract() { this._type = determineFunction(MySqlFullGrammerLexer.EXTRACT_SYMBOL); }
    public void doGroupConcat() { this._type = determineFunction(MySqlFullGrammerLexer.GROUP_CONCAT_SYMBOL); }
    public void doMax() { this._type = determineFunction(MySqlFullGrammerLexer.MAX_SYMBOL); }
    public void doMid() { this._type = determineFunction(MySqlFullGrammerLexer.SUBSTRING_SYMBOL); }
    public void doMin() { this._type = determineFunction(MySqlFullGrammerLexer.MIN_SYMBOL); }
    public void doNot() { this._type = isSqlModeActive(MySqlGrammarSqlMode.HighNotPrecedence) ? MySqlFullGrammerLexer.NOT2_SYMBOL : MySqlFullGrammerLexer.NOT_SYMBOL; }
    public void doNow() { this._type = determineFunction(MySqlFullGrammerLexer.NOW_SYMBOL); }
    public void doPosition() { this._type = determineFunction(MySqlFullGrammerLexer.POSITION_SYMBOL); }
    public void doSessionUser() { this._type = determineFunction(MySqlFullGrammerLexer.USER_SYMBOL); }
    public void doStddevSamp() { this._type = determineFunction(MySqlFullGrammerLexer.STDDEV_SAMP_SYMBOL); }
    public void doStddev() { this._type = determineFunction(MySqlFullGrammerLexer.STD_SYMBOL); }
    public void doStddevPop() { this._type = determineFunction(MySqlFullGrammerLexer.STD_SYMBOL); }
    public void doStd() { this._type = determineFunction(MySqlFullGrammerLexer.STD_SYMBOL); }
    public void doSubdate() { this._type = determineFunction(MySqlFullGrammerLexer.SUBDATE_SYMBOL); }
    public void doSubstr() { this._type = determineFunction(MySqlFullGrammerLexer.SUBSTRING_SYMBOL); }
    public void doSubstring() { this._type = determineFunction(MySqlFullGrammerLexer.SUBSTRING_SYMBOL); }
    public void doSum() { this._type = determineFunction(MySqlFullGrammerLexer.SUM_SYMBOL); }
    public void doSysdate() { this._type = determineFunction(MySqlFullGrammerLexer.SYSDATE_SYMBOL); }
    public void doSystemUser() { this._type = determineFunction(MySqlFullGrammerLexer.USER_SYMBOL); }
    public void doTrim() { this._type = determineFunction(MySqlFullGrammerLexer.TRIM_SYMBOL); }
    public void doVariance() { this._type = determineFunction(MySqlFullGrammerLexer.VARIANCE_SYMBOL); }
    public void doVarPop() { this._type = determineFunction(MySqlFullGrammerLexer.VARIANCE_SYMBOL); }
    public void doVarSamp() { this._type = determineFunction(MySqlFullGrammerLexer.VAR_SAMP_SYMBOL); }
