...
/**
 * CN: 为 MySQL 5.7 generated lexer actions 提供版本号、SQL mode、版本注释和复合 token 支持；它接收字符流并维护单次 lexer 状态，不生成结构化事实。
 * EN: Supplies MySQL 5.7 generated lexer actions with server version, SQL modes, version-comment handling, and compound-token support; it owns per-lexer state and emits no structured facts.
 *
