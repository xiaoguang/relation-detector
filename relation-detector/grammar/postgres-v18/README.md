...
Vendored for the `postgresql-18` full-grammar parser. PostgreSQL does not
...
- Java helper/base classes live under
  `com.relationdetector.postgres.fullgrammar.v18`.
- Adds PostgreSQL 18 temporal column syntax in `columnElem`:
  `PERIOD? colid (WITHOUT OVERLAPS)?`.

This grammar backs the PostgreSQL 18 full-grammar profile. Runtime selection is
