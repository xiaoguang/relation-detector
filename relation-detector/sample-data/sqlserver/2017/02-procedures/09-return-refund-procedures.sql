...
BEGIN
    ...
    USING (
        ...
        FROM [dbo].[employees] AS p
        INNER JOIN [dbo].[purchase_returns] AS c2 ON c2.[handler_id] = p.[id]
    ) AS src
...
-- relation-detector-fixture-source:sqlserver.sp_09_return_refund_procedures_2
CREATE OR ALTER PROCEDURE [dbo].[sp_09_return_refund_procedures_2]
AS
BEGIN
    ...
    USING (
        SELECT p.[id] AS [id],
               ...
               COUNT(*) OVER (PARTITION BY p.[id]) AS [row_rank]
        FROM [dbo].[employees] AS p
        INNER JOIN [dbo].[purchase_returns] AS c2 ON c2.[approved_by] = p.[id]
    ) AS src
...
-- relation-detector-fixture-source:sqlserver.sp_09_return_refund_procedures_3
CREATE OR ALTER PROCEDURE [dbo].[sp_09_return_refund_procedures_3]
AS
BEGIN
    ...
    USING (
        SELECT p.[id] AS [id],
               ...
               COUNT(*) OVER (PARTITION BY p.[id]) AS [row_rank]
        FROM [dbo].[vouchers] AS p
        INNER JOIN [dbo].[purchase_returns] AS c2 ON c2.[refund_voucher_id] = p.[id]
    ) AS src
...
-- relation-detector-fixture-source:sqlserver.sp_09_return_refund_procedures_4
CREATE OR ALTER PROCEDURE [dbo].[sp_09_return_refund_procedures_4]
AS
BEGIN
    ...
    WHERE p.[id] IN (SELECT p2.[id] FROM [dbo].[purchase_returns] AS p2);
    ...
    MERGE INTO [dbo].[purchase_return_items] AS c
    USING (
        SELECT p.[id] AS [id],
               CASE WHEN p.[id] IS NULL THEN p.[id] ELSE p.[id] END AS [mapped_id],
               COUNT(*) OVER (PARTITION BY p.[id]) AS [row_rank]
        FROM [dbo].[purchase_returns] AS p
        INNER JOIN [dbo].[purchase_return_items] AS c2 ON c2.[return_id] = p.[id]
    ) AS src
...
-- relation-detector-fixture-source:sqlserver.sp_09_return_refund_procedures_5
CREATE OR ALTER PROCEDURE [dbo].[sp_09_return_refund_procedures_5]
AS
BEGIN
    ...
    USING (
        SELECT p.[id] AS [id],
               ...
               COUNT(*) OVER (PARTITION BY p.[id]) AS [row_rank]
        FROM [dbo].[products] AS p
        INNER JOIN [dbo].[purchase_return_items] AS c2 ON c2.[product_id] = p.[id]
    ) AS src
...
-- relation-detector-fixture-source:sqlserver.sp_09_return_refund_procedures_6
CREATE OR ALTER PROCEDURE [dbo].[sp_09_return_refund_procedures_6]
AS
BEGIN
    ...
    USING (
        SELECT p.[id] AS [id],
               ...
               COUNT(*) OVER (PARTITION BY p.[id]) AS [row_rank]
        FROM [dbo].[product_batches] AS p
        INNER JOIN [dbo].[purchase_return_items] AS c2 ON c2.[batch_id] = p.[id]
    ) AS src
...
-- relation-detector-fixture-source:sqlserver.sp_09_return_refund_procedures_7
CREATE OR ALTER PROCEDURE [dbo].[sp_09_return_refund_procedures_7]
AS
BEGIN
    ...
    USING (
        SELECT p.[id] AS [id],
               ...
               COUNT(*) OVER (PARTITION BY p.[id]) AS [row_rank]
        FROM [dbo].[warehouses] AS p
        INNER JOIN [dbo].[damage_reports] AS c2 ON c2.[warehouse_id] = p.[id]
    ) AS src
...
-- relation-detector-fixture-source:sqlserver.sp_09_return_refund_procedures_8
CREATE OR ALTER PROCEDURE [dbo].[sp_09_return_refund_procedures_8]
AS
BEGIN
    ...
    USING (
        SELECT p.[id] AS [id],
               ...
               COUNT(*) OVER (PARTITION BY p.[id]) AS [row_rank]
        FROM [dbo].[employees] AS p
        INNER JOIN [dbo].[damage_reports] AS c2 ON c2.[reported_by] = p.[id]
    ) AS src
    ON c.[reported_by] = src.[id]
    WHEN MATCHED THEN UPDATE SET c.[reported_by] = ISNULL(src.[mapped_id], src.[id]);
END;
-- relation-detector-fixture-end

-- relation-detector-fixture-source:sqlserver.sp_09_return_refund_procedures_9
CREATE OR ALTER PROCEDURE [dbo].[sp_09_return_refund_procedures_9]
AS
BEGIN
    ...
    USING (
        SELECT p.[id] AS [id],
               ...
               COUNT(*) OVER (PARTITION BY p.[id]) AS [row_rank]
        FROM [dbo].[employees] AS p
        INNER JOIN [dbo].[damage_reports] AS c2 ON c2.[approved_by] = p.[id]
    ) AS src
    ON c.[approved_by] = src.[id]
    WHEN MATCHED THEN UPDATE SET c.[approved_by] = ISNULL(src.[mapped_id], src.[id]);
END;
...
-- relation-detector-fixture-source:sqlserver.sp_09_return_refund_procedures_10
CREATE OR ALTER PROCEDURE [dbo].[sp_09_return_refund_procedures_10]
AS
BEGIN
    ...
    USING (
        SELECT p.[id] AS [id],
               ...
               COUNT(*) OVER (PARTITION BY p.[id]) AS [row_rank]
        FROM [dbo].[employees] AS p
        INNER JOIN [dbo].[damage_reports] AS c2 ON c2.[executed_by] = p.[id]
    ) AS src
    ON c.[executed_by] = src.[id]
    WHEN MATCHED THEN UPDATE SET c.[executed_by] = ISNULL(src.[mapped_id], src.[id]);
END;
-- relation-detector-fixture-end
