...
-- ============================================================
-- 69. Haversine公式计算两点间距离(km)
-- 调用方: 供应商选择、物流费用估算
-- 计算公式: a = sin²(Δlat/2) + cos(lat1)*cos(lat2)*sin²(Δlon/2)
--           c = 2*atan2(√a, √(1-a))
--           d = R*c (R=6371km)
-- ============================================================
...
RETURN NUMBER
AS
v_r        NUMBER(10,2)  DEFAULT 6371.0;
    v_dlat     NUMBER(15,10);
    v_dlon     NUMBER(15,10);
    v_a        NUMBER(15,10);
    v_c        NUMBER(15,10);
    v_distance NUMBER(10,2);
BEGIN
...
AS
v_distance      NUMBER(10,2);
    v_rate_per_km   NUMBER(8,4);
    v_weight_kg     NUMBER(8,3);
    v_shipping_cost NUMBER(12,2);
    v_supplier_lat  NUMBER(10,7);
    v_supplier_lon  NUMBER(10,7);
    v_wh_lat        NUMBER(10,7);
    v_wh_lon        NUMBER(10,7);
BEGIN
...
RETURN NUMBER
AS
v_price_score      NUMBER(5,2);
    v_distance_score   NUMBER(5,2);
    v_return_score     NUMBER(5,2);
    v_quality_score    NUMBER(5,2);
    v_lead_time_score  NUMBER(5,2);
    v_total_score      NUMBER(5,2);
    v_supplier_price   NUMBER(12,2);
    v_avg_price        NUMBER(12,2);
    v_distance         NUMBER(10,2);
    v_return_rate      NUMBER(5,4);
    v_quality          NUMBER(5,2);
...
--   4. 标记黑名单/暂停供应商为不可用
--   5. 按综合评分降序排列
-- 注意: Oracle过程不能直接返回结果集，改为函数返回TABLE
-- ============================================================

CREATE OR REPLACE PROCEDURE sp_find_best_supplier(
p_product_id   NUMBER,
    p_warehouse_id NUMBER,
    p_quantity     NUMBER,
...
CREATE OR REPLACE PROCEDURE sp_compare_suppliers_for_product(
p_product_id NUMBER,
    p_result OUT SYS_REFCURSOR
)
AS
v_avg_price    NUMBER(12,2);
    v_lowest_price NUMBER(12,2);
BEGIN
...
CREATE OR REPLACE PROCEDURE sp_supplier_geographic_analysis(
p_warehouse_id NUMBER,
    p_result OUT SYS_REFCURSOR
)
AS
BEGIN
    ...
    SELECT
        ...
        s.cooperation_status
    FROM warehouses w
    JOIN supplier_products sp ON 1
...
BEGIN
    ...
    SET return_rate = COALESCE((
        SELECT SUM(pri.return_qty) * 1.0 / NULLIF(SUM(pri.return_qty) + (
            ...
            WHERE poi.product_id = sp.product_id AND po.supplier_id = sp.supplier_id
              AND po.order_date >= CURRENT_DATE - INTERVAL '12' MONTH
        ), 0)
        ...
        WHERE pr.supplier_id = sp.supplier_id
          ...
          AND pr.return_date >= CURRENT_DATE - INTERVAL '12' MONTH
    ), 0);
    ...
    SET quality_score = COALESCE((
        ...
        WHERE pb.supplier_id = sp.supplier_id
          ...
          AND ir.inspection_date >= CURRENT_DATE - INTERVAL '12' MONTH
    ), 100);
