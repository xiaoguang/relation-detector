...
WHERE so.warehouse_id = 1 AND so.order_date >= CURRENT_DATE - INTERVAL '30' DAY
...
WHERE sr.warehouse_id = 1 AND sr.return_date >= CURRENT_DATE - INTERVAL '30' DAY
...
WHERE dr.warehouse_id = 1 AND dr.report_date >= CURRENT_DATE - INTERVAL '30' DAY
...
WHERE po.status = 'received' AND po.order_date >= CURRENT_DATE - INTERVAL '30' DAY
...
SELECT
    ...
    ai.total_nodes,
    (ai.current_node_level || '/' || ai.total_nodes) AS progress,
...
SELECT
    ...
    p.weight_kg,
    -- 各门店库存
    (SELECT LISTAGG((w.name || ':' || COALESCE(i.quantity, 0)), ' | ') WITHIN GROUP (ORDER BY (w.name || ':' || COALESCE(i.quantity, 0)))
     FROM warehouses w
    ...
    -- 批号列表(含过期日期)
    (SELECT LISTAGG((pb.batch_no || '(' || pb.current_qty || ',到期:' || pb.expiry_date || ')'), '; ') WITHIN GROUP (ORDER BY pb.expiry_date)
     FROM product_batches pb WHERE pb.product_id = p.id AND pb.current_qty > 0 AND pb.status = 'active'
    ) AS batch_list,
    -- 供应商列表(含价格)
    (SELECT LISTAGG((s.name || '(' || s.city || '):¥' || sp.supplier_price || ' 交期:' || sp.lead_time_days || '天'), ' | ') WITHIN GROUP (ORDER BY sp.supplier_price)
    ...
    (SELECT COALESCE(SUM(soi.quantity), 0)
     ...
     WHERE soi.product_id = p.id AND so.order_date >= CURRENT_DATE - INTERVAL '30' DAY
       AND so.status NOT IN ('draft', 'cancelled')
    ) AS sales_30d,
    p.status
FROM products p
...
SELECT
    ...
    CASE
        ...
        WHEN pb.expiry_date <= CURRENT_DATE + INTERVAL '30' DAY THEN '临期'
        ELSE '正常'
    END AS stock_status,
    -- 近30天销量
...
LEFT JOIN (
    ...
    FROM sales_orders
    WHERE order_date BETWEEN CURRENT_DATE - INTERVAL '14' DAY AND CURRENT_DATE - INTERVAL '8' DAY
      AND status NOT IN ('draft', 'cancelled')
    GROUP BY order_date
) last_week ON dates.d = last_week.order_date + INTERVAL '7' DAY
...
SELECT
    ...
    (SELECT total_score FROM performance_reviews
     WHERE employee_id = e.id ORDER BY review_period DESC FETCH FIRST 1 ROWS ONLY) AS latest_performance,
    (SELECT grade FROM performance_reviews
     WHERE employee_id = e.id ORDER BY review_period DESC FETCH FIRST 1 ROWS ONLY) AS latest_grade,
    -- 本月出勤率
    ...
    (SELECT ROUND(AVG(net_pay), 2) FROM salary_payments
     WHERE employee_id = e.id ORDER BY salary_month DESC FETCH FIRST 3 ROWS ONLY) AS avg_salary_3m,
    -- 最近一次调薪
    (SELECT (TO_CHAR(effective_date, 'YYYY-MM-DD') || ': ' || old_salary || '->' || new_salary)
     FROM employee_salary_log WHERE employee_id = e.id ORDER BY effective_date DESC FETCH FIRST 1 ROWS ONLY
...
SELECT
    ...
    -- 质检结果
    (SELECT LISTAGG((ir.inspection_result || '(' || ROUND(ir.defect_rate, 1) || '%)'), '; ') WITHIN GROUP (ORDER BY (ir.inspection_result || '(' || ROUND(ir.defect_rate, 1) || '%)'))
     FROM inspection_reports ir WHERE ir.reference_id = po.id AND ir.reference_type = 'purchase_order'
    ) AS qc_results,
    -- 退货
    (SELECT LISTAGG((prt.return_no || ':' || prt.status), '; ') WITHIN GROUP (ORDER BY (prt.return_no || ':' || prt.status))
...
WHERE po.order_date >= CURRENT_DATE - INTERVAL '90' DAY
...
WHERE al.created_at >= CURRENT_DATE - INTERVAL '7' DAY
...
WHERE sr.return_date >= CURRENT_DATE - INTERVAL '90' DAY
...
SELECT
    ...
    -- 推荐供应商
    (SELECT (s.name || '(¥' || sp.supplier_price || '/交期' || sp.lead_time_days || '天)')
     FROM supplier_products sp JOIN suppliers s ON sp.supplier_id = s.id
     WHERE sp.product_id = p.id AND s.cooperation_status = 'active'
     ORDER BY sp.supplier_price ASC FETCH FIRST 1 ROWS ONLY
