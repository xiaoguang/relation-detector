...
SELECT
    e.department_id,
    DATE_FORMAT(sp.payment_date, '%Y-%m') AS salary_month,
    SUM(sp.net_pay) AS net_pay,
    SUM(sp.base_salary + sp.bonus - sp.deduction) AS gross_pay
FROM salary_payments sp
JOIN employees e ON sp.employee_id = e.id
GROUP BY e.department_id, DATE_FORMAT(sp.payment_date, '%Y-%m');
