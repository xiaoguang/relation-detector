...
SELECT
    ...
    (SELECT GROUP_CONCAT(DISTINCT w.name ORDER BY w.name SEPARATOR ', ')
     FROM inventory i2
...
SELECT
    CASE
        ...
        WHEN pb.expiry_date <= DATE_ADD(CURDATE(), INTERVAL 30 DAY) THEN '30天内过期'
        WHEN pb.expiry_date <= DATE_ADD(CURDATE(), INTERVAL 60 DAY) THEN '60天内过期'
        WHEN pb.expiry_date <= DATE_ADD(CURDATE(), INTERVAL 90 DAY) THEN '90天内过期'
        ELSE '90天后过期'
    ...
    GROUP_CONCAT(
        DISTINCT CONCAT(p.sku, ':', pb.batch_no, '(', pb.current_qty, ')')
...
WITH product_sales AS (
    ...
    JOIN product_categories pc ON p.category_id = pc.id
    WHERE so.order_date >= DATE_SUB(CURDATE(), INTERVAL 180 DAY)
      AND so.status NOT IN ('draft', 'cancelled')
...
SELECT
    ...
    (SELECT AVG(sp.lead_time_days) FROM supplier_products sp WHERE sp.product_id = rp.product_id AND sp.is_preferred = TRUE) AS lead_time_days
...
WITH monthly_customer_sales AS (
    ...
    WHERE so.order_date >= DATE_SUB(CURDATE(), INTERVAL 6 MONTH)
      AND so.status NOT IN ('draft', 'cancelled')
    GROUP BY so.customer_id, c.name, c.membership_level, DATE_FORMAT(so.order_date, '%Y-%m')
),
...
WITH rfm_base AS (
    SELECT
        so.customer_id,
        c.name AS customer_name,
        DATEDIFF(CURDATE(), MAX(so.order_date)) AS recency_days,
        COUNT(DISTINCT so.id) AS frequency,
        SUM(so.total_amount) AS monetary,
        DATEDIFF(CURDATE(), MIN(so.order_date)) AS customer_lifetime_days
    FROM sales_orders so
    JOIN customers c ON so.customer_id = c.id
...
SELECT
    ...
    CASE
        WHEN DATEDIFF(CURDATE(), last_sale_date) > 180 THEN '严重滞销(>180天)'
        WHEN DATEDIFF(CURDATE(), last_sale_date) > 90 THEN '滞销(>90天)'
        ELSE '近期滞销'
...
WITH current_month AS (
    ...
    FROM sales_order_items soi
    JOIN sales_orders so ON soi.order_id = so.id
    WHERE DATE_FORMAT(so.order_date, '%Y-%m') = DATE_FORMAT(CURDATE(), '%Y-%m')
      AND so.status NOT IN ('draft', 'cancelled')
    GROUP BY soi.product_id
),
last_month AS (
    ...
    FROM sales_order_items soi
    JOIN sales_orders so ON soi.order_id = so.id
    WHERE DATE_FORMAT(so.order_date, '%Y-%m') = DATE_FORMAT(DATE_SUB(CURDATE(), INTERVAL 1 MONTH), '%Y-%m')
      AND so.status NOT IN ('draft', 'cancelled')
    GROUP BY soi.product_id
),
same_month_last_year AS (
    ...
    FROM sales_order_items soi
    JOIN sales_orders so ON soi.order_id = so.id
    WHERE DATE_FORMAT(so.order_date, '%Y-%m') = DATE_FORMAT(DATE_SUB(CURDATE(), INTERVAL 1 YEAR), '%Y-%m')
      AND so.status NOT IN ('draft', 'cancelled')
    GROUP BY soi.product_id
)
SELECT
...
WITH monthly_turnover AS (
    SELECT
        ...
        e.gender,
        CASE
            WHEN TIMESTAMPDIFF(YEAR, e.hire_date, e.resignation_date) <= 1 THEN '1年内'
            WHEN TIMESTAMPDIFF(YEAR, e.hire_date, e.resignation_date) <= 3 THEN '1-3年'
            WHEN TIMESTAMPDIFF(YEAR, e.hire_date, e.resignation_date) <= 5 THEN '3-5年'
            WHEN TIMESTAMPDIFF(YEAR, e.hire_date, e.resignation_date) <= 10 THEN '5-10年'
            ELSE '10年以上'
        END AS tenure_group,
        DATE_FORMAT(e.resignation_date, '%Y-%m') AS resign_month,
        COUNT(*) AS resigned_count
    FROM employees e
    JOIN departments d ON e.department_id = d.id
    JOIN positions p ON e.position_id = p.id
    WHERE e.status = 'resigned'
      AND e.resignation_date >= DATE_SUB(CURDATE(), INTERVAL 12 MONTH)
    GROUP BY e.department_id, d.name, p.level, e.gender, tenure_group, DATE_FORMAT(e.resignation_date, '%Y-%m')
),
monthly_headcount AS (
    ...
    GROUP BY department_id, DATE_FORMAT(hire_date, '%Y-%m')
)
SELECT
    mt.dept_name,
    mt.position_level,
...
SELECT
    ...
    (SELECT COUNT(*)
     ...
     WHERE sub.manager_id = e.id
       ...
       AND sub.resignation_date >= DATE_SUB(CURDATE(), INTERVAL 6 MONTH)
    ) AS team_resigned_6m,
    -- 团队总人数
    (SELECT COUNT(*)
     FROM employees sub
    ...
    (SELECT COALESCE(SUM(so.total_amount), 0)
     ...
     JOIN employees sub ON so.salesperson_id = sub.id
     WHERE sub.department_id = e.department_id
       AND DATE_FORMAT(so.order_date, '%Y-%m') = DATE_FORMAT(DATE_SUB(CURDATE(), INTERVAL 1 MONTH), '%Y-%m')
       AND so.status NOT IN ('draft', 'cancelled')
...
WITH dept_staffing AS (
    SELECT
        ...
        COUNT(CASE WHEN e.status = 'resigned' AND e.resignation_date >= DATE_SUB(CURDATE(), INTERVAL 30 DAY) THEN 1 END) AS recent_resigned,
...
WITH book_transactions AS (
    ...
    WHERE cj.account_id = 3  -- 工商银行基本户
      AND cj.journal_date >= DATE_SUB(CURDATE(), INTERVAL 30 DAY)
),
recon_summary AS (
    SELECT
...
WITH revenue AS (
    ...
    WHERE so.order_date >= DATE_SUB(CURDATE(), INTERVAL 6 MONTH)
      AND so.status NOT IN ('draft', 'cancelled')
    GROUP BY DATE_FORMAT(so.order_date, '%Y-%m'), d.name
),
cost AS (
    SELECT
        DATE_FORMAT(so.order_date, '%Y-%m') AS period,
...
WITH RECURSIVE date_series AS (
    SELECT CURDATE() + INTERVAL 1 DAY - INTERVAL DAY(CURDATE()) - 1 DAY + INTERVAL 1 MONTH AS forecast_date
    UNION ALL
    SELECT forecast_date + INTERVAL 1 MONTH
    FROM date_series
    WHERE forecast_date < CURDATE() + INTERVAL 12 MONTH
),
asset_forecast AS (
    SELECT
        fa.id,
...
SELECT
    ...
    current_accumulated,
    current_net_value,
    DATE_FORMAT(forecast_date, '%Y-%m') AS forecast_month,
    current_accumulated + monthly_depreciation * (
        TIMESTAMPDIFF(MONTH, CURDATE(), forecast_date) + 1
...
supplier_delivery_score AS (
    SELECT
        ...
        AVG(DATEDIFF(COALESCE(po.actual_delivery_date, po.expected_delivery_date), po.expected_delivery_date)) AS avg_delay_days,
...
baseline_sales AS (
    ...
    CROSS JOIN (
        ...
        FROM sales_orders so
        WHERE so.order_date BETWEEN DATE_SUB(CURDATE(), INTERVAL 60 DAY) AND DATE_SUB(CURDATE(), INTERVAL 30 DAY)
          AND so.status NOT IN ('draft', 'cancelled')
...
SELECT
    po.promo_name,
    po.promotion_type,
    CONCAT(po.start_date, ' ~ ', po.end_date) AS promo_period,
    DATEDIFF(po.end_date, po.start_date) + 1 AS promo_days,
    po.promo_order_count,
    po.promo_customers,
    po.promo_total_sales,
    po.total_discount_cost,
    bs.avg_daily_sales_baseline,
    ROUND(bs.avg_daily_sales_baseline * (DATEDIFF(po.end_date, po.start_date) + 1), 2) AS estimated_baseline_sales,
    ROUND(po.promo_total_sales - bs.avg_daily_sales_baseline * (DATEDIFF(po.end_date, po.start_date) + 1), 2) AS incremental_sales,
    ROUND(po.promo_total_sales / NULLIF(po.total_discount_cost, 0), 2) AS sales_per_discount_yuan,
    ROUND((po.promo_total_sales - bs.avg_daily_sales_baseline * (DATEDIFF(po.end_date, po.start_date) + 1) - po.total_discount_cost)
        / NULLIF(po.total_discount_cost, 0) * 100, 2) AS roi_pct,
    CASE
        WHEN (po.promo_total_sales - bs.avg_daily_sales_baseline * (DATEDIFF(po.end_date, po.start_date) + 1) - po.total_discount_cost)
...
SELECT
    ...
    wo.due_date,
    wo.completed_date,
    DATEDIFF(COALESCE(wo.completed_date, CURDATE()), wo.start_date) AS actual_days,
    DATEDIFF(wo.due_date, wo.start_date) AS planned_days,
    CASE
        WHEN wo.completed_date IS NOT NULL AND wo.completed_date > wo.due_date THEN '延期'
        WHEN wo.completed_date IS NOT NULL THEN '按时完成'
        WHEN wo.due_date < CURDATE() THEN '已逾期'
        ELSE '进行中'
    ...
    (SELECT GROUP_CONCAT(CONCAT(pp.name, ':', wom.actual_consumed, wom.unit) SEPARATOR ', ')
...
WITH ticket_metrics AS (
    SELECT
        ...
        COUNT(CASE
            WHEN st.priority = 'urgent' AND TIMESTAMPDIFF(HOUR, st.created_at, COALESCE(st.resolved_at, NOW())) <= 4 THEN 1
            WHEN st.priority = 'high' AND TIMESTAMPDIFF(HOUR, st.created_at, COALESCE(st.resolved_at, NOW())) <= 8 THEN 1
            WHEN st.priority = 'normal' AND TIMESTAMPDIFF(HOUR, st.created_at, COALESCE(st.resolved_at, NOW())) <= 24 THEN 1
            WHEN st.priority = 'low' AND TIMESTAMPDIFF(HOUR, st.created_at, COALESCE(st.resolved_at, NOW())) <= 48 THEN 1
        END) AS sla_met_tickets
    FROM service_tickets st
    WHERE st.created_at >= DATE_SUB(CURDATE(), INTERVAL 3 MONTH)
...
FROM ticket_metrics
ORDER BY ticket_type, FIELD(priority, 'urgent', 'high', 'normal', 'low');
...
WITH customer_behavior AS (
    SELECT
        ...
        c.name AS customer_name,
        c.membership_level,
        DATEDIFF(CURDATE(), MAX(so.order_date)) AS days_since_last_order,
        COUNT(DISTINCT CASE WHEN so.order_date >= DATE_SUB(CURDATE(), INTERVAL 3 MONTH) THEN so.id END) AS orders_3m,
        COUNT(DISTINCT CASE WHEN so.order_date >= DATE_SUB(CURDATE(), INTERVAL 6 MONTH)
            AND so.order_date < DATE_SUB(CURDATE(), INTERVAL 3 MONTH) THEN so.id END) AS orders_3_6m,
        COALESCE(SUM(CASE WHEN so.order_date >= DATE_SUB(CURDATE(), INTERVAL 3 MONTH) THEN so.total_amount END), 0) AS amount_3m,
        COALESCE(SUM(CASE WHEN so.order_date >= DATE_SUB(CURDATE(), INTERVAL 6 MONTH)
            AND so.order_date < DATE_SUB(CURDATE(), INTERVAL 3 MONTH) THEN so.total_amount END), 0) AS amount_3_6m,
        (SELECT COUNT(*) FROM service_tickets st
         WHERE st.customer_id = c.id AND st.ticket_type = 'complaint'
           AND st.created_at >= DATE_SUB(CURDATE(), INTERVAL 3 MONTH)
...
SELECT '销售健康度' AS metric_category, '月销售额' AS metric_name,
    ROUND(COALESCE((SELECT SUM(total_amount) FROM sales_orders WHERE DATE_FORMAT(order_date, '%Y-%m') = DATE_FORMAT(CURDATE(), '%Y-%m') AND status NOT IN ('draft','cancelled')), 0), 2) AS current_value,
    ROUND(COALESCE((SELECT SUM(total_amount) FROM sales_orders WHERE DATE_FORMAT(order_date, '%Y-%m') = DATE_FORMAT(DATE_SUB(CURDATE(), INTERVAL 1 MONTH), '%Y-%m') AND status NOT IN ('draft','cancelled')), 0), 2) AS last_value,
    '元' AS unit, 'higher_better' AS direction
UNION ALL
SELECT '销售健康度', '订单数',
    (SELECT COUNT(*) FROM sales_orders WHERE DATE_FORMAT(order_date, '%Y-%m') = DATE_FORMAT(CURDATE(), '%Y-%m') AND status NOT IN ('draft','cancelled')),
    (SELECT COUNT(*) FROM sales_orders WHERE DATE_FORMAT(order_date, '%Y-%m') = DATE_FORMAT(DATE_SUB(CURDATE(), INTERVAL 1 MONTH), '%Y-%m') AND status NOT IN ('draft','cancelled')),
    '个', 'higher_better'
UNION ALL
SELECT '销售健康度', '平均客单价',
    ROUND((SELECT AVG(total_amount) FROM sales_orders WHERE DATE_FORMAT(order_date, '%Y-%m') = DATE_FORMAT(CURDATE(), '%Y-%m') AND status NOT IN ('draft','cancelled')), 2),
    ROUND((SELECT AVG(total_amount) FROM sales_orders WHERE DATE_FORMAT(order_date, '%Y-%m') = DATE_FORMAT(DATE_SUB(CURDATE(), INTERVAL 1 MONTH), '%Y-%m') AND status NOT IN ('draft','cancelled')), 2),
    '元', 'higher_better'
UNION ALL
SELECT '库存健康度', '缺货SKU数',
...
UNION ALL
SELECT '库存健康度', '过期库存金额',
    ROUND(COALESCE((SELECT SUM(pb.current_qty * pb.purchase_price) FROM product_batches pb WHERE pb.expiry_date < CURDATE() AND pb.current_qty > 0 AND pb.status = 'active'), 0), 2),
    NULL, '元', 'lower_better'
UNION ALL
SELECT '财务健康度', '应收账款',
...
UNION ALL
SELECT '财务健康度', '现金余额',
    (SELECT SUM(current_balance) FROM accounts WHERE is_cash = TRUE OR is_bank = TRUE),
...
UNION ALL
SELECT '人力健康度', '本月离职人数',
    (SELECT COUNT(*) FROM employees WHERE status = 'resigned' AND resignation_date >= DATE_FORMAT(CURDATE(), '%Y-%m-01')),
    NULL, '人', 'lower_better';
...
WITH RECURSIVE bom_explosion AS (
    ...
    SELECT
        ...
        b.scrap_rate,
        1 AS bom_level,
        CAST(b.parent_product_id AS CHAR(200)) AS bom_path,
        CAST(CONCAT(p_parent.sku, ' -> ', p_child.sku) AS CHAR(500)) AS bom_path_names
    FROM boms b
    ...
    SELECT
        ...
        be.bom_level + 1,
        CONCAT(be.bom_path, ' -> ', b2.child_product_id),
        CONCAT(be.bom_path_names, ' -> ', p_child.sku)
