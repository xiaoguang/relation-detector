...
HAVING attendance_rate < 85 OR late_days > 5
...
WHERE so.warehouse_id = 1 AND so.order_date >= DATE_SUB(CURDATE(), INTERVAL 30 DAY)
...
WHERE sr.warehouse_id = 1 AND sr.return_date >= DATE_SUB(CURDATE(), INTERVAL 30 DAY)
...
WHERE sp.salary_month >= DATE_FORMAT(DATE_SUB(CURDATE(), INTERVAL 1 MONTH), '%Y-%m')
...
WHERE dr.warehouse_id = 1 AND dr.report_date >= DATE_SUB(CURDATE(), INTERVAL 30 DAY)
...
WHERE po.status = 'received' AND po.order_date >= DATE_SUB(CURDATE(), INTERVAL 30 DAY)
...
SELECT
    ...
    ai.target_summary,
    ai.submitted_at,
    TIMESTAMPDIFF(HOUR, ai.submitted_at, NOW()) AS pending_hours,
    ...
    CASE
        WHEN TIMESTAMPDIFF(HOUR, ai.submitted_at, NOW()) > 48 THEN '超时'
        WHEN TIMESTAMPDIFF(HOUR, ai.submitted_at, NOW()) > 24 THEN '即将超时'
        ELSE '正常'
    END AS urgency
...
-- 语法: 多表LEFT JOIN + 条件聚合 + GROUP_CONCAT
-- 业务: 点击商品进入详情页看到的数据
-- ============================================================
...
SELECT
    ...
    p.weight_kg,
    -- 各门店库存
    (SELECT GROUP_CONCAT(CONCAT(w.name, ':', COALESCE(i.quantity, 0)) SEPARATOR ' | ')
     FROM warehouses w
    ...
    -- 批号列表(含过期日期)
    (SELECT GROUP_CONCAT(CONCAT(pb.batch_no, '(', pb.current_qty, ',到期:', pb.expiry_date, ')')
        ORDER BY pb.expiry_date SEPARATOR '; ')
     FROM product_batches pb WHERE pb.product_id = p.id AND pb.current_qty > 0 AND pb.status = 'active'
    ) AS batch_list,
    -- 供应商列表(含价格)
    (SELECT GROUP_CONCAT(CONCAT(s.name, '(', s.city, '):¥', sp.supplier_price, ' 交期:', sp.lead_time_days, '天')
        ORDER BY sp.supplier_price SEPARATOR ' | ')
    ...
    (SELECT COALESCE(SUM(soi.quantity), 0)
     ...
     WHERE soi.product_id = p.id AND so.order_date >= DATE_SUB(CURDATE(), INTERVAL 30 DAY)
       AND so.status NOT IN ('draft', 'cancelled')
    ) AS sales_30d,
    p.status
FROM products p
...
FROM (
    SELECT DATE_SUB(CURDATE(), INTERVAL seq.num DAY) AS d
    FROM (SELECT 0 AS num UNION SELECT 1 UNION SELECT 2 UNION SELECT 3
          UNION SELECT 4 UNION SELECT 5 UNION SELECT 6) seq
) dates
LEFT JOIN (
...
LEFT JOIN (
    ...
    FROM sales_orders
    WHERE order_date BETWEEN DATE_SUB(CURDATE(), INTERVAL 14 DAY) AND DATE_SUB(CURDATE(), INTERVAL 8 DAY)
      AND status NOT IN ('draft', 'cancelled')
    GROUP BY order_date
) last_week ON dates.d = DATE_ADD(last_week.order_date, INTERVAL 7 DAY)
...
SELECT
    ...
    -- 本月出勤率
    fn_get_attendance_rate(e.id, DATE_FORMAT(CURDATE(), '%Y-%m')) AS this_month_attendance,
    -- 近3月平均工资
    ...
    -- 最近一次调薪
    (SELECT CONCAT(DATE_FORMAT(effective_date, '%Y-%m-%d'), ': ', old_salary, '->', new_salary)
...
SELECT
    ...
    -- 质检结果
    (SELECT GROUP_CONCAT(CONCAT(ir.inspection_result, '(', ROUND(ir.defect_rate, 1), '%)')
        SEPARATOR '; ')
     FROM inspection_reports ir WHERE ir.reference_id = po.id AND ir.reference_type = 'purchase_order'
    ) AS qc_results,
    -- 退货
    (SELECT GROUP_CONCAT(CONCAT(prt.return_no, ':', prt.status) SEPARATOR '; ')
...
WHERE po.order_date >= DATE_SUB(CURDATE(), INTERVAL 90 DAY)
...
WHERE sr.return_date >= DATE_SUB(CURDATE(), INTERVAL 90 DAY)
