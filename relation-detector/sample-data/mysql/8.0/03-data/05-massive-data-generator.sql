-- ============================================================
-- ERP系统海量测试数据生成
-- 目标: 100+门店, 500+员工, 50+品类, 500+商品, 5000+销售单, 1000+采购单
-- 执行方式: CALL sp_generate_massive_data();
-- 预计耗时: 3-5分钟
-- ============================================================
...
BEGIN
    ...
    WHILE v_cat1_idx < 8 DO
        ...
        -- 插入二级分类
        SET v_cat2_idx = 0;
        cat2_loop: WHILE v_cat2_idx < 100 DO
            ...
            -- 插入商品 (每个二级分类8-12个)
            SET v_prod_idx = 0;
            prod_loop: WHILE v_prod_idx < 12 DO
...
BEGIN
    ...
    emp_loop: LOOP
        ...
        SET v_day = 0;
        attendance_loop: WHILE v_day < 90 DO
            SET v_att_date = DATE_SUB(CURDATE(), INTERVAL v_day DAY);
            SET v_dow = DAYOFWEEK(v_att_date);

            IF v_dow IN (1, 7) THEN SET v_day = v_day + 1; ITERATE attendance_loop; END IF;
