...
WITH product_supplier_count AS (
    SELECT
        ...
        string_agg(DISTINCT CONCAT(s.name, '(', s.city, '):', sp.supplier_price), ' | ' ORDER BY sp.supplier_price) AS supplier_details
    FROM products p
...
SELECT
    ...
    CASE
        ...
        WHEN supplier_count < 3 THEN CONCAT('供应商不足(', supplier_count, '/3)-需拓展')
        WHEN supplier_count >= 5 THEN CONCAT('供应商充足(', supplier_count, '家)')
        ELSE CONCAT('正常(', supplier_count, '家)')
...
SELECT
    ...
    CASE
        ...
        ELSE CONCAT('第', rank_by_score, '选择')
...
SELECT
    ...
    string_agg(DISTINCT pc.name, ', ' ORDER BY pc.name) AS categories,
    -- 综合评价
    ROUND(AVG(
...
SELECT
    ...
    -- 最高价供应商
    (SELECT CONCAT(s.name, '(', sp.supplier_price, ')')
     FROM supplier_products sp JOIN suppliers s ON sp.supplier_id = s.id
     WHERE sp.product_id = ps.product_id ORDER BY sp.supplier_price DESC LIMIT 1
    ) AS most_expensive_supplier,
    -- 最低价供应商
    (SELECT CONCAT(s.name, '(', sp.supplier_price, ')')
     FROM supplier_products sp JOIN suppliers s ON sp.supplier_id = s.id
     WHERE sp.product_id = ps.product_id ORDER BY sp.supplier_price ASC LIMIT 1
