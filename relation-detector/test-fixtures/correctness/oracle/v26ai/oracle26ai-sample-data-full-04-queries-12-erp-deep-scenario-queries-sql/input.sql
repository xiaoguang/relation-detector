...
LEFT JOIN inventory_location_balances ilb ON ilb.location_id = pti.location_id
    ...
    AND ((ilb.batch_id = pti.batch_id) OR (ilb.batch_id IS NULL AND pti.batch_id IS NULL))
...
-- Q13: Semantic-layer example - customer payment amount in the last 30 days
SELECT
    c.id AS customer_id,
...
-- Q14: Semantic-layer example - active customer candidate definitions
SELECT
    c.id AS customer_id,
    c.name AS customer_name,
    c.status AS status,
    ...
    CASE
        WHEN c.status = 'active'
             AND (MAX(so.order_date) >= CURRENT_DATE - INTERVAL '90' DAY
                  OR MAX(p.payment_date) >= CURRENT_DATE - INTERVAL '90' DAY)
...
-- Q15: Semantic-layer example - current fiscal year East China womenwear sales
SELECT
    fc.fiscal_year,
    fc.fiscal_month,
    rd.sales_region,
...
-- Q17: Semantic-layer example - inventory risk
SELECT
    p.id AS product_id,
    p.sku,
    p.name AS product_name,
...
-- Q18: Semantic-layer example - RFM customer segmentation draft
WITH customer_rfm AS (
    SELECT
        c.id AS customer_id,
        c.name AS customer_name,
        CURRENT_DATE - MAX(sf.fiscal_date) AS recency_days,
...
-- Q19: Semantic-layer example - failed payment records
SELECT
    p.payment_no,
    c.name AS customer_name,
    so.order_no,
