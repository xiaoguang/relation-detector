...
--   sp_customer_store_preference: 客户门店偏好分析
--   sp_store_product_affinity: 门店商品关联规则
--
-- 注意: 所有 SELECT 型存储过程在 Oracle 中转换为
...
-- ============================================================
-- 48. 客户门店消费明细 - 完整购买历史
-- 参数: p_customer_id客户ID, p_start_date/p_end_date时间范围
-- 输出: 每次消费的时间/门店/商品/数量/金额/批号
-- 统计原理: 按客户+门店+订单+商品明细展开，含退货信息
-- ============================================================

CREATE OR REPLACE PROCEDURE sp_customer_store_purchase_history(
...
CREATE OR REPLACE PROCEDURE sp_store_bestsellers(
...
BEGIN
    ...
    CASE p_period
        ...
        WHEN 'week' THEN
            -- WEEKDAY: 0=Monday in MySQL → (TRUNC(...) - TRUNC(..., 'IW') + 1)- 1 in PG
            v_start_date := CURRENT_DATE - ((TRUNC(CURRENT_DATE) - TRUNC(CURRENT_DATE, 'IW') + 1) - 1);
            v_end_date := v_start_date + INTERVAL '6' DAY;
            v_prev_start_date := v_start_date - INTERVAL '7' DAY;
            v_prev_end_date := v_start_date - INTERVAL '1' DAY;
        WHEN 'month' THEN
            v_start_date := TRUNC(CURRENT_DATE, 'MM');
            v_end_date := (TRUNC(CURRENT_DATE, 'MM') + INTERVAL '1' MONTH - INTERVAL '1' DAY);
            v_prev_start_date := TRUNC(CURRENT_DATE - INTERVAL '1' MONTH, 'MM');
            v_prev_end_date := (TRUNC(CURRENT_DATE, 'MM') - INTERVAL '1' DAY);
        WHEN 'quarter' THEN
            v_start_date := TRUNC(CURRENT_DATE, 'Q');
            v_end_date := (TRUNC(CURRENT_DATE, 'Q') + INTERVAL '3' MONTH - INTERVAL '1' DAY);
            v_prev_start_date := v_start_date - INTERVAL '3' MONTH;
            v_prev_end_date := v_start_date - INTERVAL '1' DAY;
        WHEN 'year' THEN
            v_start_date := TO_DATE(TO_CHAR(EXTRACT(YEAR FROM CURRENT_DATE)) || '-' || LPAD(1, 2, '0') || '-' || LPAD(1, 2, '0'), 'YYYY-MM-DD');
            v_end_date := CURRENT_DATE;
            v_prev_start_date := TO_DATE(TO_CHAR(EXTRACT(YEAR FROM CURRENT_DATE) - 1) || '-' || LPAD(1, 2, '0') || '-' || LPAD(1, 2, '0'), 'YYYY-MM-DD');
            v_prev_end_date := v_start_date - INTERVAL '1' DAY;
        ELSE
            RAISE_APPLICATION_ERROR(-20000, 'Invalid period: %. Must be day/week/month/quarter/year');
    END CASE;
...
CREATE OR REPLACE PROCEDURE sp_store_performance_compare(
p_start_date DATE,
    p_end_date DATE,
    p_result OUT SYS_REFCURSOR
)
...
CREATE OR REPLACE PROCEDURE sp_customer_store_preference(
p_customer_id NUMBER,
    p_result OUT SYS_REFCURSOR
)
AS
...
CREATE OR REPLACE PROCEDURE sp_store_product_affinity(
p_warehouse_id NUMBER,
...
CREATE OR REPLACE PROCEDURE sp_customer_category_trend_by_store(
p_customer_id NUMBER,
    p_start_date DATE,
    p_end_date DATE,
    p_result OUT SYS_REFCURSOR
...
CREATE OR REPLACE PROCEDURE sp_store_sales_forecast(
p_warehouse_id NUMBER,
...
BEGIN
    ...
    WITH daily_sales AS (
        SELECT
            so.warehouse_id,
            so.order_date,
            (TO_NUMBER(TO_CHAR(so.order_date, 'D')) - 1) + 1 AS dow,
            SUM(so.total_amount) AS daily_total
        FROM sales_orders so
        WHERE so.order_date >= CURRENT_DATE - INTERVAL '60' DAY
          AND so.status NOT IN ('draft', 'cancelled')
          AND (p_warehouse_id IS NULL OR so.warehouse_id = p_warehouse_id)
        GROUP BY so.warehouse_id, so.order_date, (TO_NUMBER(TO_CHAR(so.order_date, 'D')) - 1)),
    moving_avg AS (
        SELECT
            warehouse_id,
            order_date,
    ...
    SELECT
        ...
        (TO_NUMBER(TO_CHAR(CURRENT_DATE + seq.num, 'D')) - 1) + 1,
    ...
    JOIN dow_coefficient dc ON lm.warehouse_id = dc.warehouse_id
        AND dc.dow = (TO_NUMBER(TO_CHAR(CURRENT_DATE + seq.num, 'D')) - 1) + 1
