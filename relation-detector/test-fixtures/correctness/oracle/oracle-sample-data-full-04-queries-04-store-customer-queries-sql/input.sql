...
WITH weekly_sales AS (
    SELECT
        ...
        pc.name AS category_name,
        CASE
            WHEN so.order_date >= TRUNC(CURRENT_DATE, 'IW') THEN '本周'
            ELSE '上周'
        END AS week_label,
    ...
    JOIN products p ON soi.product_id = p.id
    JOIN product_categories pc ON p.category_id = pc.id
    WHERE so.order_date >= TRUNC(CURRENT_DATE, 'IW') - INTERVAL '7' DAY
...
WITH hourly_sales AS (
    SELECT
        ...
        (TO_NUMBER(TO_CHAR(so.order_date, 'D')) - 1)AS dow,
        COUNT(DISTINCT so.id) AS order_count,
    ...
    WHERE so.order_date >= CURRENT_DATE - INTERVAL '90' DAY
      AND so.status NOT IN ('draft', 'cancelled')
    GROUP BY w.name, w.id, EXTRACT(HOUR FROM so.created_at), (TO_NUMBER(TO_CHAR(so.order_date, 'D')) - 1)),
time_segment AS (
    SELECT
        store_name,
        sale_hour,
...
SELECT
    ...
    LISTAGG(customer_name, ', ') WITHIN GROUP (ORDER BY customer_name) AS sample_customers,
    -- 推荐品类在门店的畅销品
    (SELECT p.sku FROM products p
     ...
     WHERE pc.id = cp.recommended_category_id AND so.warehouse_id = cp.warehouse_id
       AND so.status NOT IN ('draft', 'cancelled')
     GROUP BY p.id, p.sku
     ORDER BY SUM(soi.amount) DESC FETCH FIRST 1 ROWS ONLY
...
monthly_store_sales AS (
    SELECT
        ...
        so.customer_id,
        CASE
            WHEN cfp.first_purchase_date >= TRUNC(so.order_date, 'MM') THEN '新客'
            ELSE '老客'
        END AS status_label,
        so.total_amount
    FROM sales_orders so
