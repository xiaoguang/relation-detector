...
BEGIN
    ...
    IF p_role = 'store_manager' THEN
        ...
        IF NOT EXISTS (
            ...
            SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = '您不是该门店的管理者，无权查看';
        END IF;
    END IF;
    ...
    SELECT
        ...
        CASE
            ...
            WHEN pb.expiry_date <= DATE_ADD(CURDATE(), INTERVAL 7 DAY) THEN '7天内过期'
        ...
        CASE
            ...
            WHEN pb.expiry_date <= DATE_ADD(CURDATE(), INTERVAL 7 DAY) THEN 2
            WHEN pb.expiry_date <= DATE_ADD(CURDATE(), INTERVAL 30 DAY) THEN 3
            WHEN pb.expiry_date <= DATE_ADD(CURDATE(), INTERVAL 60 DAY) THEN 4
            WHEN pb.expiry_date <= DATE_ADD(CURDATE(), INTERVAL 90 DAY) THEN 5
            ELSE 6
        END AS urgency_level,
        -- 已过期天数
        CASE WHEN pb.expiry_date < CURDATE()
            THEN DATEDIFF(CURDATE(), pb.expiry_date)
            ELSE 0
        END AS days_expired,
        -- 保质期使用率
        CASE WHEN p.shelf_life_days > 0
            THEN ROUND(DATEDIFF(CURDATE(), pb.production_date) * 100.0 / p.shelf_life_days, 1)
            ELSE 0
        END AS shelf_life_used_pct,
        -- 预估折价损失
        CASE
            WHEN pb.expiry_date < CURDATE() THEN ROUND(pb.current_qty * pb.purchase_price * 0.8, 2)
            WHEN pb.expiry_date <= DATE_ADD(CURDATE(), INTERVAL 30 DAY) THEN ROUND(pb.current_qty * pb.purchase_price * 0.3, 2)
            WHEN pb.expiry_date <= DATE_ADD(CURDATE(), INTERVAL 60 DAY) THEN ROUND(pb.current_qty * pb.purchase_price * 0.1, 2)
            ELSE 0
        ...
        -- 是否会过期卖不完
        CASE
            WHEN pb.expiry_date < CURDATE() THEN '已过期-需立即处理'
            WHEN pb.expiry_date <= DATE_ADD(CURDATE(), INTERVAL 30 DAY)
                AND pb.current_qty > (SELECT COALESCE(SUM(soi.quantity), 0) * 2
                    ...
                    WHERE soi.product_id = p.id
                      ...
                      AND so.status NOT IN ('draft', 'cancelled'))
            THEN '临期且库存过高-需促销处理'
            WHEN pb.expiry_date <= DATE_ADD(CURDATE(), INTERVAL 30 DAY) THEN '临期但可售完'
            ELSE '正常'
...
-- relation-detector-fixture-source: ROUTINE:erp_system.sp_category_sales_vs_expiry
...
-- relation-detector-fixture-source: ROUTINE:erp_system.sp_store_expiry_dashboard
...
BEGIN
    ...
    SELECT
        ...
        SUM(CASE WHEN pb.expiry_date < CURDATE() THEN pb.current_qty ELSE 0 END) AS expired_qty,
        ROUND(SUM(CASE WHEN pb.expiry_date < CURDATE() THEN pb.current_qty * pb.purchase_price ELSE 0 END), 2) AS expired_value,
        SUM(CASE WHEN pb.expiry_date BETWEEN CURDATE() AND DATE_ADD(CURDATE(), INTERVAL 7 DAY) THEN pb.current_qty ELSE 0 END) AS expiry_7d_qty,
        SUM(CASE WHEN pb.expiry_date BETWEEN CURDATE() AND DATE_ADD(CURDATE(), INTERVAL 30 DAY) THEN pb.current_qty ELSE 0 END) AS expiry_30d_qty,
        SUM(CASE WHEN pb.expiry_date BETWEEN CURDATE() AND DATE_ADD(CURDATE(), INTERVAL 60 DAY) THEN pb.current_qty ELSE 0 END) AS expiry_60d_qty,
        MIN(pb.expiry_date) AS earliest_expiry,
    ...
    WHERE i.warehouse_id = p_warehouse_id
      ...
      AND pb.current_qty > 0
      AND pb.expiry_date <= DATE_ADD(CURDATE(), INTERVAL 60 DAY)

    UNION ALL

    -- 按分类汇总
    SELECT
        CONCAT('CATEGORY: ', pc.name) AS section,
        COUNT(DISTINCT pb.id),
        COUNT(DISTINCT pb.product_id),
        SUM(pb.current_qty),
        ROUND(SUM(pb.current_qty * pb.purchase_price), 2),
        SUM(CASE WHEN pb.expiry_date < CURDATE() THEN pb.current_qty ELSE 0 END),
        ROUND(SUM(CASE WHEN pb.expiry_date < CURDATE() THEN pb.current_qty * pb.purchase_price ELSE 0 END), 2),
        SUM(CASE WHEN pb.expiry_date BETWEEN CURDATE() AND DATE_ADD(CURDATE(), INTERVAL 7 DAY) THEN pb.current_qty ELSE 0 END),
        SUM(CASE WHEN pb.expiry_date BETWEEN CURDATE() AND DATE_ADD(CURDATE(), INTERVAL 30 DAY) THEN pb.current_qty ELSE 0 END),
        SUM(CASE WHEN pb.expiry_date BETWEEN CURDATE() AND DATE_ADD(CURDATE(), INTERVAL 60 DAY) THEN pb.current_qty ELSE 0 END),
        MIN(pb.expiry_date),
        NULL
    FROM product_batches pb
    JOIN products p ON pb.product_id = p.id
    ...
    -- 具体处理建议
    SELECT
        CONCAT('ACTION: ', p.sku, ' ', p.name, ' 批号:', pb.batch_no, ' 数量:', pb.current_qty, ' 到期:', pb.expiry_date,
            CASE
                WHEN pb.expiry_date < CURDATE() THEN ' 【立即报废处理】'
                WHEN pb.expiry_date <= DATE_ADD(CURDATE(), INTERVAL 7 DAY) THEN ' 【紧急促销/退货】'
                WHEN pb.expiry_date <= DATE_ADD(CURDATE(), INTERVAL 30 DAY) THEN ' 【打折促销】'
                WHEN pb.expiry_date <= DATE_ADD(CURDATE(), INTERVAL 60 DAY) THEN ' 【优先出库】'
                ELSE ''
            END
        ) AS section,
...
-- relation-detector-fixture-source: ROUTINE:erp_system.sp_expiry_heatmap
...
BEGIN
    SELECT
        ...
        SUM(CASE WHEN pb.expiry_date <= DATE_ADD(CURDATE(), INTERVAL 60 DAY) THEN i.quantity ELSE 0 END) AS near_expiry_stock,
        ROUND(SUM(i.quantity * p.purchase_price), 2) AS total_stock_value,
        ROUND(SUM(CASE WHEN pb.expiry_date <= DATE_ADD(CURDATE(), INTERVAL 60 DAY) THEN i.quantity * p.purchase_price ELSE 0 END), 2) AS near_expiry_value,
        ROUND(
            SUM(CASE WHEN pb.expiry_date <= DATE_ADD(CURDATE(), INTERVAL 60 DAY) THEN i.quantity ELSE 0 END) * 100.0
        ...
        -- 临期严重程度标签
        CASE
            WHEN SUM(CASE WHEN pb.expiry_date <= DATE_ADD(CURDATE(), INTERVAL 60 DAY) THEN i.quantity ELSE 0 END) * 100.0
...
-- relation-detector-fixture-source: ROUTINE:erp_system.sp_product_batch_detail
...
BEGIN
    IF p_product_id IS NULL AND p_sku IS NOT NULL THEN
        SELECT id INTO p_product_id FROM products WHERE sku = p_sku;
    END IF;
    ...
    SELECT
        ...
        CASE
            WHEN pb.expiry_date < CURDATE() THEN '已过期-立即下架'
            WHEN pb.expiry_date <= DATE_ADD(CURDATE(), INTERVAL 7 DAY) THEN '紧急-7天内过期'
            WHEN pb.expiry_date <= DATE_ADD(CURDATE(), INTERVAL 30 DAY) THEN '关注-30天内过期'
            WHEN pb.expiry_date <= DATE_ADD(CURDATE(), INTERVAL 60 DAY) THEN '提示-60天内过期'
            ELSE '正常'
        END AS expiry_alert,
        -- 该批号在该门店的销售速度
        (SELECT COALESCE(ROUND(SUM(soi.quantity) / 30.0, 1), 0)
...
-- relation-detector-fixture-source: ROUTINE:erp_system.sp_daily_expiry_alert
...
BEGIN
    ...
    SELECT
        ...
        pb.current_qty,
        ROUND(pb.current_qty * pb.purchase_price, 2) AS loss_value,
        CONCAT('请立即下架报废处理，损失预估: ', ROUND(pb.current_qty * pb.purchase_price, 2), '元') AS suggested_action
    FROM product_batches pb
    JOIN products p ON pb.product_id = p.id
    JOIN inventory i ON pb.id = i.batch_id
    JOIN warehouses w ON i.warehouse_id = w.id
    ...
    SELECT
        ...
        pb.current_qty,
        ROUND(pb.current_qty * p.purchase_price * 0.3, 2) AS potential_loss,
        CONCAT('建议立即打折促销(建议7折)，或联系供应商退货。预估损失: ',
            ROUND(pb.current_qty * p.purchase_price * 0.3, 2), '元') AS suggested_action
    FROM product_batches pb
    JOIN products p ON pb.product_id = p.id
    JOIN inventory i ON pb.id = i.batch_id
    JOIN warehouses w ON i.warehouse_id = w.id
    ...
    SELECT
        ...
        DATEDIFF(pb.expiry_date, CURDATE()) AS days_remaining,
        pb.current_qty,
        ROUND(pb.current_qty * p.purchase_price * 0.1, 2) AS potential_loss,
        CONCAT('库存量(', pb.current_qty, ')超过30天预计销量，建议优先出库或促销') AS suggested_action
    FROM product_batches pb
    JOIN products p ON pb.product_id = p.id
    JOIN inventory i ON pb.id = i.batch_id
    JOIN warehouses w ON i.warehouse_id = w.id
    ...
    ORDER BY alert_type, days_expired DESC, potential_loss DESC
    LIMIT 50;
END
-- relation-detector-fixture-end
