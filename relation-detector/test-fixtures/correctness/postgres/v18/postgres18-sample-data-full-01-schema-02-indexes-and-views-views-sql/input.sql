...
-- relation-detector-fixture-source: VIEW:public.v_inventory_summary
SELECT
    i.product_id,
    p.sku,
...
-- relation-detector-fixture-source: VIEW:public.v_sales_summary
SELECT
    so.id,
    so.order_no,
    so.order_date,
...
-- relation-detector-fixture-source: VIEW:public.v_purchase_summary
SELECT
    po.id,
    po.order_no,
    po.order_date,
...
-- relation-detector-fixture-source: VIEW:public.v_dept_headcount
SELECT
    d.id AS dept_id,
    d.name AS dept_name,
    d.code AS dept_code,
    ...
    COUNT(CASE WHEN e.status = 'resigned' AND e.resignation_date >= CURRENT_DATE - INTERVAL '30 days' THEN 1 END) AS resigned_30d,
    COUNT(CASE WHEN e.status = 'resigned' AND e.resignation_date >= CURRENT_DATE - INTERVAL '90 days' THEN 1 END) AS resigned_90d,
...
-- relation-detector-fixture-source: VIEW:public.v_account_balance
SELECT
    a.id,
    a.code,
    a.name,
