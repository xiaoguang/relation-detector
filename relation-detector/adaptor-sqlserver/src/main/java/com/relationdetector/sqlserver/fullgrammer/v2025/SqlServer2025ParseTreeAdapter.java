...
final class SqlServer2025ParseTreeAdapter extends AbstractSqlServerParseTreeAdapter {
    ...
    public OperatorSemantic operatorSemantic(ParseTree tree) {
        if (tree instanceof ExpressionContext expression && expression.op != null) {
            return expression.DOUBLE_BAR() != null
                    ? OperatorSemantic.CONCAT_FORMAT
                    : OperatorSemantic.ARITHMETIC;
        }
        if (tree instanceof Unary_operator_expressionContext unary && unary.op != null) {
    ...
    @Override
    public boolean isExistsPredicate(ParseTree tree) {
        return tree instanceof PredicateContext predicate && predicate.EXISTS() != null;
    }

    @Override
    public boolean isInPredicate(ParseTree tree) {
        return tree instanceof PredicateContext predicate && predicate.IN() != null;
    }

    @Override
    public DdlConstraintSemantic ddlConstraintSemantic(ParseTree tree) {
        if (tree instanceof Column_definitionContext column) {
            for (Column_definition_elementContext element : column.column_definition_element()) {
                DdlConstraintSemantic semantic = classify(element.column_constraint());
                if (semantic != DdlConstraintSemantic.NONE) return semantic;
            }
        }
    ...
    private DdlConstraintSemantic classify(Column_constraintContext constraint) {
        if (constraint == null) return DdlConstraintSemantic.NONE;
        if (constraint.FOREIGN() != null) return DdlConstraintSemantic.FOREIGN_KEY;
        if (constraint.PRIMARY() != null) return DdlConstraintSemantic.PRIMARY_KEY;
        return constraint.UNIQUE() != null ? DdlConstraintSemantic.UNIQUE : DdlConstraintSemantic.NONE;
    }

    private DdlConstraintSemantic classify(Table_constraintContext constraint) {
        if (constraint.FOREIGN() != null) return DdlConstraintSemantic.FOREIGN_KEY;
        if (constraint.PRIMARY() != null) return DdlConstraintSemantic.PRIMARY_KEY;
        return constraint.UNIQUE() != null ? DdlConstraintSemantic.UNIQUE : DdlConstraintSemantic.NONE;
    }
