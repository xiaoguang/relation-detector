...
public abstract class AbstractSqlServerParseTreeAdapter extends AbstractFullGrammerParseTreeAdapter {
    ...
    @Override
    public final Optional<FullGrammerColumnReference> directColumn(ParseTree tree) {
        return hasRole(tree, Role.COLUMN_REFERENCE)
                ? FullGrammerIdentifiers.columnReference(tree.getText())
                : Optional.empty();
    }
    ...
    public final Optional<String> functionName(ParseTree tree) {
        ...
        return start == null
                ? Optional.empty()
                : Optional.of(FullGrammerIdentifiers.clean(start.getText())).filter(value -> !value.isBlank());
    }
    ...
    public final Optional<RowsetBinding> rowsetBinding(ParseTree tree) {
        ...
        String physical = String.join(".", FullGrammerIdentifiers.qualifiedParts(table.getText()));
