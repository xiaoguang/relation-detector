...
import com.relationdetector.postgres.fullgrammar.common.PostgresDdlEventVisitorCore;
import com.relationdetector.postgres.fullgrammar.common.PostgresDdlVersionPolicy;

/**
 * PostgreSQL full-grammar DDL event collector。
 *
 ...
 *
 * <p>EN: PostgreSQL full-grammar DDL event collector. It extracts FK/index facts
 * from typed parse-tree contexts and emits unified DDL structured events. It
 * does not call the token-event DDL cursor.
 */
final class PostgresFullGrammarDdlEventCollector {
    private final PostgresDdlVersionPolicy policy;

    PostgresFullGrammarDdlEventCollector(PostgresDdlVersionPolicy policy) {
        this.policy = policy;
    }
    ...
    private static final class PostgresDdlVisitor extends PostgresFullGrammarParserBaseVisitor<Void> {
        ...
        @Override
        public Void visitCreatestmt(PostgresFullGrammarParser.CreatestmtContext ctx) {
        ...
        @Override
        public Void visitColumnDef(PostgresFullGrammarParser.ColumnDefContext ctx) {
            ...
            for (PostgresFullGrammarParser.ColconstraintContext colConstraint : ctx.colquallist().colconstraint()) {
                PostgresFullGrammarParser.ColconstraintelemContext constraint = colConstraint.colconstraintelem();
                if (constraint == null) {
                    continue;
                }
        ...
        @Override
        public Void visitAltertablestmt(PostgresFullGrammarParser.AltertablestmtContext ctx) {
        ...
        @Override
        public Void visitTableconstraint(PostgresFullGrammarParser.TableconstraintContext ctx) {
            if (core.currentTable().isBlank() || ctx.constraintelem() == null) {
                return null;
            }
            PostgresFullGrammarParser.ConstraintelemContext constraint = ctx.constraintelem();
            long line = ctx.getStart().getLine();
            if (constraint.REFERENCES() != null && constraint.qualified_name() != null) {
                List<PostgresFullGrammarParser.ColumnlistContext> columnLists =
                        constraint.getRuleContexts(PostgresFullGrammarParser.ColumnlistContext.class);
        ...
        @Override
        public Void visitIndexstmt(PostgresFullGrammarParser.IndexstmtContext ctx) {
        ...
        private String relationExprTable(PostgresFullGrammarParser.Relation_exprContext ctx) {
            return ctx == null || ctx.qualified_name() == null ? "" : core.out().clean(ctx.qualified_name().getText());
        }

        private List<String> postgresColumnList(
                PostgresFullGrammarParser.ConstraintelemContext constraint,
                int index,
                List<String> fallback
        ) {
            List<PostgresFullGrammarParser.ColumnlistContext> lists =
                    constraint.getRuleContexts(PostgresFullGrammarParser.ColumnlistContext.class);
            if (index >= lists.size()) {
                return fallback;
            }
        ...
        private List<String> postgresColumnList(
                PostgresFullGrammarParser.Column_list_Context columnList,
                List<String> fallback
        ) {
        ...
        private List<String> postgresColumns(PostgresFullGrammarParser.ColumnlistContext ctx) {
            return core.out().nonBlank(ctx.getRuleContexts(PostgresFullGrammarParser.ColumnElemContext.class).stream()
                    .map(this::postgresColumn)
                    .toList());
        }

        private String postgresColumn(PostgresFullGrammarParser.ColumnElemContext elem) {
            if (elem == null || elem.colid() == null) {
                return "";
            }
            if (policy.skipTemporalColumnElements() && (elem.PERIOD() != null || elem.WITHOUT() != null)) {
                return "";
            }
            return elem.colid().getText();
        }

        private String postgresIndexColumn(PostgresFullGrammarParser.Index_elemContext elem) {
            if (elem.colid() != null) {
