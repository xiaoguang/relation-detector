...
final class PostgresFullGrammerDdlEventCollector {
    ...
    private static final class PostgresDdlVisitor extends Postgres18FullGrammerParserBaseVisitor<Void> {
        ...
        @Override
        public Void visitCreatestmt(Postgres18FullGrammerParser.CreatestmtContext ctx) {
        ...
        @Override
        public Void visitColumnDef(Postgres18FullGrammerParser.ColumnDefContext ctx) {
            ...
            for (Postgres18FullGrammerParser.ColconstraintContext colConstraint : ctx.colquallist().colconstraint()) {
                Postgres18FullGrammerParser.ColconstraintelemContext constraint = colConstraint.colconstraintelem();
                if (constraint == null) {
                    continue;
                }
        ...
        @Override
        public Void visitAltertablestmt(Postgres18FullGrammerParser.AltertablestmtContext ctx) {
        ...
        @Override
        public Void visitTableconstraint(Postgres18FullGrammerParser.TableconstraintContext ctx) {
            if (core.currentTable().isBlank() || ctx.constraintelem() == null) {
                return null;
            }
            Postgres18FullGrammerParser.ConstraintelemContext constraint = ctx.constraintelem();
            long line = ctx.getStart().getLine();
            if (constraint.REFERENCES() != null && constraint.qualified_name() != null) {
                List<Postgres18FullGrammerParser.ColumnlistContext> columnLists =
                        constraint.getRuleContexts(Postgres18FullGrammerParser.ColumnlistContext.class);
        ...
        @Override
        public Void visitIndexstmt(Postgres18FullGrammerParser.IndexstmtContext ctx) {
            ...
            for (Postgres18FullGrammerParser.Index_elemContext elem : ctx.index_params().index_elem()) {
        ...
        private String relationExprTable(Postgres18FullGrammerParser.Relation_exprContext ctx) {
            return ctx == null || ctx.qualified_name() == null ? "" : core.out().clean(ctx.qualified_name().getText());
        }

        private List<String> postgresColumnList(
                Postgres18FullGrammerParser.ConstraintelemContext constraint,
                int index,
                List<String> fallback
        ) {
            List<Postgres18FullGrammerParser.ColumnlistContext> lists =
                    constraint.getRuleContexts(Postgres18FullGrammerParser.ColumnlistContext.class);
            if (index >= lists.size()) {
                return fallback;
            }
        ...
        private List<String> postgresColumnList(
                Postgres18FullGrammerParser.Column_list_Context columnList,
                List<String> fallback
        ) {
        ...
        private List<String> postgresColumns(Postgres18FullGrammerParser.ColumnlistContext ctx) {
            return core.out().nonBlank(ctx.getRuleContexts(Postgres18FullGrammerParser.ColumnElemContext.class).stream()
                    .map(this::postgresColumn)
                    .toList());
        }

        private String postgresColumn(Postgres18FullGrammerParser.ColumnElemContext elem) {
        ...
        private String postgresIndexColumn(Postgres18FullGrammerParser.Index_elemContext elem) {
            if (elem.colid() != null) {
