...
final class PostgresFullGrammerDdlEventCollector {
    ...
    private static final class PostgresDdlVisitor extends Postgres16FullGrammerParserBaseVisitor<Void> {
        ...
        @Override
        public Void visitCreatestmt(Postgres16FullGrammerParser.CreatestmtContext ctx) {
        ...
        @Override
        public Void visitColumnDef(Postgres16FullGrammerParser.ColumnDefContext ctx) {
            ...
            for (Postgres16FullGrammerParser.ColconstraintContext colConstraint : ctx.colquallist().colconstraint()) {
                Postgres16FullGrammerParser.ColconstraintelemContext constraint = colConstraint.colconstraintelem();
                if (constraint == null) {
                    continue;
                }
        ...
        @Override
        public Void visitAltertablestmt(Postgres16FullGrammerParser.AltertablestmtContext ctx) {
        ...
        @Override
        public Void visitTableconstraint(Postgres16FullGrammerParser.TableconstraintContext ctx) {
            if (core.currentTable().isBlank() || ctx.constraintelem() == null) {
                return null;
            }
            Postgres16FullGrammerParser.ConstraintelemContext constraint = ctx.constraintelem();
            long line = ctx.getStart().getLine();
            if (constraint.REFERENCES() != null && constraint.qualified_name() != null) {
                List<Postgres16FullGrammerParser.ColumnlistContext> columnLists =
                        constraint.getRuleContexts(Postgres16FullGrammerParser.ColumnlistContext.class);
        ...
        @Override
        public Void visitIndexstmt(Postgres16FullGrammerParser.IndexstmtContext ctx) {
            ...
            for (Postgres16FullGrammerParser.Index_elemContext elem : ctx.index_params().index_elem()) {
        ...
        private String relationExprTable(Postgres16FullGrammerParser.Relation_exprContext ctx) {
            return ctx == null || ctx.qualified_name() == null ? "" : core.out().clean(ctx.qualified_name().getText());
        }

        private List<String> postgresColumnList(
                Postgres16FullGrammerParser.ConstraintelemContext constraint,
                int index,
                List<String> fallback
        ) {
            List<Postgres16FullGrammerParser.ColumnlistContext> lists =
                    constraint.getRuleContexts(Postgres16FullGrammerParser.ColumnlistContext.class);
            if (index >= lists.size()) {
                return fallback;
            }
        ...
        private List<String> postgresColumnList(
                Postgres16FullGrammerParser.Column_list_Context columnList,
                List<String> fallback
        ) {
        ...
        private List<String> postgresColumns(Postgres16FullGrammerParser.ColumnlistContext ctx) {
            return core.out().nonBlank(ctx.getRuleContexts(Postgres16FullGrammerParser.ColumnElemContext.class).stream()
                    .map(this::postgresColumn)
                    .toList());
        }

        private String postgresColumn(Postgres16FullGrammerParser.ColumnElemContext elem) {
        ...
        private String postgresIndexColumn(Postgres16FullGrammerParser.Index_elemContext elem) {
            if (elem.colid() != null) {
