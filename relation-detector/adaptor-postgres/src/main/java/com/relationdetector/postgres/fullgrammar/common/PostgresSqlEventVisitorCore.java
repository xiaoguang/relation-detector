...
public final class PostgresSqlEventVisitorCore {
    ...
    private int insertSelectQueryDepth;
    ...
    public PostgresSqlEventVisitorCore(
        ...
        this.sink = new FullGrammarEventFacade(statement, new PostgresExpressionAnalyzer(
                adapter,
                PostgresRoutineAttributes.isRoutineSql(statement)));
        PostgresRoutineAttributes.nonColumnIdentifiers(statement.attributes())
                .forEach(sink::nonColumnIdentifier);
    }
    ...
    public List<StructuredSqlEvent> mergedEvents() {
        return FullGrammarEventMerger.merge(
                sink.events(),
                FullGrammarNativeEventTypes.POSTGRES_NATIVE_EVENTS);
    }
    ...
    public void enterInsertSelectQuery() {
        if (insertSelectQueryDepth++ == 0 && hasInsertSelectTarget()) {
            InsertSelectTarget current = insertSelectTargets.pop();
            insertSelectTargets.push(new InsertSelectTarget(
                    current.targetTable(), current.targetColumns(), 0));
        }
    }

    public void leaveInsertSelectQuery() {
        if (insertSelectQueryDepth > 0) {
            insertSelectQueryDepth--;
        }
    }

    public boolean inDirectInsertSelectQuery() {
        return hasInsertSelectTarget() && insertSelectQueryDepth == 1;
    }
