...
import com.relationdetector.postgres.fullgrammer.v18.Postgres18FullGrammerParser.*;

final class Postgres18ParseTreeAdapter extends AbstractFullGrammerParseTreeAdapter {
    Postgres18ParseTreeAdapter() {
        super(
    ...
    public Optional<FullGrammerColumnReference> directColumn(ParseTree tree) {
        return tree instanceof ColumnrefContext column
                ? FullGrammerIdentifiers.columnReference(column.getText())
                : Optional.empty();
    }
    ...
    public Optional<RowsetBinding> rowsetBinding(ParseTree tree) {
        ...
        String physical = String.join(".",
                FullGrammerIdentifiers.qualifiedParts(table.relation_expr().qualified_name().getText()));
        String qualifier = table.alias_clause() == null || table.alias_clause().colid() == null
                ? lastPart(physical)
                : FullGrammerIdentifiers.clean(table.alias_clause().colid().getText());
