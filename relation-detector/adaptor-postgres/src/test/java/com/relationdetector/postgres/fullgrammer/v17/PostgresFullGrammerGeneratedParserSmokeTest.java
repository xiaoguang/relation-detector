...
class PostgresFullGrammerGeneratedParserSmokeTest {
    ...
    private void assertParsesWithoutSyntaxErrors(String sql) {
        Postgres17FullGrammerLexer lexer = new Postgres17FullGrammerLexer(CharStreams.fromString(sql));
        Postgres17FullGrammerParser parser = new Postgres17FullGrammerParser(new CommonTokenStream(lexer));
