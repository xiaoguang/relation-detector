...
public final class TokenEventRelationExtractor {
            ...
            if (event.type() == StructuredParseEventType.PREDICATE_EQUALITY) {
                ...
                if (candidate != null) {
                    candidates.add(candidate);
                        ...
                        || isAcceptedSelfHierarchy(left, right, text(event, "leftAlias"), text(event, "rightAlias"))) {
    ...
    private boolean isAcceptedSelfHierarchy(ColumnRef left, ColumnRef right, String leftAlias, String rightAlias) {
        if (!left.table().equals(right.table())
                || normalize(left.columnName()).equals(normalize(right.columnName()))
                || normalize(leftAlias).equals(normalize(rightAlias))) {
            return false;
        }
        return isHierarchyColumnPair(left.columnName(), right.columnName())
                || isHierarchyColumnPair(right.columnName(), left.columnName());
    }

    private boolean isHierarchyColumnPair(String sourceColumn, String targetColumn) {
        String source = normalize(sourceColumn);
        String target = normalize(targetColumn);
        return (source.equals("manager_id") && target.equals("emp_id"))
                || (source.equals("predecessor_id") && target.equals("task_id"))
                || (source.equals("parent_department") && target.equals("id"));
    }
