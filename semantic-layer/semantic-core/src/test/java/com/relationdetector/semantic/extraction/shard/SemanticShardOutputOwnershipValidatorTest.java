...
import com.relationdetector.semantic.extraction.shard.SemanticShardBundleIndex;
...
final class SemanticShardOutputOwnershipValidatorTest {
    ...
    void acceptsObjectsGroundedByAnOwnedFactOrCandidate() {
        ObjectNode bundle = bundle(Set.of("rel-owned"), Set.of("trip-owned"), Set.of("rel-overlap"));
        ...
        assertDoesNotThrow(() -> new SemanticShardOutputOwnershipValidator().validate(output, bundle));
    }

    @Test
    void rejectsOwnedFactMentionedOnlyAsAuditEvidence() {
        ObjectNode bundle = bundle(Set.of("rel-owned"), Set.of(), Set.of());
        ObjectNode output = emptyDocument();
        ...
        assertThrows(SemanticExtractionValidationException.class,
                () -> new SemanticShardOutputOwnershipValidator().validate(output, bundle));
    }

    @Test
    void rejectsAnObjectGroundedOnlyByOverlapEvidence() {
        ObjectNode bundle = bundle(Set.of("rel-owned"), Set.of(), Set.of("rel-overlap"));
        ObjectNode output = emptyDocument();
        ObjectNode entity = output.withArray("entities").addObject()
                .put("name", "客户")
    ...
    void rejectsMissingOrForgedShardContext() {
        ObjectNode shardBundle = bundle(Set.of("rel-owned"), Set.of(), Set.of());
        ObjectNode output = emptyDocument();
        ObjectNode bundleWithoutContext = shardBundle.deepCopy();
        bundleWithoutContext.remove("shardContext");

        assertThrows(SemanticExtractionValidationException.class,
                () -> new SemanticShardOutputOwnershipValidator().validate(output, bundleWithoutContext));

        ObjectNode forged = shardBundle.deepCopy();
    ...
    private ObjectNode bundle(Set<String> ownedFacts, Set<String> ownedCandidates, Set<String> overlap) {
