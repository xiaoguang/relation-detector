 ...
 * 决定发布完整 staging，或从完整 staging 构建精简候选目录；它不影响模型执行、语义合并或失败 staging。
 *
 * EN: Defines artifact retention for a successful semantic extraction run. The atomic artifact writer uses this
 * validated configuration either to publish the complete staging directory or to build a reduced publish candidate.
 * It never changes model execution, semantic merging, or failed staging state.
 */
