 ...
 */
public final class SemanticGraphRecordStore implements AutoCloseable {
    private static final ObjectMapper JSON = new ObjectMapper();
    private final Map<Section, ExternalJsonRecordStore> records = new EnumMap<>(Section.class);

    public SemanticGraphRecordStore(Path workspace) {
        if (workspace == null) {
    ...
    public void append(EvidenceGraph graph) {
        if (graph == null) {
    ...
    public void appendFact(EvidenceGraphFact fact) {
        if (fact == null) {
    ...
    public boolean containsEvidence(String id) {
        return records.get(Section.EVIDENCE).containsKey(id);
    }

    public void provideReferences(SemanticReferenceClosureStore closure) {
        if (closure == null) {
    ...
    public void writeArray(Section section, JsonGenerator generator, String field) throws IOException {
