...
import com.relationdetector.semantic.extraction.normalization.SemanticNormalizationSupport;
 ...
 */
public final class SemanticResultDocumentWriter {
    ...
    private void writeIsolatedEntities(JsonGenerator generator) throws IOException {
        try (ExternalJsonRecordStore linked = new ExternalJsonRecordStore(
            ...
            for (SemanticResultStore.Section section : SemanticResultStore.Section.values()) {
                if (section == SemanticResultStore.Section.ENTITIES
                        || section == SemanticResultStore.Section.REVIEW_ITEMS) {
                    continue;
                }
                sections.get(section).forEach(record ->
            ...
            sections.get(SemanticResultStore.Section.ENTITIES).forEach(record -> {
                if (linked.containsKey(record.key())) return;
                JsonNode entity = selection.renamed(
                        SemanticResultStore.Section.ENTITIES,
                        record.key(),
                        selection.selectedDocument(SemanticResultStore.Section.ENTITIES, record.value()));
                try {
                    generator.writeStartObject();
    ...
    private String graphLabel(SemanticResultStore.Section section, JsonNode item) {
    ...
    private String graphType(SemanticResultStore.Section section, JsonNode item) {
        return switch (section) {
