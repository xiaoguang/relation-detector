 ...
 */
final class SemanticPathResultSelection {
    ...
    SemanticPathResultSelection(
            Map<SemanticPathResultStore.Section, ExternalJsonRecordStore> sections
    ) {
        this.sections = sections;
    }

    SemanticExtractionPrompt reconciliationPrompt(SemanticPathRunPlan plan, int maxInputTokens) {
    ...
    void applyPatch(JsonNode patch) {
        ...
        for (JsonNode rename : patch.path("renames")) {
            SemanticPathResultStore.Section section =
                    SemanticPathResultStore.Section.fromWire(rename.path("section").asText(""));
    ...
    JsonNode selectedDocument(SemanticPathResultStore.Section section, JsonNode stored) {
    ...
    JsonNode renamed(SemanticPathResultStore.Section section, String id, JsonNode source) {
    ...
    private boolean containsVariant(String sectionName, String id, String hash) {
        SemanticPathResultStore.Section section = SemanticPathResultStore.Section.fromWire(sectionName);
        if (section == null) {
            return false;
        }
        boolean[] found = {false};
