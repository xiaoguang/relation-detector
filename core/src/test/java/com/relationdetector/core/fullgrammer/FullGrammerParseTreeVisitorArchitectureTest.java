...
class FullGrammerParseTreeVisitorArchitectureTest {
    ...
    private Path repoRoot() {
        ...
        return root.getFileName().toString().equals("core") ? root.getParent() : root;
    }

    private Path coreSource(Path root, String fileName) {
        return root.resolve("core/src/main/java/com/relationdetector/core/fullgrammer/" + fileName);
    }

    private Path tokenEventSource(Path root, String fileName) {
        return root.resolve("core/src/main/java/com/relationdetector/core/tokenevent/" + fileName);
    }

    private Path relationSource(Path root, String fileName) {
        return root.resolve("core/src/main/java/com/relationdetector/core/relation/" + fileName);
    }

    private Path lineageSource(Path root, String fileName) {
        return root.resolve("core/src/main/java/com/relationdetector/core/lineage/" + fileName);
    }
    ...
    private void assertFullGrammerImplementationsAreVersionPackaged(Path root) throws Exception {
        assertFalse(Files.exists(coreSource(root, "MySqlFullGrammerDialectModule.java")),
                "MySQL full-grammer module belongs in adaptor-mysql, not core");
        assertFalse(Files.exists(coreSource(root, "PostgresFullGrammerDialectModule.java")),
                "PostgreSQL full-grammer module belongs in adaptor-postgres, not core");
    ...
    private void assertCoreResponsibilitiesArePackaged(Path root) {
        ...
        for (String fileName : rootFiles) {
            assertFalse(Files.exists(root.resolve("core/src/main/java/com/relationdetector/core/" + fileName)),
