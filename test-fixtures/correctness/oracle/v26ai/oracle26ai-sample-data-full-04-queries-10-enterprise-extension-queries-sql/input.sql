...
WHERE st.stocktake_date >= CURRENT_DATE - INTERVAL '90' DAY
...
WHERE tr.transfer_date >= CURRENT_DATE - INTERVAL '180' DAY
...
SELECT
    ...
    LISTAGG(pra.reference_type || '#' || pra.reference_id, ', ') WITHIN GROUP (ORDER BY pra.reference_type) AS allocated_refs
...
WITH operation_chain AS (
    SELECT
        ...
        CAST(po.operation_name AS VARCHAR2(500)) AS operation_path,
        po.standard_minutes
    ...
    SELECT
        ...
        po.operation_name,
        po.predecessor_operation_id,
        CAST((oc.operation_path || ' -> ' || po.operation_name) AS VARCHAR2(500)) AS operation_path,
...
FROM customers c
JOIN customer_addresses ca ON ca.customer_id = c.id AND ca.is_default = 1
