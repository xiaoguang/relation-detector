...
monthly_active AS (
    ...
    SELECT DISTINCT
        ...
        TRUNC(MONTHS_BETWEEN(TO_DATE(TO_CHAR(so.order_date, 'YYYY-MM') || '-01', 'YYYY-MM-DD'), TO_DATE(fp.cohort_month || '-01', 'YYYY-MM-DD'))) AS month_number
...
rule4_high_return AS (
    ...
    JOIN (
        ...
        FROM sales_returns
        WHERE return_date >= CURRENT_DATE - INTERVAL '3' MONTH
        GROUP BY customer_id
        HAVING COUNT(*) > 2
...
WITH daily_demand AS (
    ...
    WHERE so.order_date >= CURRENT_DATE - INTERVAL '90' DAY
      AND so.status NOT IN ('draft', 'cancelled')
...
WITH shipping_metrics AS (
    ...
    WHERE s.shipped_at >= CURRENT_DATE - INTERVAL '6' MONTH
...
WITH monthly_attendance AS (
    ...
    JOIN departments d ON e.department_id = d.id
    WHERE a.attendance_date >= CURRENT_DATE - INTERVAL '6' MONTH
...
WITH wh_throughput AS (
    ...
    WHERE it.created_at >= CURRENT_DATE - INTERVAL '3' MONTH
...
wh_shipments AS (
    ...
    FROM shipments
    WHERE shipped_at >= CURRENT_DATE - INTERVAL '3' MONTH
