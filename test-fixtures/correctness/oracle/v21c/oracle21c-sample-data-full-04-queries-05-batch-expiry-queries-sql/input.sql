...
SELECT
    ...
    -- 7天内
    SUM(CASE WHEN pb.expiry_date BETWEEN CURRENT_DATE AND CURRENT_DATE + INTERVAL '7' DAY THEN i.quantity ELSE 0 END) AS expiry_7d,
    -- 30天内
    SUM(CASE WHEN pb.expiry_date BETWEEN CURRENT_DATE + INTERVAL '8' DAY AND CURRENT_DATE + INTERVAL '30' DAY THEN i.quantity ELSE 0 END) AS expiry_30d,
    -- 60天内
    SUM(CASE WHEN pb.expiry_date BETWEEN CURRENT_DATE + INTERVAL '31' DAY AND CURRENT_DATE + INTERVAL '60' DAY THEN i.quantity ELSE 0 END) AS expiry_60d,
    -- 临期率
    ROUND(
    ...
    -- 临期库存可售天数(基于日均销量)
    ROUND(
        SUM(CASE WHEN pb.expiry_date <= CURRENT_DATE + INTERVAL '60' DAY THEN i.quantity ELSE 0 END)
        / NULLIF(COALESCE((
    ...
    (SELECT pb2.batch_no FROM product_batches pb2
     ...
     ORDER BY pb2.expiry_date ASC FETCH FIRST 1 ROWS ONLY
...
cat_inventory AS (
    SELECT
        ...
        SUM(CASE WHEN pb.expiry_date <= CURRENT_DATE + INTERVAL '60' DAY AND pb.status = 'active'
...
SELECT
    ...
    LISTAGG(store_name, ', ') WITHIN GROUP (ORDER BY store_name) AS stores
FROM batch_age ba
...
product_expiry_risk AS (
    SELECT
        p.id AS product_id,
        SUM(CASE WHEN pb.expiry_date <= CURRENT_DATE + INTERVAL '60' DAY THEN i.quantity ELSE 0 END) AS near_expiry_qty,
        SUM(i.quantity) AS total_stock,
        ROUND(
            SUM(CASE WHEN pb.expiry_date <= CURRENT_DATE + INTERVAL '60' DAY THEN i.quantity ELSE 0 END) * 100.0
            / NULLIF(SUM(i.quantity), 0), 2
...
WITH store_expiry_stats AS (
    SELECT
        ...
        SUM(CASE WHEN pb.expiry_date BETWEEN CURRENT_DATE AND CURRENT_DATE + INTERVAL '30' DAY THEN i.quantity ELSE 0 END) AS near_expiry_30d,
        ROUND(SUM(CASE WHEN pb.expiry_date BETWEEN CURRENT_DATE AND CURRENT_DATE + INTERVAL '30' DAY THEN i.quantity * p.purchase_price ELSE 0 END), 2) AS near_expiry_30d_value,
        ROUND(
            SUM(CASE WHEN pb.expiry_date < CURRENT_DATE THEN i.quantity ELSE 0 END) * 100.0
            / NULLIF(SUM(i.quantity), 0), 2
        ) AS expired_rate_pct,
        ROUND(
            SUM(CASE WHEN pb.expiry_date <= CURRENT_DATE + INTERVAL '30' DAY THEN i.quantity ELSE 0 END) * 100.0
            / NULLIF(SUM(i.quantity), 0), 2
        ) AS at_risk_rate_pct,
...
SELECT
    ...
    CASE
        ...
        WHEN ses.last_stocktake < CURRENT_DATE - INTERVAL '30' DAY THEN '盘点不及时-需安排盘点'
