...
p_workflow_code IN VARCHAR2,
    ...
    p_target_summary IN VARCHAR2,
    p_submitted_by IN NUMBER,
    p_result OUT SYS_REFCURSOR
)
AS
    v_workflow_id NUMBER(19);
    v_instance_no VARCHAR2(30);
    v_instance_id NUMBER(19);
    v_total_nodes NUMBER(10);
    v_first_node_id NUMBER(19);
    v_first_approver_id NUMBER(19);
    v_first_approver_type VARCHAR2(30);
    v_employee_dept_id NUMBER(19);
    v_employee_manager_id NUMBER(19);
BEGIN
    -- 查找审批流
    SELECT id INTO v_workflow_id FROM approval_workflows WHERE workflow_code = p_workflow_code AND is_active = 1;
    IF v_workflow_id IS NULL THEN
        RAISE_APPLICATION_ERROR(-20000, '审批流不存在或未激活');
    END IF;

    -- 计算总节点数
    SELECT COUNT(*) INTO v_total_nodes FROM approval_nodes WHERE workflow_id = v_workflow_id;

    v_instance_no := 'APP-' || TO_CHAR(CURRENT_DATE, 'YYYYMMDD') || '-' || lpad((FLOOR(DBMS_RANDOM.VALUE * 9999) + 1), 4, '0');
    ...
    VALUES ('submit_approval', 'approval_instance', v_instance_id, p_submitted_by,
            JSON_OBJECT('instance_no' VALUE v_instance_no, 'workflow_code' VALUE p_workflow_code, 'target' VALUE p_target_summary));
...
CREATE OR REPLACE PROCEDURE sp_process_approval(
    ...
    p_delegated_to IN NUMBER
)
AS
    v_current_node_level NUMBER(10);
    v_total_nodes NUMBER(10);
    v_next_node_id NUMBER(19);
    v_next_approver_id NUMBER(19);
    v_next_approver_type VARCHAR2(30);
...
BEGIN
    ...
    IF p_action = 'approve' THEN
        IF v_current_node_level >= v_total_nodes THEN
            ...
            SET status = 'approved', completed_at = SYSTIMESTAMP
    ...
    VALUES ('process_approval', 'approval_instance', p_instance_id, p_approver_id,
            JSON_OBJECT('action' VALUE p_action, 'comment' VALUE p_comment));
...
p_customer_id IN NUMBER,
    p_settlement_period IN VARCHAR2,
    p_result OUT SYS_REFCURSOR
)
AS
    v_consignment_id NUMBER(19);
    v_total_consumed NUMBER(10);
    ...
    CURSOR cur IS
        ...
        LEFT JOIN consignment_consumptions cc ON ci.id = cc.consignment_id
            AND cc.confirmed_by_customer = 0
...
BEGIN
    ...
    v_order_no := 'SO-CON-' || p_settlement_period || '-' || lpad((FLOOR(DBMS_RANDOM.VALUE * 9999) + 1), 4, '0');
    ...
    LOOP
        ...
        SET confirmed_by_customer = 1, sales_order_id = v_order_id
        WHERE consignment_id = v_consignment_id AND confirmed_by_customer = 0;
...
CREATE OR REPLACE PROCEDURE sp_calculate_cash_flow(
    p_forecast_days IN NUMBER
)
AS
    v_day NUMBER(10) DEFAULT 0;
    v_forecast_date DATE;
    v_beginning_balance NUMBER(18,2);
    v_daily_collections NUMBER(18,2);
    v_daily_payments NUMBER(18,2);
    v_daily_salary NUMBER(18,2);
    v_daily_tax NUMBER(18,2);
    v_avg_daily_sales NUMBER(18,2);
BEGIN
    -- 期初余额
    SELECT SUM(current_balance) INTO v_beginning_balance
    FROM accounts WHERE is_cash = 1 OR is_bank = 1;
    ...
    WHILE v_day < p_forecast_days LOOP
        ...
        VALUES (v_forecast_date, 'daily', v_beginning_balance,
            ...
            CASE WHEN (TO_NUMBER(TO_CHAR(v_forecast_date, 'D')) - 1)= 5 THEN 0 ELSE v_daily_salary END,
...
AS
    v_output_tax NUMBER(18,2);
    v_input_tax NUMBER(18,2);
    v_input_transfer NUMBER(18,2);
    v_tax_payable NUMBER(18,2);
    v_deadline DATE;
BEGIN
    v_deadline := (p_tax_period || '-01') + INTERVAL '1' MONTH + INTERVAL '14' DAY;
...
BEGIN
    ...
    VALUES ('change_price', 'product', p_product_id, p_changed_by,
            JSON_OBJECT(KEY p_price_type VALUE v_old_price),
            JSON_OBJECT(KEY p_price_type VALUE p_new_price, 'reason' VALUE p_change_reason));
...
CREATE OR REPLACE PROCEDURE sp_create_performance_review(
    ...
    p_review_period IN VARCHAR2,
    p_review_type IN VARCHAR2,
    p_performance_score IN NUMBER,
    p_competency_score IN NUMBER,
    p_attitude_score IN NUMBER,
    p_attendance_score IN NUMBER,
    p_self_assessment IN CLOB,
    p_reviewer_comment IN CLOB
)
AS
    v_review_no VARCHAR2(30);
BEGIN
    v_review_no := 'PR-' || p_review_period || '-' || lpad(p_employee_id, 4, '0');
