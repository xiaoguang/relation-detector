-- relation-detector-fixture-source: ROUTINE:erp_system.fn_get_customer_clv
CREATE FUNCTION fn_get_customer_clv(
    p_customer_id BIGINT UNSIGNED
)
RETURNS DECIMAL(18,2)
DETERMINISTIC
READS SQL DATA
BEGIN
    DECLARE v_avg_order_value DECIMAL(12,2);
    DECLARE v_total_orders INT;
    DECLARE v_first_order_date DATE;
    DECLARE v_last_order_date DATE;
    DECLARE v_active_months INT;
    DECLARE v_annual_frequency DECIMAL(8,2);
    DECLARE v_avg_lifetime_years DECIMAL(5,2) DEFAULT 3.0;
    DECLARE v_clv DECIMAL(18,2);

    SELECT
        AVG(total_amount),
        COUNT(*),
        MIN(order_date),
        MAX(order_date)
    INTO
        v_avg_order_value,
        v_total_orders,
        v_first_order_date,
        v_last_order_date
    FROM sales_orders
    WHERE customer_id = p_customer_id
      AND status NOT IN ('draft', 'cancelled');

    IF v_total_orders = 0 THEN
        RETURN 0.00;
    END IF;

    SET v_active_months = TIMESTAMPDIFF(MONTH, v_first_order_date, COALESCE(v_last_order_date, v_first_order_date)) + 1;
    SET v_annual_frequency = v_total_orders / NULLIF(v_active_months, 0) * 12.0;

    -- 根据会员等级调整预估生命周期
    SELECT CASE membership_level
        WHEN 'diamond' THEN 5.0
        WHEN 'platinum' THEN 4.0
        WHEN 'gold' THEN 3.0
        WHEN 'silver' THEN 2.0
        ELSE 1.5
    END INTO v_avg_lifetime_years
    FROM customers WHERE id = p_customer_id;

    SET v_clv = ROUND(v_avg_order_value * v_annual_frequency * v_avg_lifetime_years, 2);

    RETURN v_clv;
END//


-- ============================================================
-- 汇率换算
-- 调用方: 外币交易、财务报表折算
-- 换算原理: amount * 当日汇率(取最近汇率)
-- ============================================================
-- relation-detector-fixture-end

-- relation-detector-fixture-source: ROUTINE:erp_system.fn_convert_currency
CREATE FUNCTION fn_convert_currency(
    p_amount DECIMAL(18,2),
    p_from_currency VARCHAR(3),
    p_to_currency VARCHAR(3),
    p_rate_date DATE
)
RETURNS DECIMAL(18,2)
DETERMINISTIC
READS SQL DATA
BEGIN
    DECLARE v_rate DECIMAL(12,6);

    IF p_from_currency = p_to_currency THEN
        RETURN p_amount;
    END IF;

    SELECT rate INTO v_rate
    FROM exchange_rates
    WHERE from_currency = p_from_currency
      AND to_currency = p_to_currency
      AND rate_date <= p_rate_date
    ORDER BY rate_date DESC
    LIMIT 1;

    IF v_rate IS NULL THEN
        SELECT rate INTO v_rate
        FROM exchange_rates
        WHERE from_currency = p_to_currency
          AND to_currency = p_from_currency
          AND rate_date <= p_rate_date
        ORDER BY rate_date DESC
        LIMIT 1;

        IF v_rate IS NOT NULL THEN
            RETURN ROUND(p_amount / v_rate, 2);
        END IF;
    END IF;

    RETURN ROUND(p_amount * COALESCE(v_rate, 1.0), 2);
END//


-- ============================================================
-- 项目完成百分比
-- 调用方: 项目监控仪表盘
-- 计算原理: 实际成本/预算 或 里程碑完成率
-- ============================================================
-- relation-detector-fixture-end

-- relation-detector-fixture-source: ROUTINE:erp_system.fn_get_project_completion_pct
CREATE FUNCTION fn_get_project_completion_pct(
    p_project_id BIGINT UNSIGNED
)
RETURNS DECIMAL(5,2)
DETERMINISTIC
READS SQL DATA
BEGIN
    DECLARE v_budget DECIMAL(18,2);
    DECLARE v_actual_cost DECIMAL(18,2);
    DECLARE v_completed_milestones INT;
    DECLARE v_total_milestones INT;
    DECLARE v_pct DECIMAL(5,2);

    SELECT budget INTO v_budget FROM projects WHERE id = p_project_id;

    SELECT COALESCE(SUM(amount), 0) INTO v_actual_cost
    FROM project_costs WHERE project_id = p_project_id;

    SELECT
        COUNT(CASE WHEN status = 'completed' THEN 1 END),
        COUNT(*)
    INTO v_completed_milestones, v_total_milestones
    FROM contract_milestones cm
    JOIN contracts c ON cm.contract_id = c.id
    WHERE c.id IN (SELECT id FROM contracts WHERE subject LIKE CONCAT('%', (SELECT name FROM projects WHERE id = p_project_id), '%'));

    -- 综合: 成本占比50% + 里程碑占比50%
    SET v_pct = ROUND(
        COALESCE(v_actual_cost / NULLIF(v_budget, 0) * 50, 0)
        + COALESCE(v_completed_milestones / NULLIF(v_total_milestones, 0) * 50, 0)
    , 2);

    RETURN LEAST(v_pct, 100.00);
END//


-- ============================================================
-- 客户信用评分
-- 调用方: 信用额度审批、销售风险评估
-- 评分原理: 付款及时性(40%) + 购买稳定性(30%) + 退货率(20%) + 合作年限(10%)
-- 满分100分
-- ============================================================
-- relation-detector-fixture-end

-- relation-detector-fixture-source: ROUTINE:erp_system.fn_get_customer_credit_score
CREATE FUNCTION fn_get_customer_credit_score(
    p_customer_id BIGINT UNSIGNED
)
RETURNS TINYINT UNSIGNED
DETERMINISTIC
READS SQL DATA
BEGIN
    DECLARE v_ontime_payment_pct DECIMAL(5,2);
    DECLARE v_total_orders INT;
    DECLARE v_return_rate DECIMAL(5,2);
    DECLARE v_customer_years DECIMAL(4,1);
    DECLARE v_order_regularity DECIMAL(5,2);
    DECLARE v_score INT;

    -- 付款及时性: 按时付款的订单占比
    SELECT
        COUNT(CASE WHEN so.paid_amount >= so.total_amount * 0.9
            AND DATEDIFF(so.delivery_date, so.order_date) <= c.credit_days THEN 1 END) * 100.0
        / NULLIF(COUNT(*), 0)
    INTO v_ontime_payment_pct
    FROM sales_orders so
    JOIN customers c ON so.customer_id = c.id
    WHERE so.customer_id = p_customer_id
      AND so.status = 'delivered';

    -- 订单总数
    SELECT COUNT(*) INTO v_total_orders
    FROM sales_orders WHERE customer_id = p_customer_id AND status NOT IN ('draft', 'cancelled');

    -- 退货率
    SELECT COUNT(*) * 100.0 / NULLIF(v_total_orders, 0) INTO v_return_rate
    FROM sales_returns WHERE customer_id = p_customer_id;

    -- 合作年限
    SELECT TIMESTAMPDIFF(YEAR, MIN(order_date), CURDATE()) INTO v_customer_years
    FROM sales_orders WHERE customer_id = p_customer_id;

    -- 购买规律性: 各月订单数的标准差越小越好
    SELECT 100 - LEAST(COALESCE(STDDEV(monthly_orders), 0) * 20, 50) INTO v_order_regularity
    FROM (
        SELECT DATE_FORMAT(order_date, '%Y-%m') AS month, COUNT(*) AS monthly_orders
        FROM sales_orders
        WHERE customer_id = p_customer_id AND status NOT IN ('draft', 'cancelled')
        GROUP BY DATE_FORMAT(order_date, '%Y-%m')
    ) t;

    SET v_score = ROUND(
        COALESCE(v_ontime_payment_pct, 0) * 0.40
        + COALESCE(v_order_regularity, 50) * 0.30
        + (100 - COALESCE(v_return_rate, 0)) * 0.20
        + LEAST(COALESCE(v_customer_years, 0) * 10, 10) * 0.10
    , 0);

    RETURN LEAST(GREATEST(v_score, 0), 100);
END//


-- ============================================================
-- 质检合格率
-- 调用方: 供应商评估、质量报告
-- ============================================================
-- relation-detector-fixture-end

-- relation-detector-fixture-source: ROUTINE:erp_system.fn_get_inspection_pass_rate
CREATE FUNCTION fn_get_inspection_pass_rate(
    p_product_id BIGINT UNSIGNED,
    p_start_date DATE,
    p_end_date DATE
)
RETURNS DECIMAL(5,2)
DETERMINISTIC
READS SQL DATA
BEGIN
    DECLARE v_total INT;
    DECLARE v_qualified INT;
    DECLARE v_rate DECIMAL(5,2);

    SELECT COUNT(*), COUNT(CASE WHEN inspection_result = 'qualified' THEN 1 END)
    INTO v_total, v_qualified
    FROM inspection_reports
    WHERE product_id = p_product_id
      AND inspection_date BETWEEN p_start_date AND p_end_date;

    IF v_total > 0 THEN
        SET v_rate = ROUND(v_qualified * 100.0 / v_total, 2);
    ELSE
        SET v_rate = 0;
    END IF;

    RETURN v_rate;
END//


-- ============================================================
-- 客户消费状态判定
-- 调用方: 客户分层、营销自动化
-- 状态定义:
--   active(活跃): 近30天有购买
--   stable(稳定): 30-90天有购买
--   sleeping(沉睡): 90-180天有购买
--   at_risk(流失风险): 180-365天有购买
--   churned(已流失): >365天无购买
--   new(新客): 首次购买在30天内
-- ============================================================
-- relation-detector-fixture-end

-- relation-detector-fixture-source: ROUTINE:erp_system.fn_get_customer_status
CREATE FUNCTION fn_get_customer_status(
    p_customer_id BIGINT UNSIGNED
)
RETURNS VARCHAR(20)
DETERMINISTIC
READS SQL DATA
BEGIN
    DECLARE v_days_since_last INT;
    DECLARE v_first_order_date DATE;
    DECLARE v_status VARCHAR(20);

    SELECT DATEDIFF(CURDATE(), MAX(order_date)),
           MIN(order_date)
    INTO v_days_since_last, v_first_order_date
    FROM sales_orders
    WHERE customer_id = p_customer_id
      AND status NOT IN ('draft', 'cancelled');

    IF v_first_order_date IS NULL THEN
        RETURN 'no_purchase';
    END IF;

    IF DATEDIFF(CURDATE(), v_first_order_date) <= 30 THEN
        RETURN 'new';
    END IF;

    IF v_days_since_last <= 30 THEN
        SET v_status = 'active';
    ELSEIF v_days_since_last <= 90 THEN
        SET v_status = 'stable';
    ELSEIF v_days_since_last <= 180 THEN
        SET v_status = 'sleeping';
    ELSEIF v_days_since_last <= 365 THEN
        SET v_status = 'at_risk';
    ELSE
        SET v_status = 'churned';
    END IF;

    RETURN v_status;
END//


-- ============================================================
-- 客户复购率
-- 计算原理: 购买>=2次的客户占比
-- ============================================================
-- relation-detector-fixture-end

-- relation-detector-fixture-source: ROUTINE:erp_system.fn_get_customer_repurchase_rate
CREATE FUNCTION fn_get_customer_repurchase_rate(
    p_start_date DATE,
    p_end_date DATE
)
RETURNS DECIMAL(5,2)
DETERMINISTIC
READS SQL DATA
BEGIN
    DECLARE v_total INT;
    DECLARE v_repurchased INT;
    DECLARE v_rate DECIMAL(5,2);

    SELECT COUNT(DISTINCT customer_id),
           COUNT(DISTINCT CASE WHEN order_count >= 2 THEN customer_id END)
    INTO v_total, v_repurchased
    FROM (
        SELECT customer_id, COUNT(*) AS order_count
        FROM sales_orders
        WHERE order_date BETWEEN p_start_date AND p_end_date
          AND status NOT IN ('draft', 'cancelled')
        GROUP BY customer_id
    ) t;

    IF v_total > 0 THEN
        SET v_rate = ROUND(v_repurchased * 100.0 / v_total, 2);
    ELSE
        SET v_rate = 0;
    END IF;

    RETURN v_rate;
END
-- relation-detector-fixture-end
