...
DECLARE
    ...
    v_active_months INT;
    v_annual_frequency DECIMAL(8,2);
    v_avg_lifetime_years DECIMAL(5,2) DEFAULT 3.0;
    v_clv DECIMAL(18,2);
BEGIN
    SELECT
        AVG(total_amount),
    ...
    v_active_months := EXTRACT(YEAR FROM AGE(COALESCE(v_last_order_date, v_first_order_date), v_first_order_date)) * 12
                       + EXTRACT(MONTH FROM AGE(COALESCE(v_last_order_date, v_first_order_date), v_first_order_date)) + 1;
    v_annual_frequency := v_total_orders::NUMERIC / NULLIF(v_active_months, 0) * 12.0;
...
-- relation-detector-fixture-source: ROUTINE:public.fn_convert_currency
...
-- relation-detector-fixture-source: ROUTINE:public.fn_get_project_completion_pct
...
BEGIN
    ...
    v_pct := ROUND(
        ...
        + COALESCE(v_completed_milestones::NUMERIC / NULLIF(v_total_milestones, 0) * 50, 0)
    , 2);
...
-- relation-detector-fixture-source: ROUTINE:public.fn_get_customer_credit_score
...
AS $$
DECLARE
    v_ontime_payment_pct DECIMAL(5,2);
    v_total_orders INT;
    v_return_rate DECIMAL(5,2);
    v_customer_years DECIMAL(4,1);
    v_order_regularity DECIMAL(5,2);
    v_score INT;
BEGIN
    -- 付款及时性: 按时付款的订单占比
    SELECT
    ...
    -- 合作年限
    SELECT EXTRACT(YEAR FROM AGE(CURRENT_DATE, MIN(order_date))) INTO v_customer_years
...
-- relation-detector-fixture-source: ROUTINE:public.fn_get_inspection_pass_rate
...
-- relation-detector-fixture-source: ROUTINE:public.fn_get_customer_status
...
-- relation-detector-fixture-source: ROUTINE:public.fn_get_customer_repurchase_rate
