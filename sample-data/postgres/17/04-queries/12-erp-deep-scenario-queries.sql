...
FROM (
    ...
    WHERE ce.posted_at >= CURRENT_DATE - INTERVAL '180 days'
...
SELECT
    ...
    CASE
        WHEN c.status = 'active'
             AND (MAX(so.order_date) >= CURRENT_DATE - INTERVAL '90 days'
                  OR MAX(p.payment_date) >= CURRENT_DATE - INTERVAL '90 days')
