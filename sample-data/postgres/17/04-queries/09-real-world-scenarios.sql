...
receipt_chain AS (
    SELECT
        ...
        string_agg(DISTINCT prec.receipt_no, ', ' ORDER BY prec.receipt_date) AS receipt_nos,
...
payment_chain AS (
    SELECT
        ...
        string_agg(DISTINCT cj.journal_no, ', ' ORDER BY cj.journal_date) AS payment_nos
...
WHERE pr.requisition_date >= CURRENT_DATE - INTERVAL '6 months'
...
return_stats AS (
    ...
    WHERE sr.return_date >= CURRENT_DATE - INTERVAL '12 months'
...
damage_stats AS (
    ...
    WHERE dr.report_date >= CURRENT_DATE - INTERVAL '12 months'
...
purchaser_stats AS (
    ...
    FROM purchase_orders po
    WHERE po.order_date >= CURRENT_DATE - INTERVAL '3 months'
...
SELECT
    ...
    ROUND(SUM(
        CASE WHEN pb.expiry_date <= CURRENT_DATE + INTERVAL '60 days'
    ...
    ROUND(
        ...
        + w.capacity_m3 * 20
        + SUM(CASE WHEN pb.expiry_date <= CURRENT_DATE + INTERVAL '60 days' AND pb.expiry_date > CURRENT_DATE
              THEN i.quantity * p.purchase_price * 0.30
    ...
    ROUND(
        ...
        / NULLIF((
            ...
            WHERE so2.warehouse_id = w.id
              AND so2.order_date >= CURRENT_DATE - INTERVAL '30 days'
...
WITH monthly_metrics AS (
    SELECT
        ...
        -- 日均销售额
        SUM(total_amount) / EXTRACT(DAY FROM (date_trunc('month', MAX(order_date)) + INTERVAL '1 month' - INTERVAL '1 day')) AS daily_sales,
...
monthly_purchase AS (
    SELECT
        TO_CHAR(order_date, 'YYYY-MM') AS month,
        SUM(total_amount) / EXTRACT(DAY FROM (date_trunc('month', MAX(order_date)) + INTERVAL '1 month' - INTERVAL '1 day')) AS daily_purchase,
...
monthly_inventory AS (
    ...
    FROM (
        ...
        WHERE CURRENT_DATE - seq.num * INTERVAL '1 day' >= CURRENT_DATE - INTERVAL '12 months'
    ) t
...
SELECT
    ...
    CASE
        ...
        WHEN pb.expiry_date <= CURRENT_DATE + INTERVAL '30 days' THEN '30天内到期'
        WHEN pb.expiry_date <= CURRENT_DATE + INTERVAL '90 days' THEN '90天内到期'
        ELSE '正常'
    ...
    -- 入库来源
    (SELECT string_agg(DISTINCT CONCAT(prec.receipt_no, '(', pri.accepted_qty, ')'), ', ')
        FROM purchase_receipt_items pri
        JOIN purchase_receipts prec ON pri.receipt_id = prec.id
        WHERE pri.batch_id = pb.id
    ) AS receipt_sources,
    -- 销售去向
    (SELECT string_agg(DISTINCT CONCAT(so.order_no, '->', c.name, '(', soi.quantity, ')'), ', ')
        FROM sales_order_items soi
        JOIN sales_orders so ON soi.order_id = so.id
        JOIN customers c ON so.customer_id = c.id
    ...
    -- 退货记录
    (SELECT string_agg(DISTINCT CONCAT(sr.return_no, '(', sri.return_qty, ')'), ', ')
        FROM sales_return_items sri
        JOIN sales_returns sr ON sri.return_id = sr.id
    ...
    -- 质量检验
    (SELECT string_agg(DISTINCT CONCAT(ir.report_no, ':', ir.inspection_result), ', ')
...
WITH waterfall_data AS (
    ...
    FROM promotion_usages pu
    WHERE pu.used_at >= CURRENT_DATE - INTERVAL '12 months'
    UNION ALL
    SELECT '9.仓储成本', 9,
    ...
    FROM shipments sh
    WHERE sh.shipped_at >= CURRENT_DATE - INTERVAL '12 months'
    UNION ALL
    SELECT '11.净利', 11,
        ROUND(
            ...
            - COALESCE((SELECT SUM(sr.refund_amount - sr.restock_fee) FROM sales_returns sr
              WHERE sr.return_date >= CURRENT_DATE - INTERVAL '12 months' AND sr.status NOT IN ('rejected')), 0)
            - COALESCE((SELECT SUM(dri.loss_amount) FROM damage_report_items dri
              JOIN damage_reports dr ON dri.report_id = dr.id
              WHERE dr.report_date >= CURRENT_DATE - INTERVAL '12 months' AND dr.status IN ('approved', 'executed')), 0)
            - COALESCE((SELECT SUM(pu.discount_applied) FROM promotion_usages pu
              WHERE pu.used_at >= CURRENT_DATE - INTERVAL '12 months'), 0)
            ...
            - COALESCE((SELECT SUM(sh2.shipping_fee) FROM shipments sh2
              WHERE sh2.shipped_at >= CURRENT_DATE - INTERVAL '12 months'), 0)
        , 2)
)
SELECT
    layer,
...
SELECT
    ...
    (SELECT string_agg(DISTINCT pc.name, ', ' ORDER BY pc.name)
     ...
     WHERE po2.supplier_id = sp.supplier_id
       AND po2.order_date >= CURRENT_DATE - INTERVAL '12 months')
...
SELECT
    ...
    ROUND(COALESCE((SELECT SUM(total_amount) FROM sales_orders
     WHERE order_date BETWEEN TO_CHAR(CURRENT_DATE - INTERVAL '1 month', 'YYYY-MM-01')::DATE
    ...
    ROUND(COALESCE((SELECT SUM(amount) FROM voucher_items vi
     ...
     WHERE v.voucher_date BETWEEN TO_CHAR(CURRENT_DATE - INTERVAL '1 month', 'YYYY-MM-01')::DATE
     AND (date_trunc('month', CURRENT_DATE - INTERVAL '1 month') + INTERVAL '1 month' - INTERVAL '1 day')::DATE
...
SELECT
    ...
    ROUND(COALESCE((SELECT SUM(total_amount) FROM purchase_receipts
     WHERE receipt_date BETWEEN TO_CHAR(CURRENT_DATE - INTERVAL '1 month', 'YYYY-MM-01')::DATE
     AND (date_trunc('month', CURRENT_DATE - INTERVAL '1 month') + INTERVAL '1 month' - INTERVAL '1 day')::DATE), 0), 2),
    ROUND(COALESCE((SELECT SUM(amount) FROM voucher_items vi
     JOIN vouchers v ON vi.voucher_id = v.id
     JOIN accounts a ON vi.account_id = a.id
     WHERE v.voucher_date BETWEEN TO_CHAR(CURRENT_DATE - INTERVAL '1 month', 'YYYY-MM-01')::DATE
    ...
    ROUND(COALESCE((SELECT SUM(total_amount) FROM purchase_receipts
     WHERE receipt_date BETWEEN TO_CHAR(CURRENT_DATE - INTERVAL '1 month', 'YYYY-MM-01')::DATE
     AND (date_trunc('month', CURRENT_DATE - INTERVAL '1 month') + INTERVAL '1 month' - INTERVAL '1 day')::DATE), 0) -
    COALESCE((SELECT SUM(amount) FROM voucher_items vi
     JOIN vouchers v ON vi.voucher_id = v.id
     JOIN accounts a ON vi.account_id = a.id
     WHERE v.voucher_date BETWEEN TO_CHAR(CURRENT_DATE - INTERVAL '1 month', 'YYYY-MM-01')::DATE
...
SELECT
    ...
    -- 临期库存占比
    ROUND(SUM(CASE WHEN pb.expiry_date <= CURRENT_DATE + INTERVAL '60 days' THEN i.quantity ELSE 0 END)
        * 100.0 / NULLIF(SUM(i.quantity), 0), 1) AS near_expiry_pct,
    -- 呆滞库存(>180天无销售)
    ROUND(SUM(CASE WHEN last_sold.last_sale_date IS NULL
        OR last_sold.last_sale_date < CURRENT_DATE - INTERVAL '180 days'
...
WITH computed_commission AS (
    ...
    WHERE sc.period >= TO_CHAR(CURRENT_DATE - INTERVAL '6 months', 'YYYY-MM')
...
salary_paid AS (
    ...
    WHERE sp.salary_month >= TO_CHAR(CURRENT_DATE - INTERVAL '6 months', 'YYYY-MM')
...
store_returns AS (
    ...
    WHERE sr.return_date >= CURRENT_DATE - INTERVAL '3 months'
      AND sr.status NOT IN ('rejected')
