...
SELECT
    ...
    string_agg(
        DISTINCT p.sku || ':' || pb.batch_no || '(' || pb.current_qty || ')',
        '; '
...
WITH monthly_turnover AS (
    SELECT
        ...
        e.gender,
        CASE
            WHEN EXTRACT(YEAR FROM AGE(e.resignation_date, e.hire_date)) <= 1 THEN '1年内'
            WHEN EXTRACT(YEAR FROM AGE(e.resignation_date, e.hire_date)) <= 3 THEN '1-3年'
            WHEN EXTRACT(YEAR FROM AGE(e.resignation_date, e.hire_date)) <= 5 THEN '3-5年'
            WHEN EXTRACT(YEAR FROM AGE(e.resignation_date, e.hire_date)) <= 10 THEN '5-10年'
            ELSE '10年以上'
        END AS tenure_group,
    ...
    WHERE e.status = 'resigned'
      AND e.resignation_date >= CURRENT_DATE - INTERVAL '12 months'
...
SELECT
    ...
    (SELECT COUNT(*)
     ...
     WHERE sub.manager_id = e.id
       ...
       AND sub.resignation_date >= CURRENT_DATE - INTERVAL '6 months'
    ) AS team_resigned_6m,
    -- 团队总人数
    (SELECT COUNT(*)
     FROM employees sub
...
WITH dept_staffing AS (
    SELECT
        ...
        COUNT(CASE WHEN e.status = 'resigned' AND e.resignation_date >= CURRENT_DATE - INTERVAL '30 days' THEN 1 END) AS recent_resigned,
...
WITH book_transactions AS (
    ...
    WHERE cj.account_id = 3  -- 工商银行基本户
      AND cj.journal_date >= CURRENT_DATE - INTERVAL '30 days'
),
recon_summary AS (
    SELECT
...
WITH RECURSIVE date_series AS (
    SELECT (date_trunc('month', CURRENT_DATE) + INTERVAL '1 month')::DATE AS forecast_date
    UNION ALL
    SELECT (forecast_date + INTERVAL '1 month')::DATE
    FROM date_series
    WHERE forecast_date < (CURRENT_DATE + INTERVAL '12 months')::DATE
),
asset_forecast AS (
    SELECT
        fa.id,
...
SELECT
    ...
    current_accumulated + monthly_depreciation * (
        EXTRACT(YEAR FROM AGE(forecast_date, CURRENT_DATE)) * 12 + EXTRACT(MONTH FROM AGE(forecast_date, CURRENT_DATE)) + 1
...
baseline_sales AS (
    ...
    CROSS JOIN (
        ...
        FROM sales_orders so
        WHERE so.order_date BETWEEN CURRENT_DATE - INTERVAL '60 days' AND CURRENT_DATE - INTERVAL '30 days'
          AND so.status NOT IN ('draft', 'cancelled')
...
SELECT
    ...
    (SELECT string_agg(pp.name || ':' || wom.actual_consumed || wom.unit, ', ' ORDER BY pp.name)
...
WITH ticket_metrics AS (
    ...
    WHERE st.created_at >= CURRENT_DATE - INTERVAL '3 months'
...
WITH customer_behavior AS (
    SELECT
        ...
        COUNT(DISTINCT CASE WHEN so.order_date >= CURRENT_DATE - INTERVAL '3 months' THEN so.id END) AS orders_3m,
        COUNT(DISTINCT CASE WHEN so.order_date >= CURRENT_DATE - INTERVAL '6 months'
            AND so.order_date < CURRENT_DATE - INTERVAL '3 months' THEN so.id END) AS orders_3_6m,
        COALESCE(SUM(CASE WHEN so.order_date >= CURRENT_DATE - INTERVAL '3 months' THEN so.total_amount END), 0) AS amount_3m,
        COALESCE(SUM(CASE WHEN so.order_date >= CURRENT_DATE - INTERVAL '6 months'
            AND so.order_date < CURRENT_DATE - INTERVAL '3 months' THEN so.total_amount END), 0) AS amount_3_6m,
...
UNION ALL
SELECT '人力健康度', '本月离职人数',
    (SELECT COUNT(*) FROM employees WHERE status = 'resigned' AND resignation_date >= TO_CHAR(CURRENT_DATE, 'YYYY-MM-01')::DATE),
    NULL, '人', 'lower_better';
...
WITH RECURSIVE bom_explosion AS (
    ...
    SELECT
        ...
        b.scrap_rate,
        1 AS bom_level,
        CAST(b.parent_product_id AS VARCHAR(200)) AS bom_path,
        CAST(p_parent.sku || ' -> ' || p_child.sku AS VARCHAR(500)) AS bom_path_names
    FROM boms b
