...
scored_customers AS (
    SELECT
        *,
        ...
        -- 月均消费
        ROUND(total_spent / NULLIF(EXTRACT(YEAR FROM AGE(CURRENT_DATE, CURRENT_DATE - days_since_first * INTERVAL '1 day')) * 12 + EXTRACT(MONTH FROM AGE(CURRENT_DATE, CURRENT_DATE - days_since_first * INTERVAL '1 day')) + 1, 0), 2) AS monthly_avg_spent,
        -- 月均频次
        ROUND(order_count / NULLIF(EXTRACT(YEAR FROM AGE(CURRENT_DATE, CURRENT_DATE - days_since_first * INTERVAL '1 day')) * 12 + EXTRACT(MONTH FROM AGE(CURRENT_DATE, CURRENT_DATE - days_since_first * INTERVAL '1 day')) + 1, 0), 2) AS monthly_avg_orders,
        -- 退货率
...
-- ============================================================
-- Q41: 客户品类偏好与交叉购买分析
-- 语法: CTE + 自连接 + 条件聚合 + string_agg + 偏好度计算
...
WITH monthly_total AS (
    SELECT
        ...
        ROUND(SUM(so.total_amount) / NULLIF(COUNT(DISTINCT so.id), 0), 2) AS avg_order_value
    FROM sales_orders so
    WHERE so.order_date >= CURRENT_DATE - INTERVAL '24 months'
      AND so.status NOT IN ('draft', 'cancelled')
...
WITH monthly_tax AS (
    ...
    WHERE ti.tax_period >= TO_CHAR(CURRENT_DATE - INTERVAL '12 months', 'YYYY-MM')
...
WITH approval_timeline AS (
    ...
    WHERE ai.submitted_at >= CURRENT_DATE - INTERVAL '3 months'
),
node_efficiency AS (
    SELECT
        an.workflow_id,
...
SELECT
    ...
    -- 流转历史摘要
    (SELECT string_agg(
        TO_CHAR(snl.event_time, 'YYYY-MM-DD') || ':' || snl.event_type,
        ' -> '
        ORDER BY snl.event_time
