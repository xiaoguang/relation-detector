...
monthly_active AS (
    ...
    SELECT DISTINCT
        fp.cohort_month,
        fp.customer_id,
        TO_CHAR(so.order_date, 'YYYY-MM') AS active_month,
        EXTRACT(YEAR FROM AGE(TO_DATE(TO_CHAR(so.order_date, 'YYYY-MM') || '-01', 'YYYY-MM-DD'), TO_DATE(fp.cohort_month || '-01', 'YYYY-MM-DD'))) * 12
        + EXTRACT(MONTH FROM AGE(TO_DATE(TO_CHAR(so.order_date, 'YYYY-MM') || '-01', 'YYYY-MM-DD'), TO_DATE(fp.cohort_month || '-01', 'YYYY-MM-DD'))) AS month_number
...
-- 语法: 多层CTE + LAG多期 + 窗口函数嵌套 + GROUPING SETS
-- 统计原理: 环比增长率 = (本期-上期)/上期*100%
--           通过GROUPING SETS实现多维度汇总
-- ============================================================
...
WITH monthly_purchase AS (
    ...
    GROUP BY p.category_id, pc.name, s.id, s.name, TO_CHAR(po.order_date, 'YYYY-MM')
),
price_trend AS (
    SELECT
        category_name,
...
WITH sales_stats AS (
    SELECT
        ...
        AVG((COALESCE(so.delivery_date, so.order_date) - so.order_date)) AS avg_delivery_days
    FROM sales_orders so
...
rule1_large_transaction AS (
    SELECT so.id AS order_id, so.order_no, so.order_date, so.total_amount,
           '大额交易' AS alert_type,
           '金额=' || so.total_amount || ' 超出均值+3σ=' || ROUND(ss.avg_amount + 3 * ss.stddev_amount, 2) AS alert_detail
    FROM sales_orders so
...
rule2_abnormal_frequency AS (
    SELECT so.id AS order_id, so.order_no, so.order_date, so.total_amount,
           ...
           '同日订单数=' || freq.cnt AS alert_detail
    FROM sales_orders so
    JOIN (
...
rule3_abnormal_discount AS (
    SELECT so.id AS order_id, so.order_no, so.order_date, so.total_amount,
           '异常折扣' AS alert_type,
           '折扣率=' || ROUND(so.discount_amount * 100.0 / NULLIF(so.total_amount + so.discount_amount, 0), 2) || '%' AS alert_detail
    FROM sales_orders so
...
rule4_high_return AS (
    SELECT so.id AS order_id, so.order_no, so.order_date, so.total_amount,
           '高退货率' AS alert_type,
           '退货次数=' || ret.return_count AS alert_detail
    FROM sales_orders so
    JOIN (
...
rule5_slow_payment AS (
    SELECT so.id AS order_id, so.order_no, so.order_date, so.total_amount,
           '回款延迟' AS alert_type,
           '已超期=' || (CURRENT_DATE - (so.order_date + c.credit_days * INTERVAL '1 day')) || '天' AS alert_detail
    FROM sales_orders so
    JOIN customers c ON so.customer_id = c.id
    WHERE so.status = 'delivered'
      AND so.paid_amount < so.total_amount
      AND so.order_date + c.credit_days * INTERVAL '1 day' < CURRENT_DATE
)
...
WITH income_statement AS (
    SELECT
        TO_CHAR(so.order_date, 'YYYY') AS fiscal_year,
    ...
    GROUP BY TO_CHAR(so.order_date, 'YYYY')
),
balance_sheet AS (
    SELECT
        '2024' AS fiscal_year,
...
-- ============================================================
-- Q33: 工资总额与人力成本分析 - GROUPING SETS多维汇总
-- 语法: GROUPING SETS + 条件聚合 + 多层汇总 + 预算对比
-- 统计原理: 按部门+月份交叉汇总，同时计算ROLLUP总合计
-- ============================================================

SELECT
    CASE WHEN GROUPING(d.name) = 1 THEN '【全部部门】' ELSE d.name END AS department_name,
    CASE WHEN GROUPING(sp.salary_month) = 1 THEN '【全部月份】' ELSE sp.salary_month END AS salary_month,
    COUNT(DISTINCT sp.employee_id) AS employee_count,
    SUM(sp.base_salary) AS total_base_salary,
    SUM(sp.overtime_pay) AS total_overtime,
    SUM(sp.bonus) AS total_bonus,
    SUM(sp.deduction) AS total_deduction,
    SUM(sp.social_security_personal) AS total_ss_personal,
    SUM(sp.social_security_company) AS total_ss_company,
    SUM(sp.housing_fund_personal) AS total_hf_personal,
    SUM(sp.housing_fund_company) AS total_hf_company,
    SUM(sp.income_tax) AS total_income_tax,
    SUM(sp.net_pay) AS total_net_pay,
    SUM(sp.net_pay + sp.social_security_company + sp.housing_fund_company) AS total_company_cost,
    ROUND(SUM(sp.net_pay + sp.social_security_company + sp.housing_fund_company) / NULLIF(COUNT(DISTINCT sp.employee_id), 0), 2) AS avg_cost_per_head,
    ROUND(SUM(sp.social_security_company + sp.housing_fund_company) * 100.0
        / NULLIF(SUM(sp.base_salary), 0), 2) AS company_benefit_rate_pct,
    GROUPING(d.name) AS is_dept_total,
    GROUPING(sp.salary_month) AS is_month_total
FROM salary_payments sp
JOIN employees e ON sp.employee_id = e.id
JOIN departments d ON e.department_id = d.id
WHERE sp.salary_month >= TO_CHAR(CURRENT_DATE - INTERVAL '6 months', 'YYYY-MM')
GROUP BY GROUPING SETS (
    (d.name, sp.salary_month),  -- 明细: 部门+月份
    (d.name),                    -- 小计: 按部门
    (sp.salary_month),           -- 小计: 按月份
    ()                           -- 总计
)
ORDER BY
    CASE WHEN GROUPING(d.name) = 1 THEN 1 ELSE 0 END,
    d.name,
    sp.salary_month DESC;
...
WITH shipping_metrics AS (
    SELECT
        s.carrier,
        s.shipping_method,
        TO_CHAR(s.shipped_at, 'YYYY-MM') AS ship_month,
        COUNT(*) AS total_shipments,
        AVG(EXTRACT(EPOCH FROM (COALESCE(s.delivered_at, CURRENT_TIMESTAMP) - s.shipped_at)) / 3600) AS avg_delivery_hours,
        AVG((COALESCE(s.actual_delivery_date, CURRENT_DATE) - s.estimated_delivery_date)) AS avg_delay_days,
    ...
    WHERE s.shipped_at >= CURRENT_DATE - INTERVAL '6 months'
    GROUP BY s.carrier, s.shipping_method, TO_CHAR(s.shipped_at, 'YYYY-MM')
)
SELECT
    carrier,
    shipping_method,
...
WITH budget_actual AS (
    SELECT
        ...
        EXTRACT(MONTH FROM CURRENT_DATE) AS months_passed,
        ROUND(d.budget * EXTRACT(MONTH FROM CURRENT_DATE) / 12.0, 2) AS prorated_budget_ytd,
        -- 实际支出: 采购+工资
        COALESCE((
        ...
        COALESCE((
            ...
            WHERE e.department_id = d.id
              AND TO_CHAR(sp.salary_month, 'YYYY') = TO_CHAR(CURRENT_DATE, 'YYYY')
...
SELECT
    ...
    ROUND(EXTRACT(MONTH FROM CURRENT_DATE) * 100.0 / 12, 2) AS time_elapsed_pct,
...
WITH monthly_attendance AS (
    SELECT
        ...
        e.department_id,
        d.name AS dept_name,
        TO_CHAR(a.attendance_date, 'YYYY-MM') AS att_month,
    ...
    GROUP BY e.id, e.name, e.department_id, d.name, TO_CHAR(a.attendance_date, 'YYYY-MM')
)
SELECT
    name,
    dept_name,
...
WITH wh_throughput AS (
    ...
    WHERE it.created_at >= CURRENT_DATE - INTERVAL '3 months'
...
wh_shipments AS (
    SELECT
        warehouse_id,
        COUNT(*) AS total_shipments,
        AVG(EXTRACT(EPOCH FROM (COALESCE(delivered_at, CURRENT_TIMESTAMP) - shipped_at)) / 3600) AS avg_delivery_hours,
        COUNT(CASE WHEN actual_delivery_date <= estimated_delivery_date THEN 1 END) AS on_time_shipments
    FROM shipments
    WHERE shipped_at >= CURRENT_DATE - INTERVAL '3 months'
