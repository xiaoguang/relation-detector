...
SELECT
    ...
    -- 7天内
    SUM(CASE WHEN pb.expiry_date BETWEEN CURDATE() AND DATE_ADD(CURDATE(), INTERVAL 7 DAY) THEN i.quantity ELSE 0 END) AS expiry_7d,
    -- 30天内
    SUM(CASE WHEN pb.expiry_date BETWEEN DATE_ADD(CURDATE(), INTERVAL 8 DAY) AND DATE_ADD(CURDATE(), INTERVAL 30 DAY) THEN i.quantity ELSE 0 END) AS expiry_30d,
    -- 60天内
    SUM(CASE WHEN pb.expiry_date BETWEEN DATE_ADD(CURDATE(), INTERVAL 31 DAY) AND DATE_ADD(CURDATE(), INTERVAL 60 DAY) THEN i.quantity ELSE 0 END) AS expiry_60d,
    -- 临期率
    ROUND(
    ...
    -- 临期库存可售天数(基于日均销量)
    ROUND(
        SUM(CASE WHEN pb.expiry_date <= DATE_ADD(CURDATE(), INTERVAL 60 DAY) THEN i.quantity ELSE 0 END)
        / NULLIF(COALESCE((
    ...
    -- 严重程度
    CASE
        WHEN SUM(CASE WHEN pb.expiry_date < CURDATE() THEN i.quantity ELSE 0 END) > 0 THEN '已过期'
...
cat_inventory AS (
    SELECT
        ...
        SUM(CASE WHEN pb.expiry_date <= DATE_ADD(CURDATE(), INTERVAL 60 DAY) AND pb.status = 'active'
            THEN i.quantity * p.purchase_price ELSE 0 END) AS near_expiry_value,
        COUNT(DISTINCT pb.id) AS batch_count,
        AVG(p.shelf_life_days) AS avg_shelf_life,
        AVG(DATEDIFF(CURDATE(), pb.production_date)) AS avg_age_days
    FROM products p
...
SELECT
    ...
    CASE
        WHEN prev_sale_production_date IS NOT NULL
            AND production_date < prev_sale_production_date
        THEN DATEDIFF(prev_sale_production_date, production_date)
        ELSE 0
...
WITH batch_age AS (
    SELECT
        ...
        pb.expiry_date,
        pb.current_qty,
        DATEDIFF(CURDATE(), pb.production_date) AS age_days,
...
SELECT
    ...
    GROUP_CONCAT(DISTINCT store_name ORDER BY store_name SEPARATOR ', ') AS stores
FROM batch_age ba
...
product_expiry_risk AS (
    SELECT
        p.id AS product_id,
        SUM(CASE WHEN pb.expiry_date <= DATE_ADD(CURDATE(), INTERVAL 60 DAY) THEN i.quantity ELSE 0 END) AS near_expiry_qty,
        SUM(i.quantity) AS total_stock,
        ROUND(
            SUM(CASE WHEN pb.expiry_date <= DATE_ADD(CURDATE(), INTERVAL 60 DAY) THEN i.quantity ELSE 0 END) * 100.0
            / NULLIF(SUM(i.quantity), 0), 2
...
WITH store_expiry_stats AS (
    SELECT
        ...
        SUM(i.quantity * p.purchase_price) AS total_stock_value,
        SUM(CASE WHEN pb.expiry_date < CURDATE() THEN i.quantity ELSE 0 END) AS expired_qty,
        ROUND(SUM(CASE WHEN pb.expiry_date < CURDATE() THEN i.quantity * p.purchase_price ELSE 0 END), 2) AS expired_value,
        SUM(CASE WHEN pb.expiry_date BETWEEN CURDATE() AND DATE_ADD(CURDATE(), INTERVAL 30 DAY) THEN i.quantity ELSE 0 END) AS near_expiry_30d,
        ROUND(SUM(CASE WHEN pb.expiry_date BETWEEN CURDATE() AND DATE_ADD(CURDATE(), INTERVAL 30 DAY) THEN i.quantity * p.purchase_price ELSE 0 END), 2) AS near_expiry_30d_value,
        ROUND(
            SUM(CASE WHEN pb.expiry_date < CURDATE() THEN i.quantity ELSE 0 END) * 100.0
            / NULLIF(SUM(i.quantity), 0), 2
        ) AS expired_rate_pct,
        ROUND(
            SUM(CASE WHEN pb.expiry_date <= DATE_ADD(CURDATE(), INTERVAL 30 DAY) THEN i.quantity ELSE 0 END) * 100.0
            / NULLIF(SUM(i.quantity), 0), 2
        ) AS at_risk_rate_pct,
...
store_recent_stocktake AS (
    SELECT
        ...
        COUNT(CASE WHEN created_at >= DATE_SUB(CURDATE(), INTERVAL 30 DAY) THEN 1 END) AS stocktake_count_30d
...
SELECT
    ...
    CASE
        ...
        WHEN ses.last_stocktake < DATE_SUB(CURDATE(), INTERVAL 30 DAY) THEN '盘点不及时-需安排盘点'
...
SELECT
    ...
    CASE
        ...
        WHEN pb.expiry_date <= DATE_ADD(CURDATE(), INTERVAL 30 DAY) THEN '临期'
        WHEN pb.current_qty = pb.initial_qty AND DATEDIFF(CURDATE(), pb.production_date) > p.shelf_life_days * 0.5
            THEN '在库时间过长'
