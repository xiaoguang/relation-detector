...
WHERE st.stocktake_date >= DATE_SUB(CURDATE(), INTERVAL 90 DAY)
...
WHERE tr.transfer_date >= DATE_SUB(CURDATE(), INTERVAL 180 DAY)
...
SELECT
    ...
    GROUP_CONCAT(CONCAT(pra.reference_type, '#', pra.reference_id) ORDER BY pra.reference_type SEPARATOR ', ') AS allocated_refs
...
WITH RECURSIVE operation_chain AS (
    SELECT
        ...
        CAST(po.operation_name AS CHAR(500)) AS operation_path,
        po.standard_minutes
    ...
    SELECT
        ...
        po.operation_name,
        po.predecessor_operation_id,
        CONCAT(oc.operation_path, ' -> ', po.operation_name) AS operation_path,
