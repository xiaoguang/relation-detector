...
SELECT
    ...
    -- 距上次购买天数
    DATEDIFF(so.order_date, LAG(so.order_date) OVER (PARTITION BY so.customer_id ORDER BY so.order_date)) AS days_since_last,
    -- 该门店该品类排名
...
WITH weekly_sales AS (
    SELECT
        ...
        pc.name AS category_name,
        CASE
            WHEN so.order_date >= DATE_SUB(CURDATE(), INTERVAL WEEKDAY(CURDATE()) DAY) THEN '本周'
            ELSE '上周'
        END AS week_label,
    ...
    JOIN products p ON soi.product_id = p.id
    JOIN product_categories pc ON p.category_id = pc.id
    WHERE so.order_date >= DATE_SUB(CURDATE(), INTERVAL WEEKDAY(CURDATE()) + 7 DAY)
...
SELECT
    ...
    CASE
        ...
        WHEN tw.amount_rank < lw.amount_rank THEN CONCAT('上升', lw.amount_rank - tw.amount_rank, '位')
        WHEN tw.amount_rank > lw.amount_rank THEN CONCAT('下降', tw.amount_rank - lw.amount_rank, '位')
        ELSE '持平'
    END AS rank_change,
...
WITH hourly_sales AS (
    ...
    WHERE so.order_date >= DATE_SUB(CURDATE(), INTERVAL 90 DAY)
      AND so.status NOT IN ('draft', 'cancelled')
    GROUP BY w.name, w.id, HOUR(so.created_at), DAYOFWEEK(so.order_date)
),
time_segment AS (
    SELECT
        store_name,
...
ORDER BY store_name, FIELD(day_of_week, '周一','周二','周三','周四','周五','周六','周日'),
    FIELD(time_segment, '上午(8-12)','午间(12-14)','下午(14-18)','晚间(18-22)','夜间(22-8)');
...
WITH store_category_sales AS (
    ...
    WHERE so.order_date >= DATE_SUB(CURDATE(), INTERVAL 6 MONTH)
      AND so.status NOT IN ('draft', 'cancelled')
    GROUP BY w.id, w.name, pc.id, pc.name, DATE_FORMAT(so.order_date, '%Y-%m')
),
store_total AS (
...
WITH store_customer_rfm AS (
    SELECT
        ...
        c.name AS customer_name,
        c.membership_level,
        DATEDIFF(CURDATE(), MAX(so.order_date)) AS recency,
...
SELECT
    ...
    GROUP_CONCAT(DISTINCT customer_name ORDER BY customer_name SEPARATOR ', ') AS sample_customers,
    -- 推荐品类在门店的畅销品
...
HAVING opportunity_customer_count >= 3
...
monthly_store_sales AS (
    SELECT
        ...
        so.customer_id,
        CASE
            WHEN cfp.first_purchase_date >= DATE_FORMAT(so.order_date, '%Y-%m-01') THEN '新客'
            ELSE '老客'
...
store_churn AS (
    SELECT
        so.warehouse_id,
        COUNT(DISTINCT CASE WHEN DATEDIFF(CURDATE(), MAX(so.order_date)) > 90
...
WITH customer_region AS (
    SELECT
        so.warehouse_id,
        w.name AS store_name,
        SUBSTRING_INDEX(c.address, '市', 1) AS city,
        COUNT(DISTINCT so.customer_id) AS customer_count,
        COUNT(DISTINCT so.id) AS order_count,
        SUM(so.total_amount) AS total_sales,
        AVG(so.total_amount) AS avg_order_value,
        AVG(DATEDIFF(COALESCE(s.actual_delivery_date, s.estimated_delivery_date), s.shipped_at)) AS avg_delivery_days
    FROM sales_orders so
    JOIN warehouses w ON so.warehouse_id = w.id
    JOIN customers c ON so.customer_id = c.id
    LEFT JOIN shipments s ON so.id = s.order_id
    WHERE so.status NOT IN ('draft', 'cancelled')
    GROUP BY so.warehouse_id, w.name, SUBSTRING_INDEX(c.address, '市', 1)
)
SELECT
    store_name,
