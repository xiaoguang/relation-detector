...
WITH customer_metrics AS (
    SELECT
        ...
        -- 时间维度
        DATEDIFF(CURDATE(), MAX(so.order_date)) AS days_since_last,
        DATEDIFF(CURDATE(), MIN(so.order_date)) AS days_since_first,
        -- 客单价
...
scored_customers AS (
    SELECT
        *,
        ...
        -- 月均消费
        ROUND(total_spent / NULLIF(TIMESTAMPDIFF(MONTH, DATE_SUB(CURDATE(), INTERVAL days_since_first DAY), CURDATE()) + 1, 0), 2) AS monthly_avg_spent,
        -- 月均频次
        ROUND(order_count / NULLIF(TIMESTAMPDIFF(MONTH, DATE_SUB(CURDATE(), INTERVAL days_since_first DAY), CURDATE()) + 1, 0), 2) AS monthly_avg_orders,
        -- 退货率
...
WITH monthly_customer_spend AS (
    SELECT
        so.customer_id,
        c.name AS customer_name,
        DATE_FORMAT(so.order_date, '%Y-%m') AS spend_month,
    ...
    GROUP BY so.customer_id, c.name, DATE_FORMAT(so.order_date, '%Y-%m')
),
trend_analysis AS (
    SELECT
        customer_id,
...
-- ============================================================
-- Q41: 客户品类偏好与交叉购买分析
-- 语法: CTE + 自连接 + 条件聚合 + GROUP_CONCAT + 偏好度计算
...
customer_category_mix AS (
    SELECT
        ...
        customer_name,
        GROUP_CONCAT(
            CASE WHEN preference_rank <= 3 THEN category_name END
            ORDER BY preference_rank SEPARATOR ' + '
        ) AS top3_categories,
        GROUP_CONCAT(
            CONCAT(category_name, ':', preference_pct, '%')
            ORDER BY preference_rank SEPARATOR ' | '
        ) AS category_distribution,
        SUM(CASE WHEN preference_rank = 1 THEN preference_pct ELSE 0 END) AS top1_pct,
        COUNT(DISTINCT category_id) AS category_count,
        -- 品类集中度 (HHI指数，越高越集中)
        ROUND(SUM(POW(preference_pct / 100.0, 2)) * 10000, 0) AS hhi_index
...
WITH customer_payment AS (
    SELECT
        ...
        DATEDIFF(CURDATE(), DATE_ADD(so.order_date, INTERVAL c.credit_days DAY)) AS overdue_days,
...
purchase_intervals AS (
    SELECT
        ...
        order_seq,
        prev_order_date,
        DATEDIFF(order_date, prev_order_date) AS days_since_last_purchase
...
customer_repurchase_stats AS (
    SELECT
        ...
        (SELECT days_since_last_purchase FROM purchase_intervals pi2
         WHERE pi2.customer_id = pi.customer_id ORDER BY order_date DESC LIMIT 1) AS latest_interval,
        -- 间隔趋势 (最近3次平均 vs 总体平均)
        (SELECT AVG(days_since_last_purchase) FROM (
            SELECT days_since_last_purchase FROM purchase_intervals pi3
            WHERE pi3.customer_id = pi.customer_id ORDER BY order_date DESC LIMIT 3
...
SELECT
    ...
    rs.first_order,
    rs.last_order,
    DATEDIFF(CURDATE(), rs.last_order) AS days_since_last,
    ...
    -- 预计下次购买日期
    DATE_ADD(rs.last_order, INTERVAL ROUND(rs.avg_purchase_interval, 0) DAY) AS predicted_next_purchase,
    -- 是否已超期未购
    CASE
        WHEN DATEDIFF(CURDATE(), rs.last_order) > rs.avg_purchase_interval * 1.5 THEN '超期未购-需关注'
        WHEN DATEDIFF(CURDATE(), rs.last_order) > rs.avg_purchase_interval THEN '已达预期间隔'
        ELSE '正常'
...
WITH monthly_total AS (
    SELECT
        ...
        ROUND(SUM(so.total_amount) / NULLIF(COUNT(DISTINCT so.id), 0), 2) AS avg_order_value
    FROM sales_orders so
    WHERE so.order_date >= DATE_SUB(CURDATE(), INTERVAL 24 MONTH)
      AND so.status NOT IN ('draft', 'cancelled')
...
WITH monthly_tax AS (
    ...
    WHERE ti.tax_period >= DATE_FORMAT(DATE_SUB(CURDATE(), INTERVAL 12 MONTH), '%Y-%m')
...
WITH approval_timeline AS (
    SELECT
        ...
        ai.submitted_at,
        ai.completed_at,
        TIMESTAMPDIFF(HOUR, ai.submitted_at, COALESCE(ai.completed_at, NOW())) AS total_hours,
        ai.status,
        ai.current_node_level,
        ai.total_nodes,
        (SELECT AVG(TIMESTAMPDIFF(HOUR, ai2.submitted_at, COALESCE(ai2.completed_at, NOW())))
    ...
    WHERE ai.submitted_at >= DATE_SUB(CURDATE(), INTERVAL 3 MONTH)
),
node_efficiency AS (
    SELECT
        an.workflow_id,
        ...
        AVG(TIMESTAMPDIFF(HOUR, ai.submitted_at, ar.action_at)) AS avg_node_hours,
...
SELECT
    ...
    (SELECT ne.node_name FROM node_efficiency ne
     ...
     ORDER BY ne.avg_node_hours DESC LIMIT 1
    ) AS bottleneck_node,
    ...
    (SELECT ne.node_name FROM node_efficiency ne
     WHERE ne.workflow_id = at.workflow_id
     ORDER BY ne.reject_rate DESC LIMIT 1
...
WITH project_metrics AS (
    SELECT
        ...
        p.actual_end_date,
        p.status,
        DATEDIFF(COALESCE(p.actual_end_date, CURDATE()), p.start_date) AS elapsed_days,
        DATEDIFF(p.planned_end_date, p.start_date) AS planned_days,
        ROUND(DATEDIFF(COALESCE(p.actual_end_date, CURDATE()), p.start_date) * 100.0
            / NULLIF(DATEDIFF(p.planned_end_date, p.start_date), 0), 2) AS time_elapsed_pct,
...
SELECT
    ...
    (SELECT pri.created_at FROM purchase_receipt_items pri WHERE pri.product_id = sn.product_id LIMIT 1) AS inbound_time,
    -- 销售出库信息
    ...
    sn.warranty_end,
    CASE
        WHEN sn.warranty_end IS NOT NULL AND sn.warranty_end < CURDATE() THEN '已过保'
        WHEN sn.warranty_end IS NOT NULL THEN CONCAT('保修中(剩余', DATEDIFF(sn.warranty_end, CURDATE()), '天)')
        ELSE '无保修'
    ...
    (SELECT snl.event_time FROM serial_number_logs snl
     WHERE snl.serial_number_id = sn.id ORDER BY snl.event_time DESC LIMIT 1) AS last_event_time,
    -- 流转历史摘要
    (SELECT GROUP_CONCAT(
        CONCAT(DATE_FORMAT(snl.event_time, '%Y-%m-%d'), ':', snl.event_type)
        ORDER BY snl.event_time SEPARATOR ' -> '
     ) FROM serial_number_logs snl WHERE snl.serial_number_id = sn.id
    ) AS lifecycle_path
...
SELECT
    ...
    fn_get_attendance_rate(lb.employee_id, DATE_FORMAT(CURDATE(), '%Y-%m')) AS attendance_rate_this_month,
    CASE
        WHEN lb.annual_leave_remaining < 3 AND MONTH(CURDATE()) < 10 THEN '年假余额偏低'
