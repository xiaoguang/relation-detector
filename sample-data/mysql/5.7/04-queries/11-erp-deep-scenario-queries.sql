...
SELECT
    mr.id AS mrp_run_id,
    mri.product_id,
    mri.required_quantity,
    mri.planned_purchase_quantity,
    pp.plan_no
FROM mrp_runs mr
JOIN mrp_run_items mri ON mri.mrp_run_id = mr.id
JOIN production_plans pp ON mr.production_plan_id = pp.id;
