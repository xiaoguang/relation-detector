...
SELECT
    ...
    TRUNC(MONTHS_BETWEEN(CURRENT_DATE, e.hire_date) / 12) AS years_of_service,
    TRUNC(MONTHS_BETWEEN(CURRENT_DATE, e.birth_date) / 12) AS age
FROM employees e
...
SELECT
    ...
    COUNT(CASE WHEN e.status = 'resigned' AND e.resignation_date >= CURRENT_DATE - INTERVAL '30' DAY THEN 1 END) AS resigned_30d,
    COUNT(CASE WHEN e.status = 'resigned' AND e.resignation_date >= CURRENT_DATE - INTERVAL '90' DAY THEN 1 END) AS resigned_90d,
