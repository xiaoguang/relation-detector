...
AS
    ...
    v_from_before NUMBER(10);
    v_to_before NUMBER(10);
BEGIN
    ...
    IF v_available < p_quantity THEN
        RAISE_APPLICATION_ERROR(-20000, '调出仓库库存不足: 可用=%, 调拨=%');
    END IF;
    ...
    VALUES ('transfer_inventory', 'inventory', p_operator_id,
            JSON_OBJECT('product_id' VALUE p_product_id, 'quantity' VALUE p_quantity, 'from_wh' VALUE p_from_warehouse_id, 'to_wh' VALUE p_to_warehouse_id));
...
BEGIN
    ...
    ELSE
        ...
        VALUES ('stocktake', 'inventory', p_operator_id,
                JSON_OBJECT('quantity' VALUE v_book_qty),
                JSON_OBJECT('quantity' VALUE p_actual_qty, 'diff' VALUE v_diff));
...
BEGIN
    ...
    LOOP
        ...
        VALUES (rec.product_id, rec.batch_id, rec.warehouse_id, 'scrap_out',
            ...
            ('批号过期报废: batch_id=' || rec.batch_id));

    END LOOP;

    INSERT INTO audit_log (action, target_type, employee_id, new_value)
    VALUES ('process_expired_batches', 'batch', p_operator_id,
            JSON_OBJECT('processed_date' VALUE CURRENT_DATE));
...
BEGIN
    ...
    IF v_status = 'frozen' THEN
        RAISE_APPLICATION_ERROR(-20000, '客户已被冻结，无法交易');
    END IF;

    IF v_status = 'blacklist' THEN
        RAISE_APPLICATION_ERROR(-20000, '客户在黑名单中，无法交易');
    END IF;
...
AS
    v_start_date DATE;
    v_end_date DATE;
    v_total_revenue NUMBER(18,2);
    v_total_cost NUMBER(18,2);
    v_total_salary NUMBER(18,2);
    v_total_expense NUMBER(18,2);
    v_gross_profit NUMBER(18,2);
    v_net_profit NUMBER(18,2);
BEGIN
...
AS
    ...
    v_total NUMBER(18,2);
    v_items_json CLOB;
    v_customer_count NUMBER(10);
    v_salesperson_count NUMBER(10);
    v_product_count NUMBER(10);
...
BEGIN
    ...
    WHILE v_day < p_days LOOP
        ...
        -- 每天生成3-8个订单
        v_i := 0;
        WHILE v_i < FLOOR(DBMS_RANDOM.VALUE * 6) + 3 LOOP
            v_customer_id := FLOOR(DBMS_RANDOM.VALUE * v_customer_count) + 1;
            v_salesperson_id := FLOOR(DBMS_RANDOM.VALUE * v_salesperson_count) + 1;
            v_warehouse_id := FLOOR(DBMS_RANDOM.VALUE * 3) + 1;
            v_order_no := ('SO-' || TO_CHAR(v_order_date, 'YYYYMMDD') || '-' || lpad((FLOOR(DBMS_RANDOM.VALUE * 9999) + 1), 4, '0'));
            ...
            VALUES (v_order_no, v_customer_id, v_salesperson_id, v_warehouse_id,
                v_order_date, ROUND(DBMS_RANDOM.VALUE * 50, 2),
                CASE FLOOR(DBMS_RANDOM.VALUE * 6) + 1
                    WHEN 1 THEN 'cash'
                    WHEN 2 THEN 'card'
                    ...
                    WHEN 6 THEN 'alipay'
                END,
                CASE FLOOR(DBMS_RANDOM.VALUE * 3) + 1
            ...
            v_total := 0;
            v_i := v_i + 1;
            v_item_count := FLOOR(DBMS_RANDOM.VALUE * 5) + 1;

            WHILE v_item_count > 0 LOOP
                v_product_id := FLOOR(DBMS_RANDOM.VALUE * v_product_count) + 1;
                v_qty := FLOOR(DBMS_RANDOM.VALUE * 10) + 1;
            ...
            SET total_amount = v_total,
                paid_amount = CASE WHEN status = 'delivered' THEN v_total * (0.8 + DBMS_RANDOM.VALUE * 0.2) ELSE 0 END
            WHERE id = v_order_id;
...
BEGIN
    ...
    WHILE v_day < p_days LOOP
        ...
        -- 每天2-5个采购单
        v_item_count := FLOOR(DBMS_RANDOM.VALUE * 4) + 2;
        WHILE v_item_count > 0 LOOP
            v_supplier_id := FLOOR(DBMS_RANDOM.VALUE * v_supplier_count) + 1;
            v_purchaser_id := FLOOR(DBMS_RANDOM.VALUE * 10) + 1;
            v_dept_id := FLOOR(DBMS_RANDOM.VALUE * 5) + 1;
            v_order_no := ('PO-' || TO_CHAR(v_order_date, 'YYYYMMDD') || '-' || lpad((FLOOR(DBMS_RANDOM.VALUE * 9999) + 1), 4, '0'));
            ...
            VALUES (v_order_no, v_supplier_id, v_dept_id,
                ...
                v_order_date + (FLOOR(DBMS_RANDOM.VALUE * 10) + 3) * INTERVAL '1' DAY,
                'received')
            RETURNING id INTO v_order_id;
            ...
            v_product_id := FLOOR(DBMS_RANDOM.VALUE * v_product_count) + 1;
            v_qty := FLOOR(DBMS_RANDOM.VALUE * 50) + 10;
            ...
            -- 生成批号和入库
            v_batch_no := ('BT-' || TO_CHAR(v_order_date, 'YYYYMMDD') || '-' || lpad((FLOOR(DBMS_RANDOM.VALUE * 999) + 1), 3, '0'));
            ...
            -- 生成入库单
            v_receipt_no := ('RC-' || TO_CHAR(v_order_date, 'YYYYMMDD') || '-' || lpad((FLOOR(DBMS_RANDOM.VALUE * 9999) + 1), 4, '0'));
