...
WITH return_detail AS (
    SELECT
        ...
        (SELECT s.name FROM suppliers s
         ...
         WHERE pb.product_id = p.id FETCH FIRST 1 ROWS ONLY
        ) AS supplier_name,
        sri.return_qty,
        sri.amount,
    ...
    WHERE sr.return_date >= CURRENT_DATE - INTERVAL '6' MONTH
)
SELECT
    category_name,
    return_type,
    ...
    LISTAGG(supplier_name, ', ') WITHIN GROUP (ORDER BY supplier_name) AS suppliers,
...
SELECT
    ...
    (CURRENT_DATE - cas.last_order_date_all) AS days_since_last_order,
    -- 流失判断
    CASE
        WHEN crs.return_count > 0 AND (CURRENT_DATE - cas.last_order_date_all) > 90 THEN '退货后流失'
        WHEN crs.return_count > 0 AND (CURRENT_DATE - cas.last_order_date_all) > 30 THEN '退货后活跃度下降'
        WHEN crs.return_count = 0 AND (CURRENT_DATE - cas.last_order_date_all) > 90 THEN '无退货但流失'
...
SELECT
    ...
    AVG(CASE WHEN sr.approved_at IS NOT NULL
        THEN EXTRACT(EPOCH FROM (sr.approved_at - sr.created_at)) / 3600 END) AS avg_approval_hours,
    AVG(CASE WHEN sr.status = 'refunded'
        THEN EXTRACT(EPOCH FROM (sr.updated_at - sr.approved_at)) / 3600 END) AS avg_refund_processing_hours,
    AVG(CASE WHEN sr.status = 'refunded'
        THEN EXTRACT(EPOCH FROM (sr.updated_at - sr.created_at)) / 3600 END) AS avg_total_turnaround_hours,
...
WITH supplier_return_stats AS (
    ...
    WHERE pr.return_date >= CURRENT_DATE - INTERVAL '12' MONTH
...
supplier_inspection_stats AS (
    ...
    WHERE ir.inspection_date >= CURRENT_DATE - INTERVAL '12' MONTH
    GROUP BY pb.supplier_id
)
SELECT
...
SELECT
    ...
    ROUND(SUM(dr.total_loss_amount) * 100.0 / NULLIF(SUM(SUM(dr.total_loss_amount)) OVER (PARTITION BY TO_CHAR(dr.report_date, 'YYYY-MM')), 0), 2) AS loss_share_in_month,
    SUM(SUM(dr.total_loss_amount)) OVER (PARTITION BY dr.report_type ORDER BY TO_CHAR(dr.report_date, 'YYYY-MM')
        ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) AS cumulative_loss_by_type,
    LAG(SUM(dr.total_loss_amount)) OVER (PARTITION BY dr.report_type, w.name ORDER BY TO_CHAR(dr.report_date, 'YYYY-MM')) AS prev_month_loss,
    ROUND((SUM(dr.total_loss_amount) - LAG(SUM(dr.total_loss_amount)) OVER (PARTITION BY dr.report_type, w.name ORDER BY TO_CHAR(dr.report_date, 'YYYY-MM')))
        / NULLIF(LAG(SUM(dr.total_loss_amount)) OVER (PARTITION BY dr.report_type, w.name ORDER BY TO_CHAR(dr.report_date, 'YYYY-MM')), 0) * 100, 2) AS mom_change_pct,
    CASE
        WHEN SUM(dr.total_loss_amount) > LAG(SUM(dr.total_loss_amount)) OVER (PARTITION BY dr.report_type, w.name ORDER BY TO_CHAR(dr.report_date, 'YYYY-MM')) * 1.5
        THEN '异常增长'
        ELSE '正常'
    END AS alert_flag
...
GROUP BY TO_CHAR(dr.report_date, 'YYYY-MM'), dr.report_type, w.name
...
WITH inventory_movement AS (
    ...
    WHERE it.created_at >= CURRENT_DATE - INTERVAL '6' MONTH
    GROUP BY TO_CHAR(it.created_at, 'YYYY-MM'), w.name, it.transaction_type
)
SELECT
    month,
    store_name,
