...
SELECT
    'SH-DERIVED-' || TO_CHAR(so.id),
    so.id,
    so.warehouse_id,
    '顺丰速运',
    'TN-DERIVED-' || TO_CHAR(so.id),
    'express',
    ...
    so.salesperson_id,
    so.order_date,
    CASE WHEN so.status = 'delivered' THEN so.order_date + 2 ELSE NULL END,
    so.order_date + 3,
    CASE WHEN so.status = 'delivered' THEN so.order_date + 2 ELSE NULL END,
    '上海市松江区物流园A区',
    c.address,
...
SELECT
    'TAX-DERIVED-' || TO_CHAR(po.id),
    '044002' || TO_CHAR(po.id),
    'vat_special',
    'input',
    'supplier',
    po.supplier_id,
