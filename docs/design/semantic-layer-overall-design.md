...
为避免把总体方向误读成已经完整实现的 BI/Agent 平台，本设计按三层描述能力：

| 层级 | 本设计中的含义 | 默认边界 |
| --- | --- | --- |
| `v1` | 第一版应落地的核心能力 | semantic evidence graph、semantic catalog、lexicon/embedding search、question plan、SQL draft validation outline。 |
| `Phase 2+` | v1 稳定后可扩展的工程能力 | 更完整的 Review workflow、指标版本管理、离线评测集、用户反馈调权、catalog 增量构建。 |
| `Future` | 方向性示例或长期能力 | 跨系统 fuzzy match、方言 SQL 自动改写、SQL 执行频率统计、成本估计、复杂安全审计、自动执行 SQL。 |

除非明确标注为 `ACCEPTED` 或 `v1`，文档中的复杂指标、SQL draft、跨系统关联和方言提示都只表示 semantic candidate 或 future example，不代表当前 relation-detector 已经输出这些正式事实，也不代表语义层可以自动执行这些 SQL。
...
- LLM 不能生成正式物理 relationship、正式 physical lineage 或 `ACCEPTED` metric。
- LLM 可以提出无 evidence join 的 `SUGGESTED` candidate，但不能写入正式 catalog join path。
...
SQL Validator 可以复用 relation-detector 的 parser 能力，把 SQL draft 重新解析成事件、relationship 和 lineage 检查点。但 relation-detector parser 不是完整数据库优化器；v1 只把它作为 evidence/catalog guard，不把它包装成完整 SQL 安全分析器。
...
- `SUGGESTED`：LLM、comment、SQL usage 或用户问题提出的候选，尚不能作为正式业务口径。
- `EVIDENCE_SUPPORTED`：已有 relation-detector evidence 支持，但还没有人工接受。
- `ACCEPTED`：人工或治理流程确认后的正式语义对象，可作为默认回答口径。
- `REJECTED`：明确不采用。
...
```json
{
  ...
  "scanVersion": "relation-detector-result-v1",
...
SQL draft 不直接执行。必须经过 SQL Validator，并保存 validation result。v1 输出仍应称为 draft 或 candidate SQL；是否执行由外部受控系统或人工流程决定。
