...
术语定义统一维护在 [Semantic Layer 术语表](glossary.md)。本文首次出现的能力分层、审核状态、事实层和问答术语均以该术语表为准。外部参考材料与路线对比见 [参考亿问改进分析](yiyiwen-reference-improvement.md)。
...
| 标签 | 本设计中的含义 | 默认边界 |
| --- | --- | --- |
| [`Phase 1 Scope`](glossary.md#phase-1-scope) | 第一版应落地的核心能力 | semantic evidence graph、semantic catalog、lexicon/embedding search、question plan、SQL draft validation outline。 |
| [`Phase 2+`](glossary.md#phase-2) | Phase 1 Scope 稳定后可扩展的工程能力 | 更完整的 Review workflow、指标版本管理、离线评测集、用户反馈调权、catalog 增量构建。 |
| [`Future Capability`](glossary.md#future-capability) | 未来版本才考虑的能力 | 跨系统 fuzzy match、方言 SQL 自动改写、SQL 执行频率统计、成本估计、复杂安全审计、自动执行 SQL。 |
| [`Example`](glossary.md#example) | 解释设计用的示例 | 不进入 schema，不作为验收项，不代表系统已实现。 |

未标为 `Phase 1 Scope` 的能力不属于第一版实现目标；未标为 [`BUSINESS_APPROVED`](glossary.md#business_approved) 的指标或业务口径不能作为正式回答依据。文档中的复杂 SQL、复杂指标、跨系统关联和方言提示如果标为 `Example`，只用于解释设计；如果标为 `Future Capability`，表示未来版本才考虑。它们不代表 relation-detector 已输出业务事实，也不代表语义层会自动执行 SQL。
...
离线 `SemanticKgStore/SemanticReferenceClosureStore` 也要求非 diagnostic fact/event、endpoint node 与 edge 的 evidence 非空可解析，
并拒绝冲突 node/edge ID。任一失败都原子拒绝正式结果。后续章节描述的是目标架构和后续阶段设计；
如果某节没有明确说明“当前代码已实现”，默认按设计目标或后续能力理解。

### 1.2 事件的意义与边界

[`Semantic Event`](glossary.md#semantic-event) 用来表达业务世界里“发生了什么”：谁，在什么时间，对什么对象，发生了什么行为，造成了什么结果。它把物理表字段进一步组织成业务行为语义，例如支付、退款、下单、登录、补货或库存变动。
...
在本设计中，`QuestionPlan / AnswerPlan` 是 [LogicForm-like intermediate representation](glossary.md#logicform-like-intermediate-representation)。它们承担自然语言与 SQL draft 之间的中间语义层职责，但必须比亿问材料中的产品描述更克制：
...
- metadata inventory的全部已采集记录及其scope、counts、status。
...
这里的`COMPLETE`仅证明上游collector完成配置scope；semantic consumer还会以共享typed closure rules
验证constraint/index成员、FK引用端、cardinality/ordinal与重复identity。scope未包含被引用对象时，
即使上游状态为`COMPLETE`，正式semantic处理仍会因引用不闭合而拒绝。
...
Semantic Catalog、lexicon、embedding、search、question understanding、planner、SQL draft/validator、
answer composer 与 review queue 尚未进入当前生产代码。它们的能力目标、依赖顺序、typed 输入输出、
安全边界和实施验收条件统一由
[Future Capabilities Roadmap](future-capabilities-roadmap.md) 维护。

整体设计只保留以下跨能力约束：

- 后置模块不能重建或改写 relation-detector 的物理事实。
- LLM 只能生成 `SYSTEM_PROPOSED` 候选，不能写 `BUSINESS_APPROVED`。
- join、metric、SQL draft 和 answer 必须保留可解析 evidence refs。
- SQL 结构判断必须来自 typed parser/draft elements，不能使用 regex 或名称猜测。
- 任一正式 artifact 的 evidence/identity 闭包失败都必须原子拒绝。
...
fact/evidence/candidate 使用内容稳定 ID。semantic extraction先在完整磁盘store上建立全局item、
dependency、evidence与table-touch索引，按字典序最小物理table（无table时为`global`）指定唯一owner；
超预算component按table owner和稳定root拆分，root的传递dependency/evidence closure不可再切分。
每个model-authored输出在片内backfill前必须直接引用当前shard owned fact/candidate；独立
normalization也要求bundle携带同一owner context。最终全局normalization重新验证合并后的引用与
物理/evidence闭包。物理实体和有owned grounding的纯业务实体已有canonical merge；同名但grounding
不同的对象保留并进入review。不同外排window大小必须产生相同event、owner manifest、shard和KG。
尚无
...
| 模块 | 输入 | 输出 | 在问答示例中的作用 |
| --- | --- | --- | --- |
| relation-detector事实层（当前） | metadata、DDL、SQL log、procedure、trigger、object SQL | 带采集状态的metadata inventory、relationship、Data Lineage、naming、diagnostics及evidence payload | 提供scope内物理catalog、join/lineage evidence和source location；COMPLETE不替代下游成员引用校验。 |
...
逐模块中间结果明细见 [示例附录 13.1](examples.md#131-可直接回答客户最近-30-天支付金额)。
...
逐模块中间结果明细见 [示例附录 13.2](examples.md#132-需要反问活跃客户)。
...
逐模块中间结果明细见 [示例附录 13.3](examples.md#133-只能回答表字段计划库存风险)。
...
逐模块中间结果明细见 [示例附录 13.4](examples.md#134-需要反问退款率口径冲突)。
...
逐模块中间结果明细见 [示例附录 13.5](examples.md#135-可直接回答但带口径校验财年华东女装销售情况)。
...
复杂多表关联、自关联递归、多跳 join path、时间窗口指标、HAVING/[RFM](glossary.md#rfm)、存储过程指标、SQL 日志指标和跨系统关联等长例子已移到 [Semantic Layer 示例附录](examples.md)。主文档只保留说明架构边界所需的代表性例子。
