...
ScanBundle 对应 Semantica 架构中 Raw Documents / Records 的角色，但它的内容是数据库事实记录而不是任意文档。目标上它可以承载 normalized relationships、data lineages、naming evidence、derived facts、metadata、diagnostics、source hashes 和 raw evidence references。当前代码中的 `ScanBundle` 是轻量实现：保留 relation-detector JSON fact arrays、summary、sources、inputFiles 和 diagnostics，不构建 metadata/source-hash/index，也不负责业务语义确认。
