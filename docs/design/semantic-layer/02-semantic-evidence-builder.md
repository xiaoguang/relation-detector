...
当前 `SemanticFactIds` 为 relationship、lineage、derived fact 和 diagnostic id 追加输入数组 index，`EvidenceReference.id` 也使用 owner id + evidence index。因此这些 id 对“同一有序输入”是确定性的，但事实顺序变化会改变 id；它们还不是内容寻址、跨扫描稳定的 provenance id。设计目标仍是由 canonical fact fingerprint、scan/source identity 和 observation identity 生成稳定 id。
