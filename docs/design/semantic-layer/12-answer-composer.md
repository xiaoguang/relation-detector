...
**职责：** 将 `AnswerPlan`、`SqlDraft` 和 `ValidationResult` 组装为最终响应。v1 使用模板输出，所有事实来自 catalog、plan 和 validator，不在线调用 LLM 改写事实。
...
v1 不在线调用 LLM。Phase 2+ 可以增加 LLM 文案润色，但必须满足：
