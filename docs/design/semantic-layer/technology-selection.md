...
| 分阶段落地 | Prototype 可以用 JSON 文件；production-ready v1 推荐 PostgreSQL + JSONB + pgvector。 |
...
| PostgreSQL + JSONB | 事务、并发、JSON 查询、成熟生态 | 需要数据库运维 | production-ready v1 catalog |
| PostgreSQL + JSONB + pgvector | 支持 catalog + vector search 一体化 | 需要 pgvector 扩展 | production-ready v1 search |
| SQLite | 零运维 | 向量检索和并发写有限 | 单机替代方案 |
| Elasticsearch | 搜索能力强 | 运维重、对 v1 过度设计 | 暂不采用 |

推荐路径：

```text
Prototype: JSON files
Production-ready v1: PostgreSQL + JSONB + pgvector
...
v1 采用同进程 Java interface + 构造函数注入：
...
v1 不使用 regex 提取表、字段、JOIN 条件，也不使用 keyword blacklist 判断危险操作。
...
| 维度 | Prototype | Production-ready v1 | Phase 2+ / Future |
| --- | --- | --- | --- |
