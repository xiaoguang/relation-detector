...
- `full-grammer`：版本化完整 grammar 链路。MySQL 8.0 与 PostgreSQL 16 的 full-grammer module 由 adaptor 提供，解析树 visitor 直接生成同一套结构事件。
...
```text
relation-core/src/main/java/com/relationdetector/core/parser
  SqlRelationParserRunner
  DdlRelationParserRunner

relation-core/src/main/java/com/relationdetector/core/tokenevent
...
relation-core/src/main/java/com/relationdetector/core/fullgrammer
...
relation-core/src/main/java/com/relationdetector/core/relation
  TokenEventSqlRelationParser
  TokenEventRelationExtractor
  DdlRelationExtractionVisitor
  RelationshipMerger

relation-core/src/main/java/com/relationdetector/core/lineage
  TokenEventDataLineageExtractor
  DataLineageMerger
  SqlLineageResolver

relation-core/src/main/java/com/relationdetector/core/ddl
...
- PostgreSQL `16.5` 使用 `postgresql-16`。
- MySQL `8.0.x` 使用 `mysql-8.0`。
- 如果只有 `postgresql-16`，请求 `17.x` 可临时降级到 `postgresql-16` 并记录 diagnostic；`18.x` 不自动跨两级。
...
postgresql-16
  -> adaptor-postgres/com.relationdetector.postgres.fullgrammer.v16
  -> PostgresFullGrammerStructuredSqlParser
  -> PostgresTokenEventParseTreeVisitor
  -> PostgresExpressionAnalyzer
```
...
`FullGrammerCorrectnessShadowTest` 扫描全部 SQL correctness fixture，比较 full-grammer 与 token-event 正式输出，要求：
...
`FullGrammerDdlCorrectnessShadowTest` 扫描全部 DDL correctness fixture，要求：
