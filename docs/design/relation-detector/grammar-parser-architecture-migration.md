...
发布验收由 `relation-detector/scripts/verify-release.sh` 编排：先运行 no-cache acceptance，再运行
`verify-all.sh` 生成 1198 fixtures、19 parser categories、38 JSON、parity、diagnostic、integrity
和 artifact hash 的统一 manifest。任何缺项或 `REVIEW_NEEDED` 都不能标记迁移完成。
