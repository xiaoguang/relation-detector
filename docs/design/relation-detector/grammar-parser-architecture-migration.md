...
本文同时记录目标状态和实施状态。grammar 所有权、Script Framer、当时的 SPI v4（现已由 SPI v5 取代）、正确 Grammar
...
| Script Framer 与当时的 SPI v4 | 已实施；当前公开契约为 SPI v5 | framing/SPI architecture tests |
...
迁移的历史语义基线已按 `a586fe9d -> f729e294 -> 15482ab8` 三个检查点回溯，详细证据见
`docs/parser-audit/grammar-migration-release-verification.md`。回溯结论区分三件事：

- grammar/script 迁移提交同时包含 PostgreSQL parser/golden 修正，因此 A→B 不是严格的事实零变化。
- 后续 B→C 的大幅 derived 收缩主要来自组合唯一性和 SQL Server referenced-key evidence 修正。
- 历史逐阶段执行过程无法由最终 Git 状态证明，只能声明当前状态已独立重验。

发布验收由 `relation-detector/scripts/verify-release.sh` 编排：先运行 no-cache reactor 与 smoke
correctness，再运行 `verify-all.sh`。full correctness 由 `run-correctness-isolated.sh` 按数据库家族
顺序隔离；sample-data CLI 由 `run-sample-data-isolated.sh` 使用相同九组边界顺序隔离。两条链路
都把 Oracle root 和各 versioned parser 放入独立 JVM，且同一时间只运行一个组；聚合后生成
全部 discovered fixtures、parser categories、direct/derived JSON、parity、diagnostic、integrity 和 artifact hash 的
统一 manifest。任何缺项或
`REVIEW_NEEDED` 都不能标记迁移完成。
...
40. **本地 Maven 配置不入库（后续决策覆盖原迁移提案）**：根 `.mvn/` 是 workstation-local，
...
7. 架构门禁、full correctness、全部 discovered parser matrix 和 sample-data CLI 验收。
