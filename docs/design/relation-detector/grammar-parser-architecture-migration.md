...
本文描述的是目标状态，不表示当前代码已经完成迁移。实施时必须先建立架构测试，再按本文
拆分任务；不得通过兼容桥长期保留旧结构。
...
9. **承认 PostgreSQL 的嵌套语言边界**：outer PostgreSQL token-event/full grammar 只负责
   函数声明和 body literal；PL/pgSQL body 必须交给与当前 parser mode 对应的第二阶段
   typed parser。
...
18. **记录可复现来源**：每个 PL/pgSQL module README 必须记录官方 tag、commit、源文件、
    派生规则、已支持 statement family、版本差异和已知 gap。
...
24. **统一 PostgreSQL 类名**：在 `...v16` 等 package 内使用
    ...
    `PlPgSqlContextAdapter` 和 `FullGrammarBinding`。
...
29. **重命名关系 extractor**：将同时消费 token-event/full grammar structured event 的
    `StructuredRelationshipExtractor` 改为 `StructuredRelationshipExtractor`。

30. **重命名 relationship parser wrapper**：将可包装任意 `StructuredSqlParser` 的
    `StructuredSqlRelationshipParser` 改为 `StructuredSqlRelationshipParser`。
...
- 每个 grammar module 的来源 README 与真实 `.g4` 同目录。
...
- v16/v17/v18 的 `DECLARE`、IF、LOOP、EXCEPTION、assignment 和 RAISE。
