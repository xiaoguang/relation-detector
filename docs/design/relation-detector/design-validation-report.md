...
- relationship 已将完整、顺序无关的 guard 数组纳入 candidate/observation/fingerprint identity，
  并按全部 structural observations 计算 conditional 与 polymorphic summary。剩余 evidence 收口是
  grouped relationship evidence 使用 consensus attributes，以及 relationship/derived observation summary
  累加 `occurrenceCount`。
