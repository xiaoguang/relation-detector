...
| --- | --- | --- | --- | --- | --- |
| CLI config / argument parsing | `cli.Main.MainCommand`, `SimpleYamlConfigLoader`, `SingleScanRunner`, `BatchCommand` | `phase-06 Parser mode 和 profile 选择`, `phase-08 输出与配置` | `ParserConfigRemovalTest`, `CliErrorCodeContractTest`, `BatchSchedulerTest` | FOCUSED_GREEN | argument parsing、config file read、YAML format、adaptor/input/connection/runtime 和 output write failure 均映射到固定 `ErrorCode` 与脱敏 stderr。`ConfigFormatException` 只区分可读取但无法解析的配置；普通 `IOException` 保持 `CONFIG_FILE_ERROR`。batch partial failure 保持 exit 13，并输出 typed、脱敏的 per-case error code。 |
...
| CLI error-code contract | FOCUSED_GREEN | `CliErrorCodeContractTest` 穷举 single-scan 的 `OK`、argument、config file、config format、adaptor、input、connection、runtime 和 output write 映射；真实临时文件验证 unreadable 与 malformed YAML 的边界。`BatchSchedulerTest` 验证 partial failure 的 exit 13、typed code 与脱敏报告。 |
