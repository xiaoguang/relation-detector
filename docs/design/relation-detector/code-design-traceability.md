...
| runtime config validation | `ScanConfigurationValidator`, `ScanConfig.resolve`, `ResolvedScanConfig`, `ScanEngine.scan`, `ScanCapabilityValidator` | `phase-08 配置校验` | `ScanConfigurationValidatorTest`, `RuntimeConfigurationCliTest`, `BatchCommandIntegrationTest`, capability tests | MATCHED | YAML/CLI override、batch 和 direct API 共用 core validator；全 source 关闭、live source 缺 JDBC、非法 parser mode、derived limit、execution/profile limit 或 confidence 在 capability 检查和 JDBC 前失败。关闭 source 的残留路径不解析也不计为可执行 source；path error 与 config error 保持不同 CLI code。 |
...
| 代码结构注释 | `**/package-info.java`, production class/method Javadocs | `phase-06 代码结构注释索引`, `code-implementation-guide` | `DialectGrammarArchitectureTest`, compile + semantic tests | MATCHED | 泛化模板和结构门禁已通过；core profile、MySQL/PostgreSQL root package 与 SQL Server DDL package 已按当前 live-only、versioned parser 和 structural-skeleton 职责校准。门禁仍只能证明形式，不能替代后续调用链评审。 |
...
| production Javadoc / package documentation | PARTIAL | compiler/doc-tree 门禁验证双语 package 契约、具体边界说明和禁用模板；本轮语义复核仍发现 profile、version package 与 SQL Server DDL contract 的 stale 描述，需要生产源码注释修正。 |
...
| runtime 配置校验 | MATCHED | `ScanConfigurationValidator` 覆盖 YAML/CLI override、batch、`ScanConfig`、手工 `ResolvedScanConfig` 和 `ScanEngine`；live/JDBC、source、parser、derived、execution/profile limit 及 confidence 边界在 scan 前统一验证。 |
