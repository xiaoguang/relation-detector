...
| relationship extraction / naming evidence | `core.relation.StructuredRelationshipExtractor`, `RelationshipAliasSupport`, `RelationshipMerger`; `core.naming.NamingEvidenceExtractor`, `NamingRuleEngine`, `NamingRuleSet`, `NamingEvidenceMerger`, `NamingMatchEvidenceEnhancer` | `phase-02 SQL 谓词关系守卫`, `phase-06 Relationship 抽取` | `CorrectnessFixtureRunnerTest`, `CorrectnessNamingEvidenceGoldenTest`, `NamingEvidenceExtractorTest`, `NamingRuleEngineCatalogIdentityTest`, `RelationshipMergerEvidenceAggregationTest` | PARTIAL | `NamingRuleEngine` 是唯一命名启发式边界，有向分组与多位置 observation 已实现。剩余缺口：SQL fact key 仍依赖 endpoint 保存的可读 `normalizedName`；MySQL 两段 `database.table` 仍可能按通用 `schema.table` 解释；self-join 判定仍有直接 `TableId.equals()` 路径；conditional summary 只提升第一个 guard；relationship grouped evidence 仍复制首条 observation attributes 而非 consensus attributes。 |
| endpoint / metadata identity | `TableId`, `Endpoint`, `CanonicalEndpointKey`, `AliasSymbolTable`, `StatementParsePipeline`, metadata fact records | `phase-02`, `phase-06 Hard Boundaries` | catalog identity、naming、projection、metadata enhancement tests | PARTIAL | live metadata 主要链路已保留 catalog/schema/table identity，PostgreSQL/SQL Server collector resolver 也能拒绝不一致 catalog。但 `TableId.sameIdentity()` 仅比较 catalog 与调用方提供的 `normalizedName`，没有独立校验 schema/table 字段；MySQL qualified SQL 和 Oracle catalog 输入也尚未闭环，不能宣称模型与四方言身份完全匹配。 |
| Data Lineage extraction | `core.lineage.StructuredDataLineageExtractor`, `ProjectionTraceResolver`, `core.lineage.model.*`, `DataLineageMerger` | `phase-06 Data Lineage v1`, `sql-lineage-resolver.md` | `ProjectionTraceResolverTest`, `DataLineageMergerTest`, lineage golden | PARTIAL | 正式链路只消费 structured events，`DataLineageMerger` 也会先对 source set 去重/排序，已消除单纯发射顺序造成的重复。但 fact key 仍使用 endpoint 保存的 `normalizedKey()`，它不是按 adaptor identifier rules 重建的 canonical key；MySQL 两段名轴映射错误也会传入 known-physical 过滤和 lineage fact identity。 |
| derived path inference | `core.derived.DerivedPathInferenceService`, `contracts.model.DerivedPathCandidate`, `JsonResultWriter` | `phase-06 Derived Path Evidence`, `phase-08 output` | `DerivedPathInferenceServiceTest`, `JsonResultWriterEvidenceOutputTest`, `ParserConfigRemovalTest` | PARTIAL | `derivedPaths.enabled=false` 默认关闭；路径方向、canonical path fact identity、edge variant observation 合并和 non-adjacent re-entry 防护已实现。但 table identity bridge 的 key 只包含 `schema.table`，遗漏 catalog，跨 catalog 同名表可能被桥接。 |
...
| adaptor live capabilities | `DatabaseAdaptor.capabilities`, grouped `collectors()/parsers()/profiling()`, `ScanCapabilityValidator` | `phase-03 Adaptor API 和 SPI`, adaptor phase docs | capability and live collector contract tests | PARTIAL | SPI v5 preflight 已验证 capability 与 collector。剩余缺口：live object/database-DDL 请求未同时验证对应 structured parser；profile-only 请求不会触发 PostgreSQL/SQL Server catalog resolver。内置 adaptor 当前恰好提供 parser，但 public SPI/custom adaptor 契约仍不完整。 |
...
| profiler metrics | `JdbcDataProfilerTemplate`, `DataProfileMetrics`, `DataProfileEvidenceBuilder` | `phase-07 指标` | evidence-builder tests, dialect fake-JDBC tests | PARTIAL | live query 已独立测量 source non-null rows、source distinct、matched distinct 和 target distinct。剩余缺口：PostgreSQL renderer 会把 metadata catalog 渲染成不合法的三段表名；profile-only PostgreSQL/SQL Server scan 可绕过 catalog 校验；negative evidence 还没有 tenant/soft-delete/time-window 等过滤上下文 guard。 |
| confidence / merger / output | `RelationshipMerger`, `DataLineageMerger`, `NamingEvidenceMerger`, `ConfidenceCalculator`, `JsonResultWriter` | `phase-02`, `phase-08` | `ConfidenceScoringExamplesTest`, `RelationshipMergerEvidenceAggregationTest`, `JsonResultWriterEvidenceOutputTest`, `CliEndToEndGoldenTest` | PARTIAL | raw/grouped 模型与 catalog 输出已落地。剩余缺口：relationship grouped evidence 使用首条 attributes 而非共识属性；relationship 与 derived-path observation summary 没有累加折叠 evidence 的 `occurrenceCount`，与 lineage/naming 计数语义不一致。 |
...
| table identity 与 fact canonicalization | REVIEW_NEEDED | 修复 `TableId.sameIdentity()` 的独立字段校验、MySQL `database.table` 轴映射、dialect identifier normalization、Oracle catalog 输入、profile-only catalog validation，以及 derived bridge 的 catalog key；补跨 catalog/schema 与大小写负向测试。 |
| 直接 API 文件路径 | REVIEW_NEEDED | `ScanConfig.*Paths + *Includes` 只有 CLI loader 会展开；core 直接调用必须统一展开或 fail-fast，不能成功返回空解析结果。 |
| relationship 条件与 provenance merge | REVIEW_NEEDED | conditional summary 必须保留全部 guard；grouped evidence 顶层只保留所有 observation 一致的 consensus attributes。 |
...
| live profiler 指标完整性 | PARTIAL | 四项计数已独立返回；PostgreSQL catalog 渲染、profile-only catalog 校验和 negative filter-context guard 尚未闭环。 |
...
| observation summary count | REVIEW_NEEDED | lineage/naming 会累加 `occurrenceCount`，relationship/derived path 当前只统计 evidence list entry；四类 summary 应统一。 |
