...
| --- | --- | --- | --- | --- | --- |
| CLI config / argument parsing | `cli.Main.MainCommand`, `SimpleYamlConfigLoader`, `SingleScanRunner`, `BatchCommand` | `phase-06 Parser mode 和 profile 选择`, `phase-08 输出与配置` | `ParserConfigRemovalTest`, `CliErrorCodeContractTest`, `BatchCommandIntegrationTest`, `BatchSchedulerTest` | FOCUSED_GREEN | argument parsing、config file read、YAML syntax/shape、adaptor/input/connection/runtime 和 output write failure 均映射到固定 `ErrorCode` 与脱敏 stderr。single-scan 与 batch preflight 都区分不可读文件和格式错误；batch execution partial failure 保持 exit 13，并输出 typed、脱敏的 per-case error code。 |
| adaptor discovery | `cli.AdaptorRegistry`, `core.scan.AdaptorContractValidator` | `phase-03 Adaptor SPI`, `phase-06 full-grammar module 注入链` | `AdaptorApiVersionTest`, `CommonAdaptorRegistryTest`, `CliEndToEndGoldenTest` | FOCUSED_GREEN | 通过 Java SPI 发现 `DatabaseAdaptor`；registry 与 direct `ScanEngine` 复用同一完整 validator 检查 SPI v5、database type 和显式 adaptor id，CLI 边界将 validator failure 包装为既有 `AdaptorException`。SPI v5 的 `AdaptorParsers.scriptFramer()` 提供必需的 `DialectScriptFramer`，full-grammar module 由独立 `ServiceLoader<FullGrammarDialectModule>` 注入。 |
...
| adaptor live capabilities | `AdaptorContractValidator`, `DatabaseAdaptor.capabilities`, grouped `collectors()/parsers()/profiling()`, `ScanCapabilityValidator`, `DatabaseAdaptor.resolveLiveScope` | `phase-03 Adaptor API 和 SPI`, adaptor phase docs | `ScanCapabilityValidatorTest`, capability、live collector 与 profile-only catalog tests | PARTIAL | CT-02/CT-03 已达 `FOCUSED_GREEN`：SPI/type/id、live capability、collector 及对应 consumer 均在 JDBC 前验证；live DDL 同时要求 structured DDL parser，live objects 同时要求 structured SQL parser，profile 同时要求 profiler。此行仍为 `PARTIAL` 仅因四数据库真实权限/版本 runtime smoke 是环境性边界，不代表 structured parser preflight 仍有缺口。 |
...
| CLI error-code contract | FOCUSED_GREEN | `CliErrorCodeContractTest` 穷举 single-scan 映射，并用真实临时文件验证 unreadable、malformed YAML 与 explicit-null shape。`BatchCommandIntegrationTest` 验证 manifest/case config 的 file/format preflight；`BatchSchedulerTest` 验证 execution partial failure 的 exit 13、typed code 与脱敏报告。 |
