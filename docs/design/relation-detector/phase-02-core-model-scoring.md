...
- `derivedRelationships` / `derivedDataLineages`：开启 `derivedPaths` 后的推导视图，必须保存完整 path、`TRANSITIVE_PATH` grouped evidence 和 typed `evidenceSets`；它们不是直接物理事实。每个 set 保存有序 hop、每跳排序去重的直接 evidence refs、`BigInteger combinationCount` 和路径 confidence，不构造证据笛卡尔积 `rawEvidence`。derived relationship 内部按 referenced-by 反向遍历，但输出 source/target 保持 FK-like 正向，并用 `traversalPath` 保留内部遍历顺序。
