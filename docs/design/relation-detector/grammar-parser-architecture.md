# Grammar、Script Framing 与 Routine Parser 架构

## 1. 文档目的

本文定义 relation-detector 当前的 grammar 所有权、client script framing、PostgreSQL
PL/pgSQL 嵌套解析、版本包命名和构建文件治理规则。它描述当前运行时契约与禁止边界，
不记录历史迁移步骤；实施历史可从 Git 追溯。

当前 adaptor SPI 为 v6。后续变更仍必须先建立架构测试，不得通过兼容桥恢复旧结构，
也不得把 script framing、routine shell 或 generated parser 的职责重新混合。
...
- 版本由 Maven artifact、Java package、profile 和 policy 表达；同一版本 package 内的类名
  不重复携带版本号。
- 全仓只使用 `Grammar`、`grammar`、`full-grammar`。不保留旧 Java 类型、package、目录、
  YAML 值、CLI 值、profile id、fixture id、脚本参数或
  deprecated alias。
...
| 范围 | 状态 | 代码门禁 |
| --- | --- | --- |
| Grammar module 所有权与 README 同模块 | 生效 | `GrammarOwnershipArchitectureTest` |
| Script Framer 与 SPI v6 | 生效 | framing/SPI architecture tests |
| PostgreSQL PL/pgSQL 过程外壳与 current-mode SQL callback | 生效 | 四套 `Postgres*PlPgSqlParserTest` |
| sealed routine body、缺省 LANGUAGE、BEGIN ATOMIC | 生效 | `PostgresRoutineLanguageDispatcherTest`、outer routine tests |
| FOREACH、partial recovery、dynamic SQL diagnostic | 生效 | 四套 PL/pgSQL contract tests |
| Grammar terminology 和 version package 命名 | 生效 | terminology architecture tests |
| 空目录与确认死代码治理 | 生效 | `SourceTreeHygieneTest`、reachability audit script |

本表表示源码契约和自动化门禁存在；每次发布仍须通过 correctness、parser matrix、sample-data
CLI 和 release verification manifest，不能仅凭本表判断构建通过。
...
1. **统一 grammar 所有权**：versioned full grammar、compact token-event grammar、client
   script lexer grammar、compact PostgreSQL PL/pgSQL token-event grammar 和 versioned
   PostgreSQL PL/pgSQL grammar 全部位于 `relation-detector/grammar/`。

2. **限定 adaptor 职责**：adaptor 只保留 binding、typed context adapter、visitor、
   collector、profile module、version policy 和 Java framing 入口，不再拥有 grammar 源文件。

3. **README 与 grammar 同模块**：描述 full grammar 的 README 位于对应 grammar module；
   README 中的路径必须指向真实模块，不保留只含说明文件的旧 grammar 目录。
...
23. **类名不重复版本**：当版本已经出现在 Maven artifact 和 Java package 中，类名
...
29. **关系 extractor 按职责命名**：同时消费 token-event/full grammar structured event 的
    extractor 使用 `StructuredRelationshipExtractor`。

30. **relationship parser wrapper 按职责命名**：可包装任意 `StructuredSqlParser` 的
    wrapper 使用 `StructuredSqlRelationshipParser`。
...
32. **不保留无生产用途的 sink**：`FullGrammarEventSink` 不属于当前生产 API，不为旧调用保留空壳。

33. **typed sink 按职责命名**：承担多个 collector 编排的 typed facade 使用
...
### 4.7 Grammar Terminology

35. **内部符号使用正确拼写**：Java class/interface/record/enum、method/field、package、
    import、文件名、目录名、Maven artifact/module、ANTLR package、generated output path 和
    test name 全部使用 `Grammar` / `grammar`。

36. **外部契约使用正确拼写**：YAML、CLI 参数和值、environment/property 名、profile id、
    manifest、fixture id、warning/result name、JSON 中若存在的 parser 标识、脚本和文档全部改为
    `full-grammar` 或对应正确形式。不接受旧值，不提供 compatibility alias，不输出 deprecated warning；
    使用旧值必须作为未知配置直接失败。

37. **保持零遗留门禁**：仓库级架构测试扫描错误拼写的大小写与连接符变体，生产、测试、
    fixture、脚本和文档均必须为零。仓库不能保留旧类型 shim、旧 package forwarding
...
40. **本地 Maven 配置不入库**：根 `.mvn/` 是 workstation-local，
    必须保持 ignored/untracked。可复现的仓库构建行为只能放在 tracked POM 和 scripts；不得依赖
    开发机私有 extension/cache 配置。

41. **忽略 Maven 本地缓存**：使用 `**/.mvn/build-cache/` 排除生成缓存，仓库中不得跟踪
    cache JAR、buildinfo、临时索引或机器相关路径。

42. **隔离 IDE 私有配置**：团队未明确维护共享 VS Code 配置时，`.vscode/settings.json`
    保持 ignored/untracked；grammar/parser 变更不得夹带个人 IDE 设置。
...
- 不借 terminology 修改 relationship、lineage、naming evidence、confidence 或 JSON 事实语义。
...
6. 不存在误导职责的 TokenEvent wrapper、routine、sink 和 binding 名称。
7. 全仓只存在正确的 Grammar 拼写；旧 Java/YAML/CLI/profile/fixture 值直接不存在且无法使用。
8. 根 `.mvn/` 与个人 IDE 设置均不跟踪；仓库构建契约只存在于 tracked POM 和 scripts。
9. token-event/full grammar 的直接和 derived 事实集合无计划外变化，diagnostics 无新增静默失败。
10. 所有 discovered correctness fixtures、parser categories、全部 sample-data CLI 和 Maven reactor
    测试通过。
...
任何执行者不得把“编译通过”视为完成。发布结论必须同时满足本节门禁和最新 verification
manifest 的 correctness、parser matrix、sample-data CLI 与 artifact 完整性检查。
