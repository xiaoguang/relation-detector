...
- core 提供统一 runner、关系归并和评分；SQL/DDL 文本解析由对应数据库 adaptor 暴露的 token-event parser 或版本化 full-grammar module 承担，ANTLR 只作为底层 lexer/parser/token 支撑。明显属于某个数据库方言的 DDL、日志、对象定义差异应进入对应 adaptor。这样 MySQL、PostgreSQL、Oracle 和后续 SQL Server 可以独立演进，而不把所有语法分支堆进 core。

## DatabaseAdaptor 接口

当前公开契约是 **adaptor SPI v4**。`DatabaseAdaptor` 只暴露 grouped
...
这是 adaptor SPI v4 的必需能力，不能再用 core 的通用文本 splitter、对象文件 extractor
