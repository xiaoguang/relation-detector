...
当前实现边界：这些查询返回的是 `information_schema` 中的 body/query fragment，不是完整
`CREATE PROCEDURE/FUNCTION/TRIGGER/EVENT/VIEW` declaration。fragment 可以恢复其中独立可解析的
静态 SQL，但 routine 参数声明、trigger timing/target table 等 typed scope 信息可能缺失。因此
“已采集对象记录”不能直接写成“完整对象 DDL 已进入 parser”；后者仍需 `SHOW CREATE ...` 或等价
parser-grade declaration collector。
...
当前 collector 的查询只取 `TRIGGER_SCHEMA/TRIGGER_NAME/ACTION_STATEMENT`，尚未把
`EVENT_OBJECT_TABLE`、timing/event 和完整 `CREATE TRIGGER` header组装进
`DatabaseObjectDefinition.sql`。因此 live trigger body中的 `NEW/OLD` 不能保证像文件输入的完整
trigger declaration一样解析到目标表。
...
- 计算目标列匹配数量。
- target unique/index gate来自metadata candidate selection，不由profile SQL重新证明。
- 采样 SQL 必须加 limit 或使用受控策略。
- 必须设置 statement timeout 或通过 JDBC query timeout 控制。

当前 JDBC query只独立返回 `source_distinct` 和 `matched_distinct`。`targetDistinctValuesSampled`
与 `sourceNonNullRowsSampled` 暂由source distinct代填，所以 containment ratio可直接解释；对称
overlap denominator和negative row-count gate尚不是独立数据库测量。
...
- 可读取 procedure/function/view/trigger/event catalog记录和可用body fragment；完整 parser-grade
  declaration仍是待收口项。
