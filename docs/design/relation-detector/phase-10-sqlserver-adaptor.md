...
SQL Server 当前已经完成 **ERP sample-data correctness 全量接入**：有独立 Maven 模块、DatabaseAdaptor、root token-event grammar、五个 versioned full-grammar profile、`sample-data/sqlserver/2016|2017|2019|2022|2025` 资产，以及 root token-event + 五个 versioned full-grammar correctness golden。当前文件和 parser case 数量由 [Parser 能力与统计摘要](../../parser-audit/parser-comparison-summary.md)及 verification manifest 维护。首批 Microsoft 文档可确认的 version-only 语法已经落到 `.g4` 差异和 correctness / architecture 测试中：2017 `STRING_AGG`、2022 `DATETRUNC` / `GENERATE_SERIES`、2025 `VECTOR(...)`。
...
- sample-data：`sample-data/sqlserver/2016|2017|2019|2022|2025`，版本目录保持业务布局一致；动态数量不在本文复制。
- correctness golden：root token-event 覆盖 `sample-data/sqlserver/2025`；五个 versioned full-grammar profile 各覆盖对应版本目录，另有 version-only fixture 锁定 2017/2022/2025 的首批边界。
