...
SQL Server 的 SPI v3 `SqlServerScriptParser` 使用 generated script lexer 的 typed `GO` token：只有单独一行的 `GO` 结束 batch；procedure/function/trigger batch 不会被其中的 semicolon 拆开。batch framing 与 SQL grammar 分离，避免把 client command 当成 T-SQL 结构。
