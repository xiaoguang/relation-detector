...
SQL Server 的 SPI v5 `SqlServerScriptFramer` 使用 generated script lexer 的 typed `GO` token：只有单独一行的 `GO` 结束 batch；procedure/function/trigger batch 不会被其中的 semicolon 拆开。batch framing 与 SQL grammar 分离，避免把 client command 当成 T-SQL 结构。
...
- SQL Server live metadata 通过 `sys.tables/sys.schemas/sys.columns/sys.types`、key/FK constraints 和 `sys.indexes/sys.index_columns` 采集；object collector 通过 `sys.objects/sys.schemas/sys.sql_modules` 读取 procedure/function/view/trigger 定义。metadata组合约束/索引按 ordinal 保留，无权访问 definition 会产生 warning 而非空壳对象。`SqlServerDatabaseDdlCollector` 当前从`INFORMATION_SCHEMA`重建table DDL，但尚未在scope catalog为空时继承connection catalog，且其composite FK引用列没有使用`sys.foreign_key_columns.constraint_column_id`做ordinal-safe配对；不能把database-DDL fidelity与metadata collector等同。`SqlServerDataProfiler`执行bounded containment query。
...
4. 让 database-DDL collector复用connection catalog，并改用ordinal-safe FK catalog query，避免组合FK列笛卡尔配对。
