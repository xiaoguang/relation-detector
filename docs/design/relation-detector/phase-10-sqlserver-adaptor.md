...
SQL Server 当前已经完成 **ERP sample-data correctness 全量接入**：有独立 Maven 模块、DatabaseAdaptor、root token-event grammar、五个 versioned full-grammer profile、`sample-data/sqlserver/2016|2017|2019|2022|2025` 每版 38 个 SQL 文件，以及 root token-event + 五个 versioned full-grammer correctness golden。

需要分清两个边界：

- sample-data 业务语义覆盖已经从 smoke 扩展到与 MySQL 8.0 sample-data 文件布局对齐。
- Microsoft 官方 T-SQL reference 的逐版本严格裁剪仍在 backlog；当前 sample-data 使用 SQL Server 2016-compatible 保守 T-SQL 子集，因此五个版本输出一致。
...
- correctness golden：root token-event 覆盖 `sample-data/sqlserver/2025` 的 38 个文件；五个 versioned full-grammer profile 各覆盖对应版本目录的 38 个文件。
...
| SQL Server full-grammer v2016 | 38 | 32 / 6 | 709 | 360 | 0 | 279 | 345 |
| SQL Server full-grammer v2017 | 38 | 32 / 6 | 709 | 360 | 0 | 279 | 345 |
| SQL Server full-grammer v2019 | 38 | 32 / 6 | 709 | 360 | 0 | 279 | 345 |
| SQL Server full-grammer v2022 | 38 | 32 / 6 | 709 | 360 | 0 | 279 | 345 |
| SQL Server full-grammer v2025 | 38 | 32 / 6 | 709 | 360 | 0 | 279 | 345 |
...
1. 按 Microsoft Learn T-SQL 文档为 2016/2017/2019/2022/2025 增加真实版本边界 `.g4` 差异和负向 fixture。
