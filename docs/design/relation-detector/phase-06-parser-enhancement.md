...
- `token-event`：生产兜底解析链路。ANTLR 作为底层 lexer/parser 支撑；common 使用 portable typed grammar，MySQL/PostgreSQL 使用方言 typed structural grammar，再显式附加 legacy token-event supplement 补迁移期缺口。
...
| `core.tokenevent` | token-event 事件来源：common typed grammar、方言 typed parser 生命周期、legacy supplement 和结构事件模型。 |
...
MySqlTokenEventStructuredSqlParser / PostgresTokenEventStructuredSqlParser
  ...
  -> explicit legacy supplement builder
  -> StructuredParseResult(events, warnings, attributes)
```

`CommonRelationSql.g4` 是无方言信息时的 portable SQL subset。`MySqlRelationSql.g4` / `PostgresRelationSql.g4` 是方言 token-event typed structural grammar：先覆盖 common subset，再补各自常用语法边界。`MySqlTokenEventSqlEventBuilder` / `PostgresTokenEventSqlEventBuilder` 仍保留为 `legacySupplementBuilder`，用于迁移期补齐 typed visitor 暂未覆盖的方言结构；它的参与必须通过 parse result attributes 可见，不能静默混成 typed parse-tree 事件。
...
当前生产兜底 token-event SQL 链路已经从单纯 token scanner 前移到 typed structural grammar。common token-event 使用 `CommonRelationSql.g4`，MySQL/PostgreSQL token-event 使用各自方言 typed grammar；旧 token scanner 以显式 `legacySupplementBuilder` 形式保留，服务日志截断、routine/object 混合语法、动态 SQL、局部方言语法和不完整 SQL 的迁移期兜底。token scanner 只能作为事件来源，不应承载 relationship scoring，也不能用名字或关键字白名单决定业务关系。

full-grammer 链路的目标是用版本化 `.g4` 和 typed parse-tree visitor 替代 token scanner 的结构识别。当前 SQL full-grammer event path 已由 MySQL/PostgreSQL parse-tree visitor 与 `FullGrammerTypedSqlEventSink` 生成事件。DDL full-grammer path 由 adaptor-local DDL event collector 生成 DDL events，不委托 token-event DDL visitor。运行时选中 full-grammer 时，它就是当前 SQL/DDL 的 primary parser；token-event 只在 profile 选不中、版本不支持或 hard failure 时作为 fallback。默认测试只验证 full-grammer 输出行为和 parity；实现来源诊断属性只用于 debug/code review。
...
- 新增 full-grammer profile，必须补 profile selection 测试、对应版本 fixture，以及 parity 回归测试。
- 新增方言专属能力，必须补反向负向测试。
