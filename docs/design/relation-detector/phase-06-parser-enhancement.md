...
`CommonRelationSql.g4` 是无方言信息时的 portable SQL subset。`CommonDatabaseAdaptor` 将它接入 Java SPI 和 CLI，因此 `database.type: common` 会走完整 `ScanEngine`、naming evidence、lineage、derived path 和 JSON 输出链路。`MySqlRelationSql.g4` / `PostgresRelationSql.g4` / `OracleRelationSql.g4` 是 adaptor-local 方言 token-event typed structural grammar：先覆盖 common subset，再补各自常用语法边界。当前生产事件由 typed parse-tree visitor 生成，不再通过 token-span scanner、旧补充路径或表名/列名特殊过滤补事件。
...
- `derivedRelationships`：只以列级 `FK_LIKE` relationship 为主边，可用 top-level `namingEvidence` 作为辅助边；纯 naming path 不能生成 relationship。
