...
- `token-event`：生产兜底解析链路。ANTLR 作为底层 lexer/parser 支撑；common 使用 portable typed grammar，MySQL/PostgreSQL/Oracle/SQL Server 使用各自 adaptor-local 方言 typed structural grammar，事件由 typed parse-tree visitor 生成。common 同时通过 `database.type: common` 暴露为正式 CLI parser category，自然 benchmark 入口为 `sample-data/common-natural`，parser coverage 入口为 `sample-data/common-parser-coverage`；它不是任何方言 full-grammer 或 token-event 的 fallback facade。
...
- full-grammer adaptor 只能从 typed generated context 和 `FullGrammerParseTreeAdapter` 的 typed accessors 取得列、rowset、function、operator、constraint 和 child context。禁止按 grammar rule name、反射、terminal leaf text 或 raw SQL regex 推断结构；`DialectFunctionSemanticRegistry` 只对解析出的精确 function symbol 分类。
- parser 输出保留 SQL 显式写出的 catalog、schema、quote 和标识符拼写；`ScanScope` namespace 只能作为内部 `CanonicalEndpointKey` 跨 source 匹配的解析上下文。它不得改写 `TableId` / `ColumnRef` / `Endpoint` 的可读输出，也不得把 bare endpoint 与 schema-qualified endpoint 自动等价。
- `CanonicalIdentifierResolver` 只使用 dialect `IdentifierRules` 与显式 `NamespaceContext` 构造内部 key；它不是 naming heuristic，也不是输出格式化器。
...
core/src/main/java/com/relationdetector/core/script
  ScriptFileExtractor / StructuredScriptParser / CommonScriptParser
...
flowchart TD
  ...
  ddlFiles --> ddlScripts["AdaptorParsers.scripts()"]
  ...
  scriptInput --> scriptParser["AdaptorParsers.scripts()"]
  ...
  noise --> sqlRel["TokenEventRelationExtractor"]
...
flowchart TD
  ...
  log --> scripts["AdaptorParsers.scripts()"]
