...
- `parser.mode=full-grammar`：显式要求 full-grammar。profile 缺失、版本不支持或 full-grammar hard failure 时 warning + token-event fallback。profile 已选中后的 syntax warning / partial result 仍属于 full-grammar 结果；只有抛异常、无有效 structured result 或明确 hard failure 才 fallback。
