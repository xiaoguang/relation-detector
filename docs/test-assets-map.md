...
| `FullGrammerCorrectnessShadowTest` | SQL full-grammer parity / baseline grammar | 扫描无版本 manifest 的 SQL correctness fixture，使用 MySQL 8.0 / PostgreSQL 16 默认 profile，断言 relationship 与 Data Lineage 不少于 token-event fallback 输出；`postgres/v17`、`postgres/v18` 由 `CorrectnessFixtureRunnerTest` 按各自 profile/golden 验收 |
| `FullGrammerDdlCorrectnessShadowTest` | DDL full-grammer parity / baseline grammar | 扫描无版本 manifest 的 DDL correctness fixture，使用 MySQL 8.0 / PostgreSQL 16 默认 profile，断言不低于 token-event DDL 输出；`postgres/v17`、`postgres/v18` 由 `CorrectnessFixtureRunnerTest` 按各自 profile/golden 验收 |
...
| full-grammer 是否覆盖 PostgreSQL 17/18 version fixture | 是 | `CorrectnessFixtureRunnerTest` 扫描 `postgres/v17`、`postgres/v18`，manifest 强制 `parserMode: full-grammer` 和对应 `grammarProfile`，比对独立 version golden |
