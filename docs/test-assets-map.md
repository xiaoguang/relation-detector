...
版本化 full-grammer 当前状态：已建立 SQL full-grammer shadow 和 DDL full-grammer shadow。MySQL 当前 module 为 `mysql-8.0`，PostgreSQL 当前 module 为 `postgresql-16`，两者已 vendor grammars-v4 full-grammer 并通过 profile parser 运行真实 entry rule。具体实现归属 adaptor：MySQL 在 `adaptor-mysql` 的 `com.relationdetector.mysql.fullgrammer.v8_0`，PostgreSQL 在 `adaptor-postgres` 的 `com.relationdetector.postgres.fullgrammer.v16`；版本由 package 表达，类名不带 `80` / `16`。profile 选择由 core 的 `FullGrammerDialectModule` registry 注入，人工配置 `parser.grammarProfile` 优先，之后可使用 `parser.databaseVersion` 或 JDBC metadata；同一 major 的 minor 默认复用该 major profile，最多只允许高 1 个 major 的临时降级，完全没有方言/版本信息时回退 token-event。SQL/DDL shadow parser 由 adaptor module 通过 `ServiceLoader` 注入；shadow 用于证明 full-grammer profile 不少于 production token-event gold，missing 必须修 visitor/collector，extra 进入审核。

命名边界：`parser.mode` 是系统运行模式，只允许 `auto|full-grammer|token-event`；MySQL `SQL_MODE` 是 MySQL full-grammer runtime 的语法开关，由 `MySqlGrammarSqlMode` / `MySqlGrammarSqlModes` 表达；ANTLR lexer mode 是 `.g4` 内部词法状态。测试和文档不要把这三类 mode 混用。adaptor 根包只保留 database adaptor 装配类；MySQL/PostgreSQL production token-event parser 分别放在 `mysql.tokenevent` / `postgres.tokenevent` 子包。
...
| `FullGrammerCorrectnessShadowTest` | SQL full-grammer shadow / versioned grammar | 扫描全部 SQL correctness fixture，使用 MySQL 8.0 / PostgreSQL 16 grammar profile 跑 full-grammer shadow，并断言 relationship 与 Data Lineage 不少于 production token-event 输出 |
| `FullGrammerDdlCorrectnessShadowTest` | DDL full-grammer shadow / versioned grammar | 扫描全部 DDL correctness fixture，使用 MySQL 8.0 / PostgreSQL 16 full-grammer DDL shadow parser 跑 relationship parity，断言不低于 production token-event DDL 输出 |
...
| full-grammer shadow 是否覆盖全部 SQL fixture | 是 | `FullGrammerCorrectnessShadowTest` 扫描全部 SQL correctness fixture；MySQL 8.0 / PostgreSQL 16 full-grammer shadow 与 production token-event 保持 parity，且 `FullGrammerNativeRelationEventsTest` 要求 `fullGrammerBridgedEventTypes=[]`，后续深化 typed parse-tree visitor 时不得减少关系或血缘 |
| full-grammer shadow 是否覆盖全部 DDL fixture | 是 | `FullGrammerDdlCorrectnessShadowTest` 扫描全部 DDL correctness fixture；MySQL 8.0 / PostgreSQL 16 full-grammer DDL shadow 与 production token-event DDL relationship 保持 parity，不改变 production DDL parser 或 golden |
