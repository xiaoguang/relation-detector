...
token-event 当前状态：Common portable SQL 已通过 `database.type: common` 成为正式 CLI parser category，可运行 `sample-data/portable`；MySQL/PostgreSQL/Oracle/SQL Server SQL relation、Data Lineage 和 DDL 均使用 token-event 作为无 profile 或 unsupported version 时的正式 fallback；Oracle 使用 `adaptor-oracle` 自己的 `OracleRelationSql.g4` 与 typed visitor，SQL Server 使用 `adaptor-sqlserver` 自己的 compact `SqlServerRelationSql.g4` 与 typed visitor，二者都不复用 common parser 的装配入口。公共 relation、rowset/scope、Data Lineage 写入映射、derived aggregate projection 回溯、显式临时表过滤、MySQL/PostgreSQL/Oracle/SQL Server DML 深水区、方言 rowset 防伪表都有事件/抽取测试。profile 已选中后的 full-grammer parse warning / partial result 不会委托 token-event 补事件。
