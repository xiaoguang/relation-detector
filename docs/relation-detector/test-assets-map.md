...
| `mysql/v5_7/*` | SQL/DDL versioned full-grammar | 89 个 MySQL 5.7 strict version golden；manifest 强制 `parserMode: full-grammar`、`grammarProfile: mysql/5.7`；包含 MySQL 8.0 fixture 的 5.7 原样兼容、语义等价改写和 8.0-only 版本边界负向用例 |
| `mysql/v8_0/*` | SQL/DDL versioned full-grammar | 89 个 MySQL 8.0 strict version golden；manifest 强制 `parserMode: full-grammar`、`grammarProfile: mysql/8.0`；相对 root MySQL token-event 的差异记录在 `docs/parser-audit/all-golden-semantic-review.md` |
