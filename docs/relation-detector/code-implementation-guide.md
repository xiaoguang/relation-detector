...
- SQL 与 DDL correctness fixture golden 仍保护正式输出。root baseline manifest 显式走 token-event；versioned manifest 显式走 full-grammer。full-grammer parity 测试保留为回归护栏，证明 baseline profile 不少于 token-event gold，但运行时 full-grammer 已是被选中 profile 的 primary parser。SQL fixture 证明 SQL relation extractor 行为，DDL fixture 证明 DDL event visitor 与 DDL relation extraction 行为；禁止用其中一条链路的通过结果替代另一条链路验收。
...
- 按 `SqlGrammarProfile` 继续引入 MySQL/PostgreSQL 大版本 full-grammer module。新增 profile 必须补 profile selection、versioned fixture，并通过 full-grammer parity 证明不低于 token-event；无方言、无版本信息、unsupported version 或 full-grammer hard failure 时仍使用 token-event fallback。
