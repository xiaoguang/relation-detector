...
- SQL parser 由 `SqlRelationParserRunner` 统一调度到数据库 adaptor 暴露的 Token/Event SQL parser。`parser.sql.mode`、`simple`、`antlr-shadow` 和 simple fallback 已移除；Token/Event 解析硬失败由 `ScanEngine` 记录 `SQL_PARSE_FAILED` warning，并继续扫描其它语句。
...
- 持续增强 MySQL/PostgreSQL ANTLR grammar 和 visitor，逐步把剩余 regex 兜底替换为 parse-tree visitor。
