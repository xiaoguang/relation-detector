...
- `RelationExtractionVisitor` 里仍存在少量 regex/scanner，这是 ANTLR 渐进迁移期的语义兜底，不表示 ANTLR primary 仍在委托 Simple parser。允许的用途仅限 tuple/IN/raw equality、`JOIN USING` 表级共现、CTE/derived/local-temp rowset 防伪表、系统 schema/截断 token 过滤等关系语义层能力。新增或修改这些 regex 必须同时补 correctness fixture，并在 `docs/design/phase-06-parser-enhancement.md` 更新过渡边界；未来如果改成完整 parse-tree visitor，也必须同步删除对应 regex 说明。
