...
- `SqlRelationParserRunner` 与 `DdlRelationParserRunner` 不再各自重复 profile selection。SQL 链路通过同一个 structured parse result 同时服务 relationship extraction 与 Data Lineage extraction，避免同一条 SQL 在正常路径上重复解析。production runner、direct `StatementExecutionService` overload 和 `StructuredSqlRelationshipParser` facade 都经 `StructuredSqlParseExecutor` 隔离 context，并在提交 warning 或抽取事实前原子校验 event family payload、statement provenance 和 parser result 容器。
...
- MySQL `MySqlFullGrammarGeneratedParserSmokeTest` 与 PostgreSQL 各版本
  `PostgresFullGrammarGeneratedParserSmokeTest` 验证 generated lexer/parser 可实例化并解析基础 SQL；
  对应 `*FullGrammarVersionBoundaryTest` 锁定版本边界。Oracle adaptor 由 `OracleAdaptorParserTest` 和
  `OracleParserArchitectureTest` 验证 ServiceLoader、Oracle token-event grammar、
  `INCOMPLETE_VERSIONED` full-grammar generated parser attributes、“不得持有 token-event delegate”和
  Oracle SQL 资产卫生边界。full-grammar 行为测试只验证具体 SQL/DDL 的关系、血缘、warning 行为；
  不再把内部事件来源标签作为默认测试目标，也不再用 token-event baseline 兜底验证 full-grammar。
  后续 profile 深化 parse-tree visitor 后，missing 必须修 visitor，extra 进入审核，不自动写入 golden。
...
- parser/framer 保留 SQL 显式 catalog、schema、quote 和标识符拼写。bare table 在 production scan 已有唯一、规范的 `ScanScope` 或 source-object namespace 时，可以通过 `CanonicalIdentifierResolver` 物化为完整 `TableId` / `ColumnRef` / `Endpoint`；没有唯一上下文时保持 bare。显式限定名不得被覆盖，bare 不能按名称与任意 qualified endpoint 等价。`CanonicalEndpointKey` 继续负责跨 metadata、DDL、SQL source 的 exact identity，不进入 JSON 的独立字段；namespace materialization 是 identifier resolution，不是 naming inference。
...
- `JsonResultWriter` 使用 Jackson 输出稳定 JSON。顶层包含 `relationships`、`dataLineages`、`derivedRelationships`、`derivedDataLineages`、`namingEvidence`、`derivedNamingEvidence`、`warnings`。direct relationship / lineage / naming evidence 使用 `rawEvidence` + grouped `evidence` 双层结构；derived relationship / lineage 使用`evidenceSets`保存结构hop与每跳直接支持集合，不输出计算型`rawEvidence`笛卡尔积。`NAMING_MATCH` 在 relationship 里只保存 `evidenceRef` 和轻量摘要。`derivedNamingEvidence` 只是 `TRANSITIVE_NAMING_PATH` 的阅读/统计视图，完整 evidence 仍通过相同 id 到 top-level `namingEvidence` 查询。summary 对三类事实提供统一的 direct/derived/total fact count；direct侧统计真实observation，derived侧统计evidence-set及support-combination数量，两者均不参与confidence或事实判断。
